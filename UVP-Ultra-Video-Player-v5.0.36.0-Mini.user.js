// ==UserScript==
// @name         UVP Ultra Video Player Mini
// @namespace    uvp-native-overlay
// @version      5.0.36.0
// @description  Mini variant of UVP Native Overlay Player — identical features to Full, without baked-in hls.js/dash.js. Libraries load on-demand from CDN with SHA-384 SRI (no @require). Isolated hardware-accelerated overlay, native controls, PiP, and local remuxing for publicly accessible web streams.
...
(function () {
  ...
  // Unprivileged documents: no location to bind interceptors to.
  ...
  // Frames: @noframes should already skip injection. A frame instance cannot
  // hand capturedUrls to the parent (separate realm). Parent places the ▶ on
  // the iframe element in THIS document. Do not use frame viewport size here
  // — at document-start it is often 0.
  try {
    if (typeof window.top !== 'undefined' && window.top !== null && window.self !== window.top) return;
  } catch (e) { return; }
  ...
  // ==================== SEGMENT CACHE ====================
  // Bounded LRU cache for HLS segment ArrayBuffers. Active only when
  // enablePlaybackPrefetch is true. Generation guard prevents stale stores
  // after teardown/seek/level-switch.
  UVP.SegmentCache = {
    _cache: new Map(),
    _bytes: 0,
    maxBytes: 0,
    ...
    _inFlight: new Map(),
    _generation: 0,
    init(maxBytes) { this.maxBytes = maxBytes; this.clear(); },
    clear() { this._generation++; this._abortInFlight(); this._cache.clear(); this._bytes = 0; },
    getGeneration() { return this._generation; },
    get(key) {
      if (!this._cache.has(key)) { this._misses++; return null; }
      const val = this._cache.get(key);
      this._cache.delete(key); this._cache.set(key, val);
      this._hits++;
      this._bytesServed += val.byteLength;
      return val;
    },
    has(key) { return this._cache.has(key); },
    set(key, val) {
      if (this._cache.has(key)) this._bytes -= this._cache.get(key).byteLength;
      this._cache.set(key, val);
      this._bytes += val.byteLength;
      this._bytesStored += val.byteLength;
      while (this._bytes > this.maxBytes && this._cache.size > 0) {
        const oldest = this._cache.keys().next().value;
        this._bytes -= this._cache.get(oldest).byteLength;
        this._cache.delete(oldest);
      }
    },
    evictByLevelPrefix(levelUrl) {
      if (!levelUrl) return;
      let prefix = '';
      try { prefix = new URL(levelUrl, location.href).pathname.replace(/[^/]+$/, ''); } catch (e) { return; }
      for (const key of Array.from(this._cache.keys())) {
        try {
          const kp = new URL(key, location.href).pathname;
          if (kp.startsWith(prefix) && kp !== prefix) {
            this._bytes -= this._cache.get(key).byteLength;
            this._cache.delete(key);
          }
        } catch (e) {}
      }
    },
    evictByUrlSet(validUrls) {
      for (const key of Array.from(this._cache.keys())) {
        if (!validUrls.has(key)) {
          this._bytes -= this._cache.get(key).byteLength;
          this._cache.delete(key);
        }
      }
    },
    trackInFlight(url, ctrl) { this._inFlight.set(url, ctrl); },
    untrackInFlight(url) { this._inFlight.delete(url); },
    abortInFlight() { this._abortInFlight(); },
    _abortInFlight() {
      for (const ctrl of this._inFlight.values()) { try { ctrl.abort(); } catch (e) {} }
    ...
    getMetrics() {
      ...
      return {
        ...
        bytesServed: (this._bytesServed >> 20) + 'MB',
        bytesStored: (this._bytesStored >> 20) + 'MB',
        currentBytes: (this._bytes >> 20) + 'MB',
  ...
  // BroadcastChannel-based cross-tab coordination. Only active when prefetch
  // is enabled AND an overlay is open. Prefetch depth is driven by network
  // class (getDeviceProfile.netClass), NOT tab count — each tab's video needs
  // the same buffer regardless of how many tabs are open. Zero overhead when
  // prefetch is off.
  UVP.MultiTab = {
    _channel: null,
    _tabId: null,
    _peers: new Map(),        // tabId → { seen, cacheMaxBytes }
    _heartbeat: null,
    _cacheMaxBytes: 0,
    _active: false,           // true only when an HLS overlay is open
    init() {
      if (this._channel) return;
      this._tabId = (typeof crypto !== 'undefined' && crypto.randomUUID)
        ? crypto.randomUUID()
        : (Date.now() + '-' + Math.random());
      this._peers = new Map();
      this._active = false;
      try {
        this._channel = new BroadcastChannel('uvp-prefetch-coord');
        this._channel.onmessage = (e) => this._onMessage(e.data);
        if (DEBUG) console.log('[UVP] MultiTab coordinator initialized, tabId:', this._tabId);
      } catch (e) {
        this._channel = null;
        if (DEBUG) console.warn('[UVP] BroadcastChannel unavailable, multi-tab coordination disabled:', e);
      }
    },
    destroy() {
      if (this._heartbeat) { clearInterval(this._heartbeat); this._heartbeat = null; }
      if (this._channel) {
        if (this._active) { try { this._channel.postMessage({ type: 'leave', tabId: this._tabId }); } catch (e) {} }
        try { this._channel.close(); } catch (e) {}
      }
      this._channel = null;
      this._peers = new Map();
      this._tabId = null;
      this._cacheMaxBytes = 0;
      this._active = false;
    },
    // Called when HLS overlay opens. Announces to peers and starts heartbeat.
    activate() {
      if (!this._channel || this._active) return;
      this._active = true;
      try { this._channel.postMessage({ type: 'join', tabId: this._tabId, cacheMaxBytes: this._cacheMaxBytes }); } catch (e) {}
      this._heartbeat = setInterval(() => {
        if (this._channel) {
          try { this._channel.postMessage({ type: 'heartbeat', tabId: this._tabId, cacheMaxBytes: this._cacheMaxBytes }); } catch (e) {}
        }
        this._prune();
      }, 5000);
      if (DEBUG) console.log('[UVP] MultiTab activated, cacheMaxBytes:', this._cacheMaxBytes);
    },
    // Called when overlay closes or hard-resets. Posts leave, stops heartbeat.
    deactivate() {
      if (!this._channel || !this._active) return;
      this._active = false;
      if (this._heartbeat) { clearInterval(this._heartbeat); this._heartbeat = null; }
      try { this._channel.postMessage({ type: 'leave', tabId: this._tabId }); } catch (e) {}
      if (DEBUG) console.log('[UVP] MultiTab deactivated');
    },
    setCacheMaxBytes(bytes) { this._cacheMaxBytes = bytes; },
    _onMessage(data) {
      if (!data || !data.type || !data.tabId || data.tabId === this._tabId) return;
      switch (data.type) {
        case 'join':
          this._peers.set(data.tabId, { seen: Date.now(), cacheMaxBytes: data.cacheMaxBytes || 0 });
          if (this._active) { try { this._channel.postMessage({ type: 'heartbeat', tabId: this._tabId, cacheMaxBytes: this._cacheMaxBytes }); } catch (e) {} }
          break;
        case 'heartbeat':
          this._peers.set(data.tabId, { seen: Date.now(), cacheMaxBytes: data.cacheMaxBytes || (this._peers.get(data.tabId) || {}).cacheMaxBytes || 0 });
          break;
        case 'leave':
          this._peers.delete(data.tabId);
          break;
      }
    },
    _prune() {
      const now = Date.now();
      for (const [id, info] of this._peers) {
        if (now - info.seen > 15000) this._peers.delete(id);
      }
    },
    getActiveTabs() { return this._peers.size + 1; },
    // Prefetch depth driven by network class, not tab count. Each tab's video
    // needs the same lookahead regardless of how many tabs are open.
    getPrefetchDepth() {
      var nc = UVP.Utils.getDeviceProfile().netClass;
      var byNet = CONFIG.prefetchAheadByNetClass || { low: 1, mid: 2, high: 4 };
      return Math.max(1, Math.min(CONFIG.prefetchAheadSegments, byNet[nc] || 2));
    }
  };
  ...
  // Read persisted debug-logging preference; defaults to false (off).
  // Toggled at runtime via the "UVP: Toggle debug logging" menu command.
  let DEBUG;
  try { DEBUG = (typeof GM_getValue === 'function') ? GM_getValue('uvp-debug', false) : false; }
  catch (e) { DEBUG = false; }
  ...
  // Evidence-based device profiling. Returns three independent classes:
  //   memClass — drives buffer sizing. Priority: performance.memory.
  //     jsHeapSizeLimit (actual V8 heap ceiling, far more precise than
  //     navigator.deviceMemory which is capped at 8GB and Chrome-only)
  //     → navigator.deviceMemory → 'mid' default.
  //   cpuClass — drives worker/parallelism decisions (enableWorker).
  //     Based on navigator.hardwareConcurrency (logical cores). NOT a
  //     memory fallback — cores and RAM are independent dimensions.
  //   netClass — drives prefetch aggressiveness. Based on
  //     navigator.connection.effectiveType + saveData. Updated dynamically
  //     via connection.change listener (set up in MultiTab.init).
  // Never used for ABR level selection or pinning.
  UVP.Utils._deviceProfile = null;
  UVP.Utils.getDeviceProfile = function() {
    if (UVP.Utils._deviceProfile) return UVP.Utils._deviceProfile;
    var memClass = 'mid', cpuClass = 'mid', netClass = 'mid';
    try {
      if (performance && performance.memory && performance.memory.jsHeapSizeLimit) {
        var heapMB = performance.memory.jsHeapSizeLimit >> 20;
        if (heapMB <= 256) memClass = 'low';
        else if (heapMB <= 512) memClass = 'mid';
        else memClass = 'high';
      } else if (navigator.deviceMemory) {
        if (navigator.deviceMemory <= 2) memClass = 'low';
        else if (navigator.deviceMemory <= 4) memClass = 'mid';
        else memClass = 'high';
      }
    } catch (e) {}
    try {
      if (navigator.hardwareConcurrency) {
        if (navigator.hardwareConcurrency <= 2) cpuClass = 'low';
        else if (navigator.hardwareConcurrency <= 4) cpuClass = 'mid';
        else cpuClass = 'high';
      }
    } catch (e) {}
    try {
      ...
      if (conn) {
        if (conn.saveData) netClass = 'low';
        else if (conn.effectiveType) {
          if (conn.effectiveType === 'slow-2g' || conn.effectiveType === '2g') netClass = 'low';
          else if (conn.effectiveType === '3g') netClass = 'mid';
          else netClass = 'high';
        }
      }
    } catch (e) {}
    UVP.Utils._deviceProfile = { memClass: memClass, cpuClass: cpuClass, netClass: netClass };
    return UVP.Utils._deviceProfile;
  };
  // Backward-compat: existing buffer-sizing call sites use
  // getDeviceMemoryTier(). Returns memClass from the cached profile.
  UVP.Utils.getDeviceMemoryTier = function() {
    return UVP.Utils.getDeviceProfile().memClass;
  };
  ...
  const CONFIG = {
    ...
    buttonLabel: '',
    loopPlayback: true,
    rescanIntervalMs: 1000,         // unused by the button engine (kept for menu/docs); engine is rAF + video-appear wake
    findAllVideosCacheMs: 300,      // reuse DOM video list this long across a rescan + geometry apply
    jsonScanMaxChars: 65536,        // prefix of same-origin JSON scanned for media URLs
    jsonScanMaxContentLength: 262144, // skip clone+scan when Content-Length exceeds this
    minVideoWidth: 80,
    ...
    keyTimeoutMs: 8000,         // keys are tiny (16-32B); fail fast and escalate to recovery instead of waiting segTimeoutMs
    mp4TimeoutMs: 120000,
    stallThresholdSec: 8,
    playheadJumpSec: 15,         // larger currentTime delta in one poll is a seek/reset/live-edge snap, not playback
    maxConcurrentDownloads: 5,
    segMaxRetries: 3,
    // DASH per-resource loader profiles. Mirrors the HLS per-resource loader
    // pattern (ManifestLoader/FragmentLoader/KeyLoader) but applied dynamically
    // inside the XHRLoader wrap via dashResourceType().
    // MPD: tiny XML — fail fast so recovery/fallback escalates immediately.
    // Init: small but critical for playback start — moderate retries, shorter timeout.
    // Index/sidx: tiny — fail fast, 1 retry. Fragment: full segment budget.
    dashMpdRetries: 3,
    dashMpdTimeoutMs: 10000,     // mirrors manifestTimeoutMs
    dashInitRetries: 2,
    dashInitTimeoutMs: 12000,
    dashIndexRetries: 1,
    dashIndexTimeoutMs: 8000,
    dashSegmentRetries: 3,       // mirrors segMaxRetries
    dashSegmentTimeoutMs: 20000, // mirrors segTimeoutMs
    ...
    overlayMiniEnterDelayMs: 500,  // hold inline overlay this long after target looks gone (site pause/recycle flicker)
    ...
    pipExitDecisionMs: 250,       // Path B wait after leavepictureinpicture. Chrome pauses on PiP-window X only AFTER the event; back-to-tab and other-tab PiP steal do not pause.
    ...
    hlsMaxRecoveries: 3,          // max instant fatal-recovery attempts that produce NO buffered fragment before escalating
                                  // to overlay hard recovery; a recovery that yields a FRAG_BUFFERED resets the full budget
    ...
    hostRegistryTtlMs: 3600000,  // persistent host registry entry TTL (1h) — longer than the old in-memory TTL since it survives reloads
    hostRegistryFlushMs: 2000,   // debounce window for flushing registry to GM storage (batches burst of segment loads)
    hostScorePageThreshold: 0.8, // Bayesian confidence (pageOk+1)/(total+2) >= this → use page-transport (fast path)
    hostScoreGmThreshold: 0.3,    // confidence < this AND opaque-failure dominance → use GM_xhr (CORS-type evidence only; HTTP-statused failures never pin GM)
    hostScoreHalfLifeMs: 1800000, // outcome counters decay with a 30min half-life — stale evidence fades and hosts self-heal back to 'probe'
    hostReprobeMs: 600000,       // a gm-classified host gets one page-transport re-probe per window instead of waiting out the TTL
    hostOpaqueGmMinFails: 2,     // minimum statusless (CORS-type) failures before a host may classify as gm
    manifestCacheTtlMs: 5000,    // in-memory manifest cache TTL; avoids re-fetching unchanged playlists during recovery / level switches
    ...
    dashSeekGraceMs: 30000,      // Attach only (!fillSettled). After first playhead fill, isDashInGrace uses evidence + budget.
    dashSeekMaxGraceMs: 45000,   // Hard cap from seekStartTime. Not the MSE attach watchdog.
    dashSeekRequestBudgetMs: 8000, // No-evidence window after seekStartTime for dash.js to issue the first XHR.
    dashQuotaClearGraceMs: 15000, // After MSE QuotaExceeded / playback-buffer clear, do not stall-escalate while dash.js prunes and refills.
    dashProgressExtendMs: 8000,  // Fill evidence (loader/progress/fragment) must stay this recent to keep grace.
    ...
    dashPlayMinBufferSec: 1.5,   // VOD DASH play() / recovery "enough buffer" gate at the playhead. Not the live window.
    abrSwitchGraceMs: 3000,      // stall monitor ignores playhead freezes this long after an ABR level switch (init + first media of the new rung)
    // ---- Quality mode: 'max' pins the top video rung; 'auto' is engine ABR ----
    // A pin (hls.js nextLevel/loadLevel, dash.js setQualityFor) is intentional
    // in 'max'. 'auto' leaves nextLevel / autoSwitchBitrate alone.
    abrHlsUpSwitchFactor: 0.8,          // stock 0.7 needs 1.43x headroom; 0.8 needs 1.25x. 0.9 climbed on one fat sample
    abrHlsMaxStarvationDelaySec: 8,     // stock 4; bigger starved-buffer budget keeps a high level worth trying
    abrHlsMaxLoadingDelaySec: 4,        // stock 4 (explicit)
    abrHlsStartupEstimateBps: 1500000,  // stock 500kbps starts at the bottom. 3Mbps jumped straight to 1080p before a real sample
    abrHlsBackBufferSec: 90,            // VOD only. stock Infinity grows back-buffer memory unbounded on long VOD
    abrHlsMaxBufferBytes: 100663296,    // stock 60MB truncates the 60s forward buffer at 4K (~12-15s of runway)
    // Live HLS: liveDurationInfinity uses liveMaxBackBufferLength, NOT
    // backBufferLength. Stock liveMaxBackBufferLength is Infinity — hours of
    // live would keep every segment behind the playhead.
    abrHlsLiveMaxBufferSec: 60,         // match Healthy v5.0.12.2 proven value. Stock hls.js maxBufferLength. Must stay > live sync (~3 segments).
    abrHlsLiveMaxMaxBufferSec: 120,     // bounded cap; stock 600. 36s too aggressive for live headroom.
    abrHlsLiveBackBufferSec: 24,        // bounded; stock liveMaxBackBufferLength is Infinity
    // Device-aware buffer tiers (live/Chaturbate overrides take precedence).
    abrHlsMaxBufferLengthByTier:      { low: 30, mid: 60, high: 60 },
    abrHlsMaxMaxBufferLengthByTier:   { low: 60, mid: 120, high: 120 },
    abrHlsMaxBufferBytesByTier:       { low: 67108864, mid: 100663296, high: 167772160 },
    abrHlsBackBufferSecByTier:        { low: 30, mid: 60, high: 90 },
    abrDashBufferToKeepByTier:         { low: 15, mid: 30, high: 30 },
    abrDashBufferTimeDefaultByTier:   { low: 12, mid: 24, high: 24 },
    abrDashBufferTimeTopQByTier:      { low: 20, mid: 36, high: 36 },
    abrDashLiveBufferToKeepByTier:    { low: 10, mid: 20, high: 20 },
    abrDashLiveBufferTimeDefaultByTier: { low: 12, mid: 18, high: 18 },
    enablePlaybackPrefetch: false,       // Segment cache + prefetch (off by default)
    prefetchAheadSegments: 4,           // Max segments to prefetch ahead (capped by netClass)
    prefetchAheadByNetClass: { low: 1, mid: 2, high: 4 }, // Prefetch depth by network quality (getDeviceProfile.netClass)
    abrDashSafetyFactor: 0.85,         // same as stock dash.js. 0.9 treated one inflated GM sample as a fat pipe
    abrDashInitBitrateFraction: 0.35,  // startup rung = highest ladder bitrate <= 35% of the top (720p on a 1080/4K ladder)
    abrDashInitBitrateFallbackKbps: 3600, // external MPDs with no known ladder
    // Leftover knob. Portal cap OFF. ABR is throughput-only.
    abrMaxDevicePixelRatio: 2,
    ...
    hlsStartupBufferGraceMs: 20000, // VOD attach watchdog + first-segment flowing grace
    attachLoadingMaxMs: 120000, // attach watchdog only: LOADING after a real stamp may wait this long. Not maxBufferLength.
    // First-click readiness gate (YouTube): player libraries load on-demand
    // (CDN fetch + SHA-384 SRI + page-realm eval), so the interception wrappers
    // are ready at document-start without @require delay. The bounded waits
    // below handle the remaining race: the InnerTube client ladder and the
    // n-sig solver may not have completed when the user clicks immediately
    // after page load — the overlay would open against missing or still-
    // locked formats and stall. On timeout the original behavior applies.
    ...
    liveEngineWaitMs: 12000,   // live attach watchdog (attachWaitCapMs). VOD uses hlsStartupBufferGraceMs.
    liveFragmentFreshMs: 15000, // HLS target duration is commonly 2–10s; reload gap can exceed 8s
  };
  ...
  const state = {
    ...
    liveDetectReason: null,    // who set overlayIsLive (hls-level-loaded can be cleared on VOD)
    buttonDismissed: false, buttonExtracting: false, targetVideo: null, syncInterval: null,
    ...
    backgroundStoppedLoad: false, // true only when the teardown pause (freeze/pagehide) actually ran hls.stopLoad()
    ...
    dashFailover: null,
    overlayGeneration: 0,
    isDetachedPip: false,       // overlay box hidden; this document's overlay <video> is in the system PiP window
    pipExitByToggle: false,     // Path A: set by our PiP button before exitPictureInPicture(); leave handler must not use the 250ms pause test
    isMiniPlayer: false,
    _activePlaybackSource: null,
    _activePlaybackFrame: null, // iframe this overlay was opened from (parent document)
    _activePoster: null,
    _activeRgId: null,
    buttonTargetVideo: null,
    buttonTargetFrame: null,    // play-button target when this document has no host <video>
    playButton: null,
    playButtonHost: null,
    buttonHiddenByScroll: false,
    // Ranked playback candidates (generic sites): [primary, ...format-class
    // alternates]. No pre-play probe. recoverPlayer walks untried alternates.
    playbackCandidates: [],
    playbackFailedCandidates: null,
    deferredSeekTo: null,       // site/resume time applied only after the first MSE buffer exists
    bufferHold: false,          // paused only to fill playhead buffer; not a user pause
    abrSwitchAt: 0,             // last ABR level-switch timestamp (stall grace)
    enginePlaybackReady: false, // HLS: FRAG_BUFFERED. DASH uses dashActivity.engineReady instead
    engineWaitStartedAt: 0,     // playVideo / hardReset start of the current attach window
    engineEscalated: false,     // attach watchdog already fired for this attach window
    bufferQuotaCooldown: 0,    // timestamp of last handleBufferQuota call; prevents rapid re-trigger
  };
  ...
  if (typeof window !== 'undefined' && window.addEventListener) {
    window.addEventListener('pointermove', (e) => {
      ...
      // Geometry vs retarget: hovering the current box only reapplies rects.
      // A parked engine, or a move that is not on the current box, retargets.
      if (pointerUpdateTimer || state.overlay || state.buttonHiddenByScroll) return;
      if (!UVP.Overlay || !UVP.Overlay.retargetPlaybackUi) return;
      if (!state.buttonSyncFrame) {
        // Throttle pointer-triggered discovery: when the last retarget found
        // no target, skip for findAllVideosCacheMs to avoid 60 Hz full-page scans.
        const _now = Date.now();
        if (UVP.Overlay._pointerRetargetThrottle && (_now - UVP.Overlay._pointerRetargetThrottle) < CONFIG.findAllVideosCacheMs) return;
        pointerUpdateTimer = requestAnimationFrame(() => {
          pointerUpdateTimer = null;
          if (!state.overlay && !state.buttonHiddenByScroll) {
            UVP.Overlay.retargetPlaybackUi();
            // If still no target after retarget, set throttle.
            if (!state.buttonSyncFrame) UVP.Overlay._pointerRetargetThrottle = Date.now();
          }
        });
        return;
      }
      ...
      const btn = UVP.Overlay.getPlayButton();
      const target = state.buttonTargetVideo || state.targetVideo || state.buttonTargetFrame;
      if (!hit(btn) && !hit(target)) return;
      pointerUpdateTimer = requestAnimationFrame(() => {
        pointerUpdateTimer = null;
        if (!state.overlay && !state.buttonHiddenByScroll) UVP.Overlay.scheduleGeometry();
      });
    }, { passive: true });
  }
  ...
  //
  // In v5.0.8 the button was in the light DOM, so e.relatedTarget was the button
  // and the identity check (rel === btn) sufficed.  The BETA button lives inside a
  // closed shadow DOM attached to a host div (data-uvp-btn-host).  The browser
  // retargets e.relatedTarget to the shadow HOST element (not null, not the
  // button), so the identity check (rel === btn) always fails and the site
  // receives the exit event — deactivating the card, unmounting the video, and
  // causing the play button to oscillate between videos.  Fix: also check if
  // rel is the shadow host, and fall back to a coordinate hit-test against the
  // button's rect for any other retargeted/null case.
  if (typeof document !== 'undefined' && document.addEventListener) {
    const onHoverExitToButton = (e) => {
      const btn = UVP.Overlay.getPlayButton();
      if (!btn || btn.style.display === 'none') return;
      const rel = e.relatedTarget;
      // Light-DOM case (v5.0.8): relatedTarget is the button itself.
      if (rel && (rel === btn || btn.contains(rel))) {
        e.stopPropagation();
        e.stopImmediatePropagation();
        return;
      }
      // Closed shadow-DOM case: relatedTarget is retargeted to the shadow
      // host element (the data-uvp-btn-host div).  Check if rel is the host.
      if (rel && state.playButtonHost && rel === state.playButtonHost) {
        e.stopPropagation();
        e.stopImmediatePropagation();
        return;
      }
      // Fallback: coordinate hit-test.  Catches any remaining retargeting
      // edge case (null, window, etc.) by checking if the pointer coordinates
      // are inside the button's rendered rect.
      if (e.clientX != null) {
        try {
  ...
  // Light-DOM videos via querySelectorAll; open shadowRoots via a host walk
  // that does not re-collect light-DOM <video>s. Document-root results are
  // reused for findAllVideosCacheMs so 4 Hz findBestVideo is not a full walk.
  let _videoCache = { list: null, at: 0 };
  UVP.Utils.invalidateVideoCache = function() { _videoCache.list = null; _videoCache.at = 0; };
  UVP.Utils.findAllVideos = function(root) {
    root = root || document;
    const now = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
    if (root === document && _videoCache.list && (now - _videoCache.at) < CONFIG.findAllVideosCacheMs) {
      return _videoCache.list.filter((v) => v && v.isConnected && v !== state.video);
    }
    const results = [];
    const seen = new Set();
    const add = (v) => {
      if (!v || v === state.video || seen.has(v)) return;
      seen.add(v);
      results.push(v);
    };
    const collect = (node) => {
      ...
      if (node.querySelectorAll) {
        try { node.querySelectorAll('video').forEach(add); } catch (e) {}
      }
      const walkHosts = (el) => {
        if (!el) return;
        if (el.shadowRoot) collect(el.shadowRoot);
        let c = el.firstElementChild;
        while (c) { walkHosts(c); c = c.nextElementSibling; }
      };
      walkHosts(node);
    };
    try { collect(root); } catch (e) {}
    if (root === document) { _videoCache.list = results; _videoCache.at = now; }
    return results;
  };
  ...
  const UVP_TT = (() => {
    ...
    const candidateNames = ['uvp_trusted_policy'];
    const rules = {
      createScript: (s) => s,
      createScriptURL: (u) => u,
      createHTML: (h) => h,
  ...
  // ==================== SAFE CALL HELPERS ====================
  // Normalize Fetch Response / GM_xhr result / raw header map / header block
  // into a lowercase-key object. Used by host fingerprinting and the DASH
  // fallback so Content-Range / Content-Type / Content-Length survive.
  UVP.Utils.parseResponseHeaders = function(src) {
    const h = {};
    if (!src) return h;
    ...
    try {
      if (src.headers && typeof src.headers.forEach === 'function') {
        src.headers.forEach((v, k) => { h[String(k).toLowerCase()] = v; });
        return h;
      }
    } catch (e) {}
    if (typeof src.responseHeaders === 'string') return UVP.Utils.parseResponseHeaders(src.responseHeaders);
    if (typeof src === 'object') {
      try {
        for (const k of Object.keys(src)) {
          if (src[k] != null && typeof src[k] !== 'function') h[String(k).toLowerCase()] = String(src[k]);
        }
      } catch (e) {}
    }
    return h;
  ...
  let _jsonLdScripts = null;  // Cached JSON-LD script elements per extraction cycle
  UVP.Utils.invalidateUrlCache = function() { _urlCache = null; UVP.Utils.invalidateVideoCache(); }
  UVP.Utils.invalidateInlineScriptCache = function() { _inlineScripts = null; _jsonLdScripts = null; }
  ...
  UVP.Utils.getJsonLdScripts = function() {
    if (!_jsonLdScripts) _jsonLdScripts = Array.from(document.querySelectorAll('script[type="application/ld+json"]'));
    return _jsonLdScripts;
  }
  ...
  UVP.Overlay.noteEngineActivity = function() {
    lastFragmentFetch.at = Date.now();
  };
  UVP.Overlay.noteAbrSwitch = function() {
    state.abrSwitchAt = Date.now();
    UVP.Overlay.noteEngineActivity();
  };
  UVP.Overlay.isAbrSwitching = function(now) {
    const t = now || Date.now();
    return !!(state.abrSwitchAt && (t - state.abrSwitchAt) < (CONFIG.abrSwitchGraceMs || 3000));
  };
  // 'max' = lock the highest video quality. 'auto' = engine ABR.
  // Do not set hls.currentLevel on a live playhead — that flushes MSE and
  // freezes. Pin nextLevel/loadLevel so the switch lands on a segment edge.
  UVP.Overlay.applyHlsQualityMode = function(hls, mode) {
    if (!hls) return;
    const max = (mode || UVP.Utils.getPlaybackQuality()) === 'max';
    try {
      const levels = hls.levels || [];
      if (max && levels.length) {
        const top = levels.length - 1;
        try { hls.startLevel = top; } catch (e) {}
        try { hls.nextLevel = top; } catch (e) {}
        try { hls.loadLevel = top; } catch (e) {}
      } else {
        try { hls.nextLevel = -1; } catch (e) {}
        try { hls.loadLevel = -1; } catch (e) {}
        try { hls.startLevel = -1; } catch (e) {}
      }
    } catch (e) { if (DEBUG) console.warn('[UVP] applyHlsQualityMode:', e); }
  };
  UVP.Overlay.dashTopVideoQualityIndex = function(player) {
    if (!player) return 0;
    try {
      if (typeof player.getTopBitrateInfoFor === 'function') {
        const info = player.getTopBitrateInfoFor('video');
        if (info && typeof info.qualityIndex === 'number') return info.qualityIndex;
      }
    } catch (e) {}
    try {
      if (typeof player.getBitrateInfoListFor === 'function') {
        const list = player.getBitrateInfoListFor('video') || [];
        let bestIdx = 0, bestBr = -1;
        for (let i = 0; i < list.length; i++) {
          const br = Number(list[i] && (list[i].bitrate || list[i].bandwidth)) || 0;
          const qi = (list[i] && typeof list[i].qualityIndex === 'number') ? list[i].qualityIndex : i;
          if (br >= bestBr) { bestBr = br; bestIdx = qi; }
        }
        return bestIdx;
      }
    } catch (e) {}
  ...
  UVP.Overlay.applyDashQualityMode = function(player, mode) {
    if (!player || typeof player.updateSettings !== 'function') return;
    const max = (mode || UVP.Utils.getPlaybackQuality()) === 'max';
    try {
      player.updateSettings({
        streaming: {
          abr: {
            autoSwitchBitrate: { video: !max, audio: true },
            limitBitrateByPortal: false,
            usePixelRatioInLimitBitrateByPortal: false
          }
        }
      });
    } catch (e) { if (DEBUG) console.warn('[UVP] applyDashQualityMode settings:', e); }
    if (max && typeof player.setQualityFor === 'function') {
      try { player.setQualityFor('video', UVP.Overlay.dashTopVideoQualityIndex(player)); } catch (e) {}
    }
  };
  ...
  window.fetch = async function (...args) {
    ...
    try { const response = await origFetch.apply(this, args); if (response && response.url && response.headers) { const ct = response.headers.get('content-type') || ''; const responseKind = UVP.Utils.classifyMediaUrl(response.url, ct); if (responseKind !== 'unknown') { UVP.Utils.captureUrl(response.url, null, responseKind); UVP.Utils.noteInterceptedMedia(response.url); try { const _fpHost = new URL(response.url, location.href).hostname; if (_fpHost && _fpHost !== location.hostname) { const _fpHdrs = {}; if (response.headers && typeof response.headers.forEach === 'function') { response.headers.forEach((v, k) => { _fpHdrs[k.toLowerCase()] = v; }); } if (UVP.PlaybackTransport && typeof UVP.PlaybackTransport.recordHostProfile === 'function') { UVP.PlaybackTransport.recordHostProfile(_fpHost, _fpHdrs, 'present'); } } } catch (_fpE) {} } // Generic JSON response scanner: on sites without a dedicated extractor,
      ...
      if (ct.includes('application/json') && response.ok && url) {
        try {
          ...
          if (UVP.Utils.sameSiteHost(reqHost, location.hostname)) {
            ...
            if (!(cl > CONFIG.jsonScanMaxContentLength)) {
            const clone = response.clone();
            clone.text().then(txt => {
              if (!txt) return;
              if (txt.length > CONFIG.jsonScanMaxChars) txt = txt.slice(0, CONFIG.jsonScanMaxChars);
    ...
    const mediaListener = function () {
      try {
        ...
        if (kind !== 'unknown') { UVP.Utils.captureUrl(finalUrl, null, kind); UVP.Utils.noteInterceptedMedia(finalUrl);
          // Host fingerprint: collect readable headers only. Do not invent
          // Access-Control-Allow-Origin from status — a 4xx still means CORS
          // delivered the response. A non-zero status on a cross-origin XHR
          // is the CORS-present signal; status 0 is left unknown (error
          // events, not this load listener, usually carry the CORS block).
          try {
            const _fpHost = new URL(finalUrl, location.href).hostname;
            if (_fpHost && _fpHost !== location.hostname) {
              const _fpHdrs = UVP.Utils.parseResponseHeaders(this.getAllResponseHeaders ? this.getAllResponseHeaders() : '');
              // Do NOT call getResponseHeader('access-control-allow-origin') —
              // cross-origin XHR triggers "Refused to get unsafe header" console
              // warnings. getAllResponseHeaders already exposes CORS-safelisted
              // headers; infer CORS presence from status (0 = blocked).
              const corsHint = (this.status > 0) ? 'present' : null;
              if (UVP.PlaybackTransport && typeof UVP.PlaybackTransport.recordHostProfile === 'function') {
                UVP.PlaybackTransport.recordHostProfile(_fpHost, _fpHdrs, corsHint);
              }
            }
          } catch (_fpE) {}
        }
  ...
  // SPA navigation: deduplicate timer scheduling.  Previously each nav event
  // (pushState/replaceState/popstate/urlchange) scheduled 3+ independent
  // setTimeouts (50/300/1000ms), and multiple events could fire for one nav —
  // up to 11 timers for a single URL change.  scheduleSpaNav uses two shared
  // timers (fast 50ms + late 1000ms) so any number of nav events in the same
  // tick coalesce to at most 2 onSpaNav calls.  onSpaNav self-deduplicates
  // via lastSpaUrl, so the extra call is a no-op if the URL hasn't changed.
  let _spaNavFastTimer = null, _spaNavLateTimer = null;
  UVP.Boot.scheduleSpaNav = function() {
    if (!_spaNavFastTimer) _spaNavFastTimer = setTimeout(() => { _spaNavFastTimer = null; UVP.Boot.onSpaNav(); }, 50);
    if (!_spaNavLateTimer) _spaNavLateTimer = setTimeout(() => { _spaNavLateTimer = null; UVP.Boot.onSpaNav(); }, 1000);
  };
  const origPushState = history.pushState;
  history.pushState = function (...args) { origPushState.apply(this, args); UVP.Boot.scheduleSpaNav(); };
  const origReplaceState = history.replaceState;
  history.replaceState = function (...args) { origReplaceState.apply(this, args); UVP.Boot.scheduleSpaNav(); };
  window.addEventListener('popstate', () => { UVP.Boot.scheduleSpaNav(); }, { passive: true });
  ...
  try {
    if ('onurlchange' in window) {
      if (typeof window.onurlchange === 'undefined') {
        window.onurlchange = () => { UVP.Boot.scheduleSpaNav(); };
      } else {
        window.addEventListener('urlchange', () => { UVP.Boot.scheduleSpaNav(); }, { passive: true });
      }
    }
  ...
  UVP.Boot.onSpaNav = async function() {
    ...
    // RedGifs feed: URL changes while the same page scrolls. Keep the overlay.
    // Extract pools reset here. Button retarget waits until scroll-end
    // (updateButtonPosition) unless this nav happened while idle.
    if (UVP.Extractors.isRedgifs()) {
      ...
      try { _pageScrollRootCache.had = false; } catch (e) {}
      if (state.overlay && !state.isDetachedPip) UVP.Overlay.scheduleGeometry();
      if (!state.buttonHiddenByScroll && CONFIG.showButton) UVP.Overlay.retargetPlaybackUi();
      return;
    }
    ...
    // SPA route change with an overlay open:
    //   this document's overlay video is in PiP → hide the box, keep the session
    //     (leavepictureinpicture or a later openOverlay tears it down)
    //   not in PiP → closeOverlay(true)
    ...
    if (UVP.Extractors.isYouTube()) {
      ...
      if (!UVP.Extractors.ytFormatsCurrent()) {
        ytFmtMap.clear(); ytDecipheredUrls.clear(); ytNsigRewrites.clear(); ytPlayerData.isLive = false; ytPlayerData.liveAndroidTried = false; ytPlayerData.videoId = null; ytPlayerData.fmtTs = 0;
        setTimeout(UVP.Extractors.extractYouTubeUrls, CONFIG.siteExtractInitialDelayMs);
      }
    }
    UVP.Extractors.SITES.forEach(s => { if (s.spa && typeof s.extract === 'function' && s.test()) setTimeout(s.extract, CONFIG.siteExtractInitialDelayMs); });
    ...
    if (CONFIG.showButton) UVP.Overlay.wakeButtonEngine();
    else UVP.Overlay.updateButtonPosition();
...
#uvp-native-btn.uvp-loading { cursor: wait; pointer-events: none !important; }
#uvp-native-btn.uvp-loading::after { content: ''; width: 22px; height: 22px; margin: 0; border: 3px solid rgba(255,255,255,.35); border-top-color: #fff; border-radius: 50%; animation: uvp-btn-spin 0.7s linear infinite; }
@keyframes uvp-btn-spin { to { transform: rotate(360deg); } }
#uvp-native-btn:active { background: #ff5a5a; border-color: transparent; transform: translate(-50%, -50%) scale(0.95) !important; }
.uvp-overlay { position: fixed; z-index: 2147483646; background: #000; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 30px rgba(0,0,0,.6); border-radius: 4px; overflow: hidden; touch-action: pan-y; }
...
`;
  ...
  UVP.Utils.assertSafeVideoUrl = function(url) {
    ...
    if (typeof url !== 'string' || url.length < 12 || url.length > 16384) return null;
    if (!UVP.Utils.isUsableUrl(url)) return null;
    try { return new URL(url).href; } catch (e) { return null; }
  };
  // Predict remaining validity of a signed media URL.
  // Returns milliseconds until expiry, or Infinity if no expiry signal found.
  // YouTube googlevideo URLs carry `expire` as a Unix timestamp (seconds).
  UVP.Utils.urlExpiryMs = function(url) {
    if (!url || typeof url !== 'string') return Infinity;
    try {
      const u = new URL(url, location.href);
      // YouTube googlevideo: expire=<unix_seconds>
      const expire = parseInt(u.searchParams.get('expire') || '0', 10);
      if (expire > 0) return Math.max(0, (expire * 1000) - Date.now());
      // Some CDNs use expires=<unix_seconds> or expiration
      const expires = parseInt(u.searchParams.get('expires') || '0', 10);
      if (expires > 0) return Math.max(0, (expires * 1000) - Date.now());
      return Infinity;
    } catch (e) {
      return Infinity;
    }
  };
  // Check if a URL is expired or about to expire within the warning window.
  UVP.Utils.isUrlExpiringSoon = function(url, warningMs) {
    const remaining = UVP.Utils.urlExpiryMs(url);
    if (remaining === Infinity) return false;
    return remaining <= (warningMs || 30000); // default 30s warning window
  };
  UVP.Utils.isUrlExpired = function(url) {
    return UVP.Utils.urlExpiryMs(url) <= 0;
  };
  // Lightweight HEAD request to pre-validate a playback URL.
  // Returns { ok: boolean, status: number, reason: string }.
  // Uses page-transport fetch (not GM_xhr) — this is a pre-check, not a
  // media load. A CORS failure on HEAD does NOT mean the URL is bad
  // (many CDNs don't support CORS on HEAD but work fine for GET via
  // GM_xhr), so CORS failures return ok: true (optimistic).
  UVP.Utils.validatePlaybackUrl = async function(url) {
    if (!url || typeof url !== 'string') return { ok: false, status: 0, reason: 'no url' };
    // Fast path: check expiry without a network request
    if (UVP.Utils.isUrlExpired(url)) return { ok: false, status: 410, reason: 'expired' };
    // Non-HTTP URLs: can't HEAD (blob:, data:)
    if (!/^https?:\/\//i.test(url)) return { ok: true, status: 0, reason: 'non-http (optimistic)' };
    // Try HEAD via page-transport. 3s cap: this probe delays playVideo, so a
    // slow CDN must not add noticeable startup latency (recovery handles the
    // rare false-negative — the probe is optimistic below).
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(function() { controller.abort(); }, 3000); // 3s max
      ...
      if (r.ok) return { ok: true, status: r.status, reason: 'ok' };
      // 403/410 = expired or forbidden, 404 = gone
      if (r.status === 403 || r.status === 410 || r.status === 404) {
        return { ok: false, status: r.status, reason: 'http ' + r.status };
      }
      // 405/501: CDN doesn't implement HEAD — the URL itself may be fine.
      if (r.status === 405 || r.status === 501) {
        return { ok: true, status: r.status, reason: 'head unsupported (optimistic)' };
      }
      // Other errors (5xx, etc) might be transient — optimistic
      return { ok: true, status: r.status, reason: 'transient (optimistic)' };
    } catch (e) {
      // CORS TypeError or abort — URL might still work via GM_xhr
      // Don't block playback, let the engine try
      return { ok: true, status: 0, reason: 'cors/abort (optimistic)' };
    }
  };
  ...
  UVP.Utils.classifyMediaUrl = function(url, contentType) {
    ...
    if (/\.m4s$/i.test(leaf) ||
        ...
        /(?:^|[_-])init\.(?:mp4|m4s|m4v)$/i.test(leaf) ||
  ...
  UVP.Utils.isDashAssociatedFragment = function(url) {
    if (!url || !/(?:[_-]\d+|[_-]init)\.(?:mp4|webm|m4s)(?:[?#]|$)/i.test(url)) return false;
    let candidate;
  ...
  UVP.Utils.harvestDeclaredMediaUrls = function() {
    const keys = ['src', 'file', 'source', 'url', 'dash', 'hls', 'mpd', 'stream', 'manifest', 'video', 'media'];
    const consider = (raw, base) => {
      if (!raw || typeof raw !== 'string') return;
      let v = raw.trim();
      if (!v || v.length < 8) return;
      try { v = decodeURIComponent(v); } catch (e) {}
      UVP.Utils.captureUrl(v, base || location.href);
    };
    const harvestParams = (href) => {
      if (!href) return;
      try {
        const u = new URL(href, location.href);
        for (const key of keys) {
          const val = u.searchParams.get(key);
          if (val) consider(val, u.href);
        }
        ...
        if (/\.(m3u8|mpd|mp4|webm)([?#]|$)/i.test(u.pathname)) consider(u.href, location.href);
      } catch (e) {}
    };
    harvestParams(location.href);
    try {
      for (const f of document.querySelectorAll('iframe[src], iframe[data-src]')) {
        harvestParams(f.getAttribute('src') || f.getAttribute('data-src') || '');
      }
    } catch (e) {}
    try {
      for (const el of document.querySelectorAll('input, textarea')) {
        const v = el.value || el.getAttribute('value') || '';
        if (/\.(m3u8|mpd)([?#]|$)/i.test(v)) consider(v, location.href);
      }
    } catch (e) {}
  };

  UVP.Utils.decodeMaybeTwice = function(v) {
    let out = String(v || '');
    for (let i = 0; i < 2; i++) {
  ...
  UVP.Utils.getDeclaredPageStream = function() {
    const keys = ['src', 'file', 'source', 'dash', 'hls', 'mpd', 'stream', 'manifest'];
    const accept = (raw, base) => {
      if (!raw) return null;
      const v = UVP.Utils.decodeMaybeTwice(raw).trim();
      if (!/\.(?:mpd|m3u8)(?:[?#]|$)/i.test(v)) return null;
      const resolved = UVP.Utils.resolveSafeUrl(v, base || location.href);
      return UVP.Utils.assertSafeVideoUrl(resolved);
    };
    try {
      const u = new URL(location.href);
      for (const key of keys) {
        const hit = accept(u.searchParams.get(key), location.href);
        if (hit) return hit;
      }
    } catch (e) {}
    try {
      for (const el of document.querySelectorAll('input, textarea')) {
        const hit = accept(el.value || el.getAttribute('value') || '', location.href);
        if (hit) return hit;
      }
    } catch (e) {}
    return null;
  ...
  UVP.Utils.pickBestUrl = function(preferFormat, excludeUrl) {
    ...
    if (preferFormat && /^m3u8$/i.test(preferFormat)) return latestTopLevel(hlsManifests, /\/(?:master|index|playlist|llhls)\.m3u8([?#]|$)/i);
    if (mpd) return mpd;
    if (rgHd) return rgHd;
    const formatRe = preferFormat ? new RegExp('\\.' + preferFormat + '([?#]|$)', 'i') : null;
    const m3u8s = hlsManifests;
    // firstHls(): pick the newest host-scoped top-level HLS playlist.
    // Anchor host is the first JSON-LD contentUrl host when tagged, else the
    // page host. Prefer master/index/playlist naming (CDN convention for the
    // full-variant playlist). Without host-scoping this would hijack picks
    // on multi-CDN pages (tube-site ads ship their own master.m3u8).
    const firstHls = () => {
      let anchor = null;
      for (const u of jsonLdUrls) { try { anchor = new URL(u).hostname; break; } catch (e) {} }
      if (!anchor) anchor = location.hostname;
      ...
      for (let i = pool.length - 1; i >= 0; i--) {
        if (/\/(?:master|index|playlist|llhls)\.m3u8([?#]|$)/i.test(pool[i]) && !/_audio|audio-only|_a\d+/i.test(pool[i])) return pool[i];
      }
      for (let i = pool.length - 1; i >= 0; i--) {
  ...
  // After site getUrl returns null, generic/pickBestUrl may still run.
  // Reject leftovers that belong to a different YouTube id or X media id.
  UVP.Utils.acceptClickScopedUrl = function(url, bestVideo) {
    if (!url) return null;
    if (!UVP.Utils.assertSafeVideoUrl(url)) return null;
    if (UVP.Extractors.isYouTube()) {
      if (!UVP.Extractors.getYtVideoId()) return null;
      if (ytFmtMap.has(url)) return url;
      try {
        const pick = UVP.Extractors.pickYouTubeUrl();
        if (pick && pick === url) return url;
      } catch (e) {}
      return null;
    }
    if (UVP.Extractors.isX()) {
      let mediaId = null;
      if (bestVideo && bestVideo.poster) mediaId = UVP.Extractors.getXMediaIdFromPoster(bestVideo.poster);
      if (mediaId) {
        try {
          const path = new URL(url).pathname;
          if (path.includes('/' + mediaId + '/') || path.includes('/tweet_video/' + mediaId)) return url;
        } catch (e) {}
        return null;
      }
      if (/video\.twimg\.com/i.test(url)) return null;
    }
    return url;
  };
  ...
  // Ranked playback candidate list (generic sites): [primary, ...format-class
  // alternates]. Every pick flows through the same affinity filter as
  // pickBestUrl, so feed/playlist pools stay scoped to the CURRENT video.
  // Consumers: showNativePlayer verification (probe top of the list) and
  // recoverPlayer's candidate ladder (walk the untried remainder).
  UVP.Utils.rankPlaybackCandidates = function(primary) {
    const out = [];
    const push = (u) => {
      const safe = UVP.Utils.assertSafeVideoUrl(u);
      if (safe && !out.includes(safe)) out.push(safe);
    };
    push(primary);
    if (!UVP.Utils.isDashManifest(primary)) {
      push(UVP.Utils.pickBestUrl('m3u8', primary));
    }
    push(UVP.Utils.pickByBestResolution(u => UVP.Utils.isDirectMedia(u)));
    const fb = UVP.Utils.pickPlaybackFallback(primary);
    if (fb && !UVP.Utils.isDashManifest(fb) && !UVP.Utils.isHlsManifest(primary)) push(fb);
  ...
  // 'muxed': prefer itag-22 720p progressive (in-container audio). If 22 is absent,
  //          same path as 'max' — never silently degrade to 360p.
  ...
  // 'max' pins the top video rung (no ABR switches). 'auto' is stock
  // engine ABR. Toggled from the userscript menu.
  ...
  // Overlay CSS × min(dpr, abrMaxDevicePixelRatio). Measure helper only.
  UVP.Utils.portalScaleFactor = function() {
    let dpr = 1;
    try { dpr = (typeof window !== 'undefined' && window.devicePixelRatio) || 1; } catch (e) {}
    const cap = CONFIG.abrMaxDevicePixelRatio || 2;
    return Math.min(dpr, cap);
  };
  UVP.Utils.portalDevicePixels = function(el) {
    const cssW = (el && el.clientWidth) || 0;
    const cssH = (el && el.clientHeight) || 0;
    let raw = 1;
    try { raw = (typeof window !== 'undefined' && window.devicePixelRatio) || 1; } catch (e) {}
    const dpr = UVP.Utils.portalScaleFactor();
    return {
      cssW: cssW,
      cssH: cssH,
      rawDpr: raw,
...
UVP.Extractors.extractSpankBangUrls = function() {
  ...
  UVP.Extractors.getRgIdFromElement = function(el) {
    if (!el) return null;
    const cache = (id) => { if (id && el.dataset) el.dataset.rgId = id; return id; };
    // Live card id FIRST. RedGifs recycles a single <video> across feed
    // cards; a cached dataset.rgId from the previous card is a false identity
    // and pinned the overlay onto the wrong gif (blocking page navigation).
    const feedEl = el.closest ? el.closest('[data-feed-item-id]') : null;
    if (feedEl) { const id = UVP.Extractors.sanitizeRgId(feedEl.getAttribute('data-feed-item-id')); if (id) return cache(id); }
    if (el.id && el.id.startsWith('gif_')) { const id = UVP.Extractors.sanitizeRgId(el.id.replace('gif_', '')); if (id) return cache(id); }
    const gifContainer = el.closest ? el.closest('[id^="gif_"]') : null;
    if (gifContainer && gifContainer.id.startsWith('gif_')) { const id = UVP.Extractors.sanitizeRgId(gifContainer.id.replace('gif_', '')); if (id) return cache(id); }
    if (el.tagName === 'VIDEO' || (el.querySelector && el.querySelector('video'))) {
      const video = el.tagName === 'VIDEO' ? el : el.querySelector('video');
      if (video) {
        const src = video.currentSrc || video.src || '';
        if (src && !src.startsWith('blob:')) { const m = src.match(/\/([^/]+?)(?:-mobile|-sd|-hd|\.mp4|\.m4s|\.webp|\.jpg|\.gif)?(?:[?#]|$)/i); if (m && m[1] && m[1].length > 5) { const id = UVP.Extractors.sanitizeRgId(m[1]); if (id) return cache(id); } }
        if (video.poster) { const m = video.poster.match(/\/([^/]+?)(?:-mobile|-poster|-sd|-hd)?\.(?:jpg|webp)$/i); if (m && m[1] && m[1].length > 5) { const id = UVP.Extractors.sanitizeRgId(m[1]); if (id) return cache(id); } }
      }
    }
    const watchLink = el.closest ? el.closest('a[href*="/watch/"]') : null;
    if (watchLink) { const id = UVP.Extractors.getRgIdFromUrl(watchLink.href); if (id) return cache(id); }
    // Cached id is last-resort only when no live DOM signal exists.
    if (el.dataset && el.dataset.rgId) return el.dataset.rgId;
    // Page URL is page-level, not element-level — never cache it onto a recycled node.
    if (location.pathname.includes('/watch/')) return UVP.Extractors.getRgIdFromUrl(location.href);
    return null;
  }
    ...
    if (vid && vidHash) {
      ...
      try {
        ...
        if (typeof fetch === 'function') {
          const resp = await origFetch(apiUrl, { headers: { 'Origin': location.origin, 'Referer': location.href } });
 ...
 // Stage 2: POST to /api/videos/stream with the data-streamkey to get a JSON
  ...
  UVP.Extractors.getXMediaIdFromPoster = function(posterUrl) {
    ...
    // Regular / Amplify videos: numeric media id in the path.
    let m = posterUrl.match(/\/(?:ext_tw_video_thumb|amplify_video_thumb)\/(\d+)/);
    if (m) return m[1];
    // Native GIFs: tweet_video_thumb/<fileId>.jpg → tweet_video/<fileId>.mp4
    // The old (\d+) capture never matched these alphanumeric filenames, so
    // pickXUrl ran unscoped and returned a previously captured tweet video.
    m = posterUrl.match(/\/tweet_video_thumb\/([^\/?#.]+)/);
    return m ? m[1] : null;
  }
  ...
  UVP.Extractors.pickXUrl = function(mediaId) {
    ...
    if (mediaId) {
      const scoped = all.filter(u => u.includes('/' + mediaId + '/') || /\/tweet_video\//i.test(u) && u.includes(mediaId));
  ...
  UVP.Extractors.extractJsonLdVideoUrls = function() {
    const ldScripts = UVP.Utils.getJsonLdScripts();
    for (const script of ldScripts) {
      try {
        const data = JSON.parse(script.textContent);
  ...
  // variant ladder for downloads; the overlay's engine-owned ABR seeks the
  // top rung.
  ...
  const ytPlayerData = { playerJsUrl: null, isLive: false, videoId: null, fmtTs: 0, liveAndroidTried: false };
  ...
  const ytNsigRewrites = new Map(); // old googlevideo URL → rewritten URL (query or path n)
  ...
  const ytFormatInfo = (fmt, resolved) => ({
    ...
    colorInfo: fmt.colorInfo || null,
    ...ytAudioTrackInfo(fmt)
  });

  UVP.Extractors.processYtStreamingData = function(sd, videoDetails, clientName) {
    if (!sd) return 0;
    ...
    if (_vid && ytPlayerData.videoId !== _vid) {
      ...
      ytPlayerData.liveAndroidTried = false;
      ...
      ytNsigRewrites.clear();
      fmtResMap.clear();
      ytPlayerData.playerJsUrl = null;
      ytNsigCache.playerUrl = null;
      ytNsigCache.func = null;
      // Drop ensure join so B does not await A's player-JS fetch.
      UVP.Extractors.dropYtNsigEnsureJoin();
      UVP.Utils.invalidateUrlCache();
    ...
    if (sd.hlsManifestUrl && /^https?:\/\//i.test(sd.hlsManifestUrl)) {
      ...
      if (resolved) {
        ytFmtMap.set(resolved, { itag: 0, height: 0, mimeType: 'application/x-mpegURL', hasN: false, bitrate: 0, fps: 0, isHls: true, isLive: !!(videoDetails && videoDetails.isLive === true), client: clientName || '' });
        count++;
      }
    }
    if (sd.dashManifestUrl && /^https?:\/\//i.test(sd.dashManifestUrl)) {
      ...
      if (resolved) {
        ytFmtMap.set(resolved, { itag: 0, height: 0, mimeType: 'application/dash+xml', hasN: false, bitrate: 0, fps: 0, isDash: true, isLive: !!(videoDetails && videoDetails.isLive === true), client: clientName || '' });
        count++;
      }
    }
    if (videoDetails) {
      if (videoDetails.isLive === true) {
        ytPlayerData.isLive = true;
      } else {
        let hasLiveManifest = false;
        for (const info of ytFmtMap.values()) {
          if (info && info.isLive && (info.isHls || info.isDash)) { hasLiveManifest = true; break; }
        }
        if (!hasLiveManifest) ytPlayerData.isLive = false;
      }
    }
    // After the format loop, check if any captured URLs are expiring soon
    // and log a warning for debugging. The actual refresh happens in
    // openOverlay's pre-validation step.
    if (DEBUG && count > 0) {
      let expiringCount = 0;
      for (const [u] of ytFmtMap) {
        if (UVP.Utils.isUrlExpiringSoon(u, 60000)) expiringCount++;
      }
      if (expiringCount > 0) {
        console.warn('[UVP] ' + expiringCount + ' format URL(s) expiring within 60s');
      }
    }
  ...
  UVP.Extractors.extractYouTubeUrls = async function() {
    ...
    const mine = (async () => {
      ...
      // Also run when existing formats lack SegmentBase ranges (initRange/
      // indexRange) — mobile/JS-less clients (VISIONOS, ANDROID, IOS) return
      // URLs without n-params but ALSO without SegmentBase ranges, so the
      // DASH/MPD path can't build a manifest from them. The WEB client returns
      // adaptive formats WITH ranges AND n-params (deciphered by the n-solver).
      const _hasRangedFormats = () => [...ytFmtMap.values()].some(info =>
        info && info.initRange && info.indexRange
      );
      if ((ytFmtMap.size === 0 || !_hasRangedFormats()) && stillCurrent()) {
        try {
          const apiKey = getYtApiKey();
          const visitorData = getYtVisitorData();
          const apiUrl = `https://www.youtube.com/youtubei/v1/player?key=${apiKey}&prettyPrint=false`;
      ...
      if ((ytFmtMap.size === 0 || UVP.Extractors.ytNeedsLiveManifestWalk()) && stillCurrent()) {
        // Live streams: ANDROID is the only InnerTube client that reliably
        // returns hlsManifestUrl for live content. VISIONOS returns
        // LOGIN_REQUIRED for live and wastes up to 5s (GM_xhr timeout) before
        // failing. Reorder to try ANDROID first when the page already flagged
        // isLive (Tier 1 sets ytPlayerData.isLive from videoDetails.isLive).
        const tier3Clients = ytPlayerData.isLive
          ? [...YT_CLIENTS.filter(c => c.clientName === 'ANDROID'), ...YT_CLIENTS.filter(c => c.clientName !== 'ANDROID')]
          : YT_CLIENTS;
  ...
  // ==================== YOUTUBE HDR DETECTION ====================
  // YouTube's InnerTube API returns colorInfo with STRING enums (verified
  // against live ANDROID/WEB responses), e.g.
  //   transferCharacteristics: "COLOR_TRANSFER_CHARACTERISTICS_SMPTEST2084" (PQ)
  //   transferCharacteristics: "COLOR_TRANSFER_CHARACTERISTICS_ARIB_STD_B67" (HLG)
  // Some clients/pipelines emit raw CICP numbers instead (16 = PQ, 18 = HLG).
  // Both forms must be recognized or HDR detection silently never fires.
  // On YouTube, HDR rides vp9.2 (itags 330-337) and 10-bit AV1 (694-701).
  const HDR_TRANSFER_MAP = {
    'smpte2084': 'pq',
    'st2084': 'pq',
    'pq': 'pq',
    'arib-std-b67': 'hlg',
    ...
    '16': 'pq',
    '18': 'hlg',
    'color_transfer_characteristics_smptest2084': 'pq',
    'color_transfer_characteristics_arib_std_b67': 'hlg',
    'color_transfer_characteristics_hlg': 'hlg',
  };
  const isHdrFormat = (info) => {
    const ci = info && info.colorInfo;
    if (!ci) return false;
    const tc = String(ci.transferCharacteristics || '').toLowerCase();
    return !!HDR_TRANSFER_MAP[tc];
  };
  const hdrTransferType = (info) => {
    const ci = info && info.colorInfo;
    if (!ci) return null;
    const tc = String(ci.transferCharacteristics || '').toLowerCase();
    return HDR_TRANSFER_MAP[tc] || null;
  };
  // Normalize YouTube's colorInfo enum strings (or raw CICP numbers) to the
  // numeric CICP values the DASH cicp: descriptors require.
  const CICP_PRIMARIES_MAP = {
    '1': '1', 'bt709': '1', 'color_primaries_bt709': '1',
    '9': '9', 'bt2020': '9', 'color_primaries_bt2020': '9',
  };
  const CICP_MATRIX_MAP = {
    '1': '1', 'bt709': '1', 'color_matrix_coefficients_bt709': '1',
    '9': '9', 'bt2020': '9', 'bt2020-ncl': '9', 'bt2020_ncl': '9', 'color_matrix_coefficients_bt2020_ncl': '9',
  };
  const CICP_TRANSFER_MAP = {
    '1': '1', 'bt709': '1', 'color_transfer_characteristics_bt709': '1',
    '16': '16', 'smpte2084': '16', 'st2084': '16', 'pq': '16', 'color_transfer_characteristics_smptest2084': '16',
    '18': '18', 'arib-std-b67': '18', 'hlg': '18', 'color_transfer_characteristics_arib_std_b67': '18', 'color_transfer_characteristics_hlg': '18',
  };
  const cicpValue = (v, map) => {
  ...
  const ytVideoCodecRank = (mime, info) => {
    // HDR-aware: when the format is HDR, AV1 (YouTube's only HDR codec) ranks highest.
    // SDR follows the original VP9 > AV1/HEVC > AVC preference.
    const isHdr = info ? isHdrFormat(info) : false;
    if (isHdr) {
      if (/av01|av1/i.test(mime || '')) return 4;
      if (/hvc1|hev1/i.test(mime || '')) return 3;
      if (/vp0?9|vp9/i.test(mime || '')) return 2;
    } else {
      if (/vp0?9|vp9/i.test(mime || '')) return 3;
      if (/av01|av1|hvc1|hev1/i.test(mime || '')) return 2;
    }
    if (/avc1|avc3|h264/i.test(mime || '')) return 1;
    return 0;
  };
  ...
  const ytCompareEntries = (a, b) => {
    ...
    const ac = ytVideoCodecRank(a.info.mimeType, a.info), bc = ytVideoCodecRank(b.info.mimeType, b.info);
  ...
  // - 'max' (default): highest adaptive video stream (1080p/4K) paired with best audio stream
  // - 'muxed': itag 22 (720p progressive) when present; otherwise identical to 'max'
  ...
  UVP.Extractors.pickYouTubeDownloadUrl = function(quality) {
    ...
    if (q === 'lowest') {
      ...
      for (const [u, info] of ytFmtMap) {
        ...
        if (ytIsBad(u, info)) continue; // skip throttled/403'd n-locked URLs, same as the muxed path
  ...
  UVP.Extractors.getYtNDecipher = async function(expectedGeneration) {
    ...
    // v2 structural cache. Re-eval + probe on restore; origin can write sessionStorage.
    ...
    try { sessionStorage.removeItem('uvp-yt-nsig'); } catch (e) {}
    const jsCode = await UVP.Extractors.fetchYtPlayerJs(playerUrl);
    if (!stillCurrent()) return null;
    if (!jsCode) throw new Error('empty player JS');
    // Current YouTube: structural preprocess only. No legacy _w8_ / fromCharCode path.
    try {
      const pp = UVP.Extractors.preprocessYtPlayerForEval(jsCode);
    ...
    } catch (e) { if (DEBUG) console.warn('[UVP] structural n-solver path failed:', e && e.message); }
    throw new Error('n solver not found (structural path failed)');
  };

  UVP.Extractors.ytUrlHasNChallenge = function(u) {
    return !!u && (/[?&]n=/.test(u) || /\/n\/[A-Za-z0-9_-]+\//.test(u));
  };
    ...
    for (const u of pool) {
      ...
      try {
        ...
        if (newUrl !== u) {
          ...
          ytNsigRewrites.set(u, newUrl);
          ytDecipheredUrls.add(newUrl);
          fixed++;
        }
  ...
  // n-sig ensure. Failures swallowed (no-n clients still play).
  // Join key: generation + playerJsUrl + videoId. No URL → no inflight.
  let _ytNsigEnsureInflight = null;
  let _ytNsigEnsureGeneration = -1;
  let _ytNsigEnsurePlayerUrl = null;
  let _ytNsigEnsureVideoId = null;
  UVP.Extractors.dropYtNsigEnsureJoin = function() {
    _ytNsigEnsureInflight = null;
    _ytNsigEnsureGeneration = -1;
    _ytNsigEnsurePlayerUrl = null;
    _ytNsigEnsureVideoId = null;
  };
  UVP.Extractors.ytNsigNeedsDecipher = function() {
    return [...capturedUrls].some(u => /googlevideo\.com/i.test(u) && /[?&]n=/.test(u) && !ytDecipheredUrls.has(u))
      || [...ytFmtMap.keys()].some(u => /[?&]n=/.test(u) && !ytDecipheredUrls.has(u));
  };
  UVP.Extractors.ensureYtNDeciphered = function(expectedGeneration, _retryDepth) {
    const generation = (typeof expectedGeneration === 'number') ? expectedGeneration : ytRouteGeneration;
    const retryDepth = _retryDepth | 0;
    if (generation !== ytRouteGeneration) return Promise.resolve(0);
    // Skip rows already in ytDecipheredUrls — n= stays on the URL after rewrite.
    if (!UVP.Extractors.ytNsigNeedsDecipher()) return Promise.resolve(0);
    if (!ytPlayerData.playerJsUrl) UVP.Extractors.ensureYtPlayerJsUrl();
    const playerUrlNow = ytPlayerData.playerJsUrl || null;
    // No player JS yet: return 0 without parking an inflight.
    if (!playerUrlNow) return Promise.resolve(0);
    const videoIdNow = UVP.Extractors.getYtVideoId() || ytPlayerData.videoId || null;
    if (_ytNsigEnsureInflight
        && _ytNsigEnsureGeneration === generation
        && _ytNsigEnsurePlayerUrl === playerUrlNow
        && _ytNsigEnsureVideoId === videoIdNow) {
      return _ytNsigEnsureInflight;
    }
    _ytNsigEnsureGeneration = generation;
    _ytNsigEnsurePlayerUrl = playerUrlNow;
    _ytNsigEnsureVideoId = videoIdNow;
    const work = (async () => {
      try {
        const playerUrlAtStart = playerUrlNow;
        if (generation !== ytRouteGeneration) return 0;
        if (ytPlayerData.playerJsUrl !== playerUrlAtStart) return 'retry';
    ...
    const wrapped = work.then((result) => {
      if (_ytNsigEnsureInflight === wrapped) UVP.Extractors.dropYtNsigEnsureJoin();
      if (result === 'retry' && retryDepth < 2 && generation === ytRouteGeneration && UVP.Extractors.ytNsigNeedsDecipher()) {
        return UVP.Extractors.ensureYtNDeciphered(generation, retryDepth + 1);
      }
      return typeof result === 'number' ? result : 0;
    }, (err) => {
      if (_ytNsigEnsureInflight === wrapped) UVP.Extractors.dropYtNsigEnsureJoin();
      if (DEBUG) console.warn('[UVP] ensureYtNDeciphered failed:', err);
      return 0;
    });
    _ytNsigEnsureInflight = wrapped;
    return wrapped;
  };
  ...
  // Query the number of in-flight requests for a host (semaphore active count).
  // Used by the prefetch path to avoid saturating the perHostConcurrency cap
  // and starving hls.js's own fragment loader (hls.js #6753, #1614).
  UVP.Network.getActiveHostRequests = function(hostname) {
    if (!UVP.Network._hostSemaphores) return 0;
    const sem = UVP.Network._hostSemaphores.get(hostname);
    return sem ? sem.active : 0;
  };
  // ==================== COOKIE POLICY ====================
  ...
  UVP.Network.requestPageContext = function(url, type = 'text', extraHeaders = {}, onProgress, scope, onResponse) {
    ...
    return new Promise((resolve, reject) => {
      ...
      origFetch(url, { method: 'GET', headers, credentials: 'same-origin', signal: ctrl ? ctrl.signal : undefined })
        .then((res) => {
          ...
          if (onResponse) { try { onResponse(res); } catch (e) {} }
  ...
  UVP.Network.requestCrossOriginWithRetry = function(url, type = 'text', onProgress, extraHeaders = {}, retries = 3, timeoutMs = 0, scope, onResponse) {
    if (!UVP.Utils.isUsableUrl(url)) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const runPageTransport = () => {
        ...
        }, scope, onResponse).then((response) => {
          if (scope && !scope.check()) { cancel(); return; }
        ...
        const opts = {
          ...
          onload: (res) => {
            ...
            if (res.status >= 200 && res.status < 300) {
              ...
              if (!stripCustomHeaders && !/googlevideo\.com$/i.test(host)) {
  ...
  UVP.Download.resolveToMediaPlaylist = async function(m3u8Url, scope, onPhase) {
    ...
    let loopCount = 0;
    // Download quality preference: 'max' (default), 'muxed', or 'lowest' — set via the
  ...
  UVP.Download.parseSegments = function(manifest, mediaUrl) {
    ...
    let truncated = false; // set when the 50,000-segment safety cap fired mid-parse
    let activeKey = null;
    let keyInfo = null;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (j < lines.length) {
        if (segments.length >= 50000) { console.warn('[UVP] manifest segment cap reached, truncating'); truncated = true; break; }
  ...
  UVP.Download.buildJob = async function(m3u8Url, filename, scope, onPhase) {
    ...
    // Fail-don't-ship: parseSegments caps at 50,000 segments; shipping the
    // truncated remainder would produce a silently incomplete file.
    if (parsed.truncated) throw new Error('playlist exceeds 50,000-segment safety cap (truncated)');
  ...
  UVP.Download.saveJobMeta = function(job, force) {
    ...
    // Stream mode writes segments directly to the user-picked file, not IndexedDB,
    // so persisting a completed-set would poison a later resume (no file handle to
    // reopen → gapped merge). Stream-mode meta carries no completed-set; resume
    // does a fresh IndexedDB download.
      ...
      if (window.showSaveFilePicker) {
        ...
        try {
          ...
          if (pickErr && pickErr.name === 'AbortError') {
            // User cancelled the Save As dialog — a real cancel. `null` is the
            // cancel contract (not `false`, which is the error path).
            await UVP.Download.cleanupDownloadIfCurrent(job);
            return null;
          }
  ...
  UVP.Download.makeUpdateUI = function() { let lastPct = -1, lastTxt = null, trackShown = false; return (percent, text) => { state.downloadPercent = percent; state.downloadText = text; const overlay = state.overlay; if (!overlay) return; if (!trackShown) { trackShown = true; const track = overlay.shadowRoot.querySelector('.uvp-progress-track'); if (track) track.style.opacity = '1'; } if (percent === lastPct && text === lastTxt) return; lastPct = percent; lastTxt = text; const fill = overlay.shadowRoot.querySelector('.uvp-progress-fill'); if (fill) fill.style.transform = `scaleX(${(Math.max(0, Math.min(100, percent)) / 100).toFixed(4)})`; const saveBtn = overlay.shadowRoot.querySelector('.uvp-save'); if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = text; } }; }
  ...
  UVP.Download.downloadMP4 = async function(url, filename, updateUI, scope) {
    ...
    const engineFallback = async () => {
      ...
      try {
        ...
        if (scope && !scope.check()) throw new Error('cancelled');
        // Explicit user cancel of the Save As picker — do NOT fall through to a
        // full-RAM blob download; surface the cancel as a quiet cancel.
  ...
  UVP.Download.downloadAudioOnly = async function() {
    ...
    if (!UVP.StateMachine.can('DOWNLOADING')) {
      if (UVP.StateMachine.is('RECOVERING')) UVP.Overlay.showToast('Recovering…');
      return;
    }
    ...
    try {
      ...
      if (!scope.check()) return;
      // Quiet cancel: user aborted a Save As picker mid-chain.
      if (e && e.message === 'cancelled') {
        await UVP.Download.cleanupDownloadForScope(scope);
        if (!UVP.Cancel.isCurrent(scope)) return;
        if (UVP.StateMachine.can('PLAYING')) UVP.StateMachine.transition('PLAYING', 'audio picker cancel');
        UVP.Download.endDownload(saveBtn, 'audio picker cancel');
  ...
  UVP.Overlay.extractFreshUrl = async function() {
    ...
    const site = UVP.Extractors.SITES.find(s => (s.test || s.isSite)());
  ...
  UVP.Download.handleSaveAction = async function(url) {
    ...
    try {
      if (/\.m3u8([?#]|$)/i.test(url)) {
        ...
        if (window.showSaveFilePicker) {
          try {
            // Suggested filename uses the container extension matching the stream
            // (.ts for MPEG-TS, .mp4 otherwise).
          ...
          preFileHandle = await window.showSaveFilePicker({
            ...
            if (pickErr && pickErr.name === 'AbortError') {
              ...
              if (saveBtn) saveBtn.textContent = 'Save';
              UVP.Overlay.showToast('Download cancelled');
              // DOWNLOADING was entered at 'handleSaveAction start'; roll back so
              // later Save clicks aren't silently swallowed by the can('DOWNLOADING') gate.
              if (UVP.StateMachine.can('PLAYING')) UVP.StateMachine.transition('PLAYING', 'hls picker cancel');
              return;
            }
            preFileHandle = null;
          }
      ...
      if (!scope.check()) return;
      // Quiet cancel: user aborted a Save As picker mid-chain (stream engine,
      // merge phase). Roll back to PLAYING and end the download UI without
      // an error flash, consistent with the HLS picker-cancel path above.
      if (e && e.message === 'cancelled') {
        await UVP.Download.cleanupDownloadForScope(scope);
        if (!UVP.Cancel.isCurrent(scope)) return;
        if (UVP.StateMachine.can('PLAYING')) UVP.StateMachine.transition('PLAYING', 'save picker cancel');
        UVP.Download.endDownload(saveBtn, 'save picker cancel');
        UVP.Overlay.showToast('Download cancelled');
        return;
      }
  ...
  UVP.Overlay.findBestVideo = function() {
    ...
    for (const video of videos) {
      ...
      // Stability note: onHoverExitToButton (top of IIFE) stops the site from
      // receiving mouseout/pointerout when the pointer enters the closed
      // shadow-DOM button, so the card stays active and the video's rect does
      // not change — the +400 bonus is stable across rescans (no oscillation).
  ...
  // Parent-document embed-src test. Returns true only for iframe srcs that
  // look like a video player embed.  Called before getBoundingClientRect in
  // findPlayerIframe so non-video iframes (reCAPTCHA, ads, analytics) are
  // rejected cheaply without forcing layout.
  UVP.Overlay.frameSrcLooksLikePlayer = function(src) {
    const s = String(src || '');
    if (!s) return false;
    // Fast reject: known non-video iframe domains (captcha, ads, analytics,
    // consent managers, pixel sandboxes).  Eliminates the majority of
    // iframes on commercial pages before any positive regex runs.
    if (/recaptcha|doubleclick|googlesyndication|googleadservices|googletag|tagmanager|hotjar|cookieyes|connect\.facebook\.net|facebook\.com\/tr|web-pixel|bat\.bing|clarity\.ms|sandbox\/modern/i.test(s)) return false;
    return /\/(?:e|embed|v|watch|player|dash-player)\//i.test(s) ||
      /\.(m3u8|mpd|mp4)([?#]|$)/i.test(s) ||
      /[?&](?:src|file|url|dash|hls)=https?/i.test(s) ||
      /vidara|vidwara|lulu|streamtape|filemoon|dood|mixdrop/i.test(s);
  };

  // Largest visible embed-like iframe in THIS document.  Only iframes whose
  // src passes frameSrcLooksLikePlayer are considered — non-video iframes
  // (reCAPTCHA, ads, analytics pixels) are skipped before the expensive
  // getBoundingClientRect call, avoiding layout thrash on ad-heavy pages.
  // Same-origin content is read later by getUrlFromPlayerFrame.
  UVP.Overlay.findPlayerIframe = function() {
    const minW = CONFIG.minVideoWidth;
    const vw = window.innerWidth, vh = window.innerHeight;
    let best = null, bestArea = 0;
    let list = [];
    try { list = Array.from(document.querySelectorAll('iframe')); } catch (e) { return null; }
    for (const f of list) {
      if (!f || !f.isConnected) continue;
      let src = '';
      try { src = f.currentSrc || f.src || f.getAttribute('src') || f.getAttribute('data-src') || ''; } catch (e) {}
      // Filter first: only embed-like iframes qualify.  This runs before
      // getBoundingClientRect so non-video iframes never force a layout
      // flush — the dominant cost on pages with many ad/captcha iframes.
      if (!UVP.Overlay.frameSrcLooksLikePlayer(src)) continue;
      ...
      if (visibleW < minW || visibleH < 80) continue;
  ...
  UVP.Overlay.getPlayButton = function() {
    if (state.playButton && state.playButton.isConnected) return state.playButton;
    try { return document.getElementById('uvp-native-btn'); } catch (e) { return null; }
  };

  UVP.Overlay.updateButtonPosition = function() {
    // Delegates to the unified scheduleSync: one rAF, rescan + geometry,
    // reads before writes.  The display guards previously here are already
    // covered by applyButtonGeometry (buttonEngineWanted + overlayOwnsUi).
    UVP.Overlay.scheduleSync(true);
  }
  UVP.Overlay.ensureButton = function() {
    if (state.sitePlayerFailed || (!state.overlay && state.buttonDismissed)) return;
    let btn = UVP.Overlay.getPlayButton();
    if (!btn || !btn.isConnected) {
      const host = document.createElement('div');
      host.setAttribute('data-uvp-btn-host', '1');
      host.style.cssText = 'all:initial;position:fixed;inset:0;z-index:2147483647;pointer-events:none;overflow:visible;';
      const shadow = host.attachShadow({ mode: 'closed' });
      const style = document.createElement('style');
      style.textContent = "#uvp-native-btn { all: initial; box-sizing: border-box; position: fixed; z-index: 2147483647; width: 56px; height: 56px; background: rgba(20, 20, 24, .75); color: #fff; border: 2px solid rgba(255,255,255,.4); border-radius: 50%; display: none; align-items: center; justify-content: center; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); cursor: pointer; box-shadow: 0 4px 15px rgba(0,0,0,.5); transition: transform 0.1s; font-family: system-ui, Roboto, sans-serif; line-height: 1; margin: 0; padding: 0; outline: none; user-select: none; pointer-events: auto; }\n#uvp-native-btn::after { content: '▶'; font-size: 24px; margin-left: 4px; }\n#uvp-native-btn.uvp-loading { cursor: wait; pointer-events: none !important; }\n#uvp-native-btn.uvp-loading::after { content: ''; width: 22px; height: 22px; margin: 0; border: 3px solid rgba(255,255,255,.35); border-top-color: #fff; border-radius: 50%; animation: uvp-btn-spin 0.7s linear infinite; }\n@keyframes uvp-btn-spin { to { transform: rotate(360deg); } }\n#uvp-native-btn:active { background: #ff5a5a; border-color: transparent; transform: translate(-50%, -50%) scale(0.95) !important; }";
      ...
      btn.addEventListener('click', (ev) => {
        ...
        if (state.buttonExtracting) return;
        UVP.Overlay.showNativePlayer();
      });
      shadow.appendChild(style);
      shadow.appendChild(btn);
      (document.documentElement || document.body).appendChild(host);
      state.playButtonHost = host; // referenced by onHoverExitToButton for shadow-host retargeting check
      state.playButton = btn;
    }
  ...
  // Geometry is event-driven: ResizeObserver + IntersectionObserver on the
  // target box, plus scroll / resize / visualViewport. One rAF (scheduleSync)
  // applies the current rect — overlay geometry first, then button geometry,
  // reads before writes. No 10 Hz button loop and no perpetual overlay rAF.
  UVP.Overlay._geomRaf = null;
  UVP.Overlay._syncFrame = null;
  UVP.Overlay._syncNeedsRescan = false;
  UVP.Overlay._retargetFrame = null;
  UVP.Overlay._pointerRetargetThrottle = 0;
  UVP.Overlay._geomWatchBound = false;
  UVP.Overlay._watchedEl = null;
  UVP.Overlay._resizeObs = null;
  UVP.Overlay._intersectObs = null;
  UVP.Overlay._overlayGeomKey = '';
  UVP.Overlay._overlayDomRescanAt = 0;
  UVP.Overlay._miniOutSince = 0;
  UVP.Overlay._miniEnterTimer = null;
  // Unified frame update: one rAF for both overlay and button geometry.
  // Geometry reads (getBoundingClientRect) happen before style writes to
  // avoid layout thrashing.  rescanButtonTargets is only re-run when the
  // caller didn't already do it synchronously (flagged via needsRescan).
  UVP.Overlay.scheduleSync = function(needsRescan) {
    if (needsRescan) UVP.Overlay._syncNeedsRescan = true;
    if (UVP.Overlay._syncFrame) return;
    UVP.Overlay._syncFrame = requestAnimationFrame(() => {
      UVP.Overlay._syncFrame = null;
      if (UVP.Overlay._syncNeedsRescan) {
        UVP.Overlay._syncNeedsRescan = false;
        UVP.Overlay.rescanButtonTargets();
      }
      if (state.overlay && !state.isDetachedPip) UVP.Overlay.applyOverlayGeometry();
      if (!state.buttonHiddenByScroll) UVP.Overlay.applyButtonGeometry();
    });
  };
  // Coalesce multiple observer/SPA retarget requests into one per frame.
  UVP.Overlay.scheduleRetarget = function() {
    if (UVP.Overlay._retargetFrame) return;
    UVP.Overlay._retargetFrame = requestAnimationFrame(() => {
      UVP.Overlay._retargetFrame = null;
      UVP.Overlay.retargetPlaybackUi();
    });
  };
  UVP.Overlay.clearMiniEnterHold = function() {
    UVP.Overlay._miniOutSince = 0;
    if (UVP.Overlay._miniEnterTimer) {
      clearTimeout(UVP.Overlay._miniEnterTimer);
      UVP.Overlay._miniEnterTimer = null;
    }
  };
  UVP.Overlay.geometryTargetEl = function() {
    if (state.overlay && !state.isDetachedPip && state.targetVideo && state.targetVideo.isConnected) {
      if (UVP.Extractors.isYouTube()) {
        const container = state.targetVideo.closest('#movie_player, .html5-video-player, ytd-player');
        if (container) return container;
      }
      return state.targetVideo;
    }
    const video = state.buttonTargetVideo;
    ...
    const frame = state.buttonTargetFrame;
    if (frame && frame.isConnected) return frame;
    return null;
  };
  UVP.Overlay.watchGeometryElement = function(el) {
    if (el === UVP.Overlay._watchedEl) return;
    try {
      if (UVP.Overlay._watchedEl) {
        if (UVP.Overlay._resizeObs) UVP.Overlay._resizeObs.unobserve(UVP.Overlay._watchedEl);
        if (UVP.Overlay._intersectObs) UVP.Overlay._intersectObs.unobserve(UVP.Overlay._watchedEl);
      }
    } catch (e) {}
    UVP.Overlay._watchedEl = el || null;
    if (!el) return;
    try { if (UVP.Overlay._resizeObs) UVP.Overlay._resizeObs.observe(el); } catch (e) {}
    try { if (UVP.Overlay._intersectObs) UVP.Overlay._intersectObs.observe(el); } catch (e) {}
  };
  UVP.Overlay.ensureGeometryWatch = function() {
    if (UVP.Overlay._geomWatchBound) return;
    UVP.Overlay._geomWatchBound = true;
    const kick = () => UVP.Overlay.scheduleGeometry();
    document.addEventListener('scroll', kick, { capture: true, passive: true });
    window.addEventListener('resize', kick, { passive: true });
    document.addEventListener('fullscreenchange', kick, { passive: true });
    try {
    ...
    try {
      if (typeof ResizeObserver === 'function') UVP.Overlay._resizeObs = new ResizeObserver(kick);
    } catch (e) {}
    try {
      if (typeof IntersectionObserver === 'function') {
        UVP.Overlay._intersectObs = new IntersectionObserver(kick, { threshold: [0, 0.01, 0.1, 0.5, 1] });
      }
    } catch (e) {}
  };
  UVP.Overlay.scheduleGeometry = function() {
    // Delegates to the unified scheduleSync (no rescan — geometry-only).
    // Button is opacity 0 for the scroll-debounce window. Do not find rects
    // or write left/top until onScrollOrWheel reveals it again.
    UVP.Overlay.scheduleSync(false);
  };
  UVP.Overlay.rescanButtonTargets = function() {
    const buttonSuppressed = state.sitePlayerFailed || (!state.overlay && state.buttonDismissed);
    if (buttonSuppressed) {
      state.buttonTargetVideo = null;
      state.buttonTargetFrame = null;
      return;
    }
    const best = UVP.Overlay.findBestVideo();
    const frame = best ? null : UVP.Overlay.findPlayerIframe();
    state.buttonTargetVideo = best;
    state.buttonTargetFrame = frame;
    if (!state.overlay) state.targetVideo = best;
    if (!best && !frame && !state.overlay && !state.isDetachedPip && !state.isMiniPlayer) {
      const anyHostVideo = (UVP.Utils.findAllVideos ? UVP.Utils.findAllVideos(document) : []).some(v => v && v.isConnected);
      if (!anyHostVideo) {
        const btn = UVP.Overlay.getPlayButton();
        if (btn) btn.style.display = 'none';
        UVP.Overlay.stopButtonSync();
        UVP.Overlay.watchGeometryElement(null);
      }
    }
  };
  UVP.Overlay.applyButtonGeometry = function() {
    if (state.buttonHiddenByScroll) return;
    const btn = UVP.Overlay.getPlayButton();
    if (!btn) return;
    if (!UVP.Overlay.buttonEngineWanted()) {
    ...
    const minW = CONFIG.minVideoWidth;
    const buttonSuppressed = state.sitePlayerFailed || (!state.overlay && state.buttonDismissed);
    const activeTarget = state.buttonTargetVideo;
    const alreadyWatching = state.overlay && UVP.Overlay.isSameAsActivePlayback(activeTarget);
    const activeFrame = state.buttonTargetFrame;
    const overlayOwnsUi = state.overlay && !state.isDetachedPip && !state.isMiniPlayer;
    const alreadyWatchingFrame = !!(state.overlay && activeFrame && state._activePlaybackFrame === activeFrame);
    if (buttonSuppressed || overlayOwnsUi || alreadyWatching || alreadyWatchingFrame || (!activeTarget && !activeFrame)) {
      btn.style.display = 'none';
      return;
    }
    const targetEl = (activeTarget && activeTarget.isConnected) ? activeTarget : ((activeFrame && activeFrame.isConnected) ? activeFrame : null);
    if (!targetEl) {
      state.buttonTargetVideo = null;
      state.buttonTargetFrame = null;
    ...
    UVP.Overlay.watchGeometryElement(UVP.Overlay.geometryTargetEl() || targetEl);
    let rect = targetEl.getBoundingClientRect();
    if (UVP.Extractors.isYouTube() && activeTarget) {
  ...
  UVP.Overlay.applyOverlayGeometry = function() {
    ...
    if (state.targetVideo) {
      ...
      if (!inDom) {
        ...
        } else if ((state.overlayUrl || state._activePoster) && t - UVP.Overlay._overlayDomRescanAt >= 500) {
          UVP.Overlay._overlayDomRescanAt = t;
      ...
      if (state.targetVideo && UVP.Extractors.isRedgifs() && state._activeRgId) {
        ...
        if (curRgId && curRgId !== state._activeRgId) rect = null;
      }
      ...
      UVP.Overlay.watchGeometryElement(UVP.Overlay.geometryTargetEl());
      if (isOutOfView) {
        if (!state.isMiniPlayer) {
          const hold = CONFIG.overlayMiniEnterDelayMs || CONFIG.scrollDebounceMs || 500;
          const now = Date.now();
          if (!UVP.Overlay._miniOutSince) UVP.Overlay._miniOutSince = now;
          if (now - UVP.Overlay._miniOutSince < hold) {
            if (!UVP.Overlay._miniEnterTimer) {
              UVP.Overlay._miniEnterTimer = setTimeout(() => {
                UVP.Overlay._miniEnterTimer = null;
                if (state.overlay && !state.isDetachedPip) UVP.Overlay.applyOverlayGeometry();
              }, hold - (now - UVP.Overlay._miniOutSince));
            }
            return;
          }
        }
        UVP.Overlay.clearMiniEnterHold();
        ...
        if (key !== UVP.Overlay._overlayGeomKey) {
          UVP.Overlay._overlayGeomKey = key;
    ...
    } else if (UVP.Overlay._overlayGeomKey !== 'full') {
      UVP.Overlay.clearMiniEnterHold();
      UVP.Overlay._overlayGeomKey = 'full';
      state.isMiniPlayer = false;
  ...
  UVP.Overlay.startOverlaySync = function() {
    ...
    UVP.Overlay._overlayGeomKey = '';
    UVP.Overlay.clearMiniEnterHold();
    UVP.Overlay.ensureGeometryWatch();
    UVP.Overlay.applyOverlayGeometry();
  };
  UVP.Overlay.stopButtonSync = function() {
    if (state.buttonSyncFrame) { cancelAnimationFrame(state.buttonSyncFrame); state.buttonSyncFrame = null; }
  };
  UVP.Overlay.buttonEngineWanted = function() {
    if (!CONFIG.showButton) return false;
    if (typeof document !== 'undefined' && document.hidden) return false;
    if (state.sitePlayerFailed) return false;
    const isFloating = state.isDetachedPip || state.isMiniPlayer;
    if (!isFloating && state.buttonDismissed && !state.overlay) return false;
    return true;
  };
  // Retarget only: pick the host <video> or player iframe in THIS document,
  // then ask geometry to apply. MutationObserver, loadedmetadata, pointer
  // miss/park, SPA, and overlay-close call this. Scroll/resize/RO/IO do not.
  UVP.Overlay.retargetPlaybackUi = function() {
    if (!UVP.Overlay.buttonEngineWanted()) {
      UVP.Overlay.stopButtonSync();
      return;
    }
    try { UVP.Utils.invalidateVideoCache(); } catch (e) {}
    UVP.Overlay.ensureButton();
    UVP.Overlay.ensureGeometryWatch();
    state.buttonSyncFrame = 1;
    UVP.Overlay.rescanButtonTargets();
    // Immediate rescan above already updated targets — skip the rAF rescan.
    UVP.Overlay._syncNeedsRescan = false;
    UVP.Overlay.watchGeometryElement(UVP.Overlay.geometryTargetEl());
    UVP.Overlay.scheduleSync(false);
  };
  UVP.Overlay.wakeButtonEngine = function() {
    UVP.Overlay.retargetPlaybackUi();
  };
  ...
  UVP.Overlay.closeOverlay = function(reloading = false) {
    ...
    state.pipExitByToggle = false;
    state.isMiniPlayer = false;
    UVP.Overlay.clearMiniEnterHold();
    state._activePlaybackSource = null;
    state._activePlaybackFrame = null;
    state._activePoster = null;
    state._activeRgId = null;
    state.buttonTargetVideo = null;
    if (state.buttonSyncFrame) { cancelAnimationFrame(state.buttonSyncFrame); state.buttonSyncFrame = null; }
    if (UVP.Overlay._syncFrame) { cancelAnimationFrame(UVP.Overlay._syncFrame); UVP.Overlay._syncFrame = null; }
    if (UVP.Overlay._retargetFrame) { cancelAnimationFrame(UVP.Overlay._retargetFrame); UVP.Overlay._retargetFrame = null; }
    ...
    // Restore play() on site videos blocked while the overlay owned playback.
    // Immediate cleanup only — blocked shadows also self-heal on their next
    // call (see blockSiteVideoPlay), so elements no longer in the DOM are
    // covered even if this scan can't reach them.
    try {
      const vidsToRestore = UVP.Utils.findAllVideos ? UVP.Utils.findAllVideos(document) : Array.from(document.querySelectorAll('video'));
      for (const v of vidsToRestore) {
        if (v && v.__uvpPlayBlocked) {
          try { delete v.play; } catch (e) {}
          try { v.__uvpPlayBlocked = false; } catch (e) {}
        }
      }
    } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
    if (state.escHandler) {
    ...
    state.deferredSeekTo = null;
    state.bufferHold = false;
    state.enginePlaybackReady = false;
    state.engineWaitStartedAt = 0;
    if (state.syncInterval) { cancelAnimationFrame(state.syncInterval); state.syncInterval = null; }
    // DASH session teardown: destroys dash.js player and revokes MPD blob URL.
    UVP.Overlay.destroyDashSession();
    // Clear manifest cache so stale entries don't carry across videos.
    if (UVP.PlaybackTransport && UVP.PlaybackTransport.clearManifestCache) UVP.PlaybackTransport.clearManifestCache();
    // Session ending: drop this document's PiP if any. The leave handler then
    // no-ops (overlay already gone / state.video no longer this element).
    ...
    UVP.SegmentCache.clear();
    UVP.MultiTab.deactivate();
    ...
    state.video = null; state.overlayUrl = null; state.targetAudioUrl = null; state.wasPaused = false; state.overlayIsLive = false; state.liveDetectReason = null;
    state.playbackCandidates = []; state.playbackFailedCandidates = null;
    ...
    // Restart button engine on overlay close (idempotent; parks if no <video>).
    if (CONFIG.showButton) UVP.Overlay.wakeButtonEngine();
  ...
  UVP.Overlay.isActivePlaybackElement = function(v) {
    if (!v || v.tagName !== 'VIDEO' || v === state.video) return false;
    // RedGifs recycles one <video> node across cards — element identity is
    // not a content identity. Require the RG id to still match or the overlay
    // stays glued to whatever card currently owns that node and eats clicks.
    if (UVP.Extractors.isRedgifs() && state._activeRgId) {
      return UVP.Extractors.getRgIdFromElement(v) === state._activeRgId;
    }
  ...
  UVP.MPD.codecsOf = function(mimeType, height) {
    ...
    let c = m ? m[1].trim() : '';
    // YouTube emits legacy VP9 codec strings: "vp9" (profile 0, 8-bit SDR) and
    // "vp9.2" (profile 2, 10-bit HDR, itags 330-337). Legacy strings are not
    // RFC 6381 — Chrome's MediaSource.isTypeSupported rejects "vp9.2" — so
    // expand to the full vp09 form. Profiles 2/3 are 10-bit (DD=10); the wrong
    // bit depth in the SourceBuffer codec string mis-configures the decoder.
    const vp9m = c.match(/^vp0?9(?:\.(\d))?$/i);
    if (vp9m) {
      const h = Number(height) || 0;
      const profile = vp9m[1] || '0';
      const depth = (profile === '2' || profile === '3') ? '10' : '08';
      let level;
      if (h >= 2160) level = '51';
      else if (h >= 1440) level = '41';
      else if (h >= 1080) level = '31';
      else if (h >= 720) level = '21';
      else level = '10';
      c = 'vp09.0' + profile + '.' + level + '.' + depth;
    }
    return c;
  };
  ...
  // HDR-aware codec preference: when HDR content is present, AV1 (YouTube's only
  // HDR codec) ranks above VP9. SDR content keeps the original VP9 > AV1 > AVC order.
  function getMpdCodecPref(hasHdr) {
    if (hasHdr) return { av01: 4, vp9: 3, hvc1: 3, hev1: 3, avc1: 1, avc3: 1, vp8: 0 };
    return { vp9: 3, av01: 2, hvc1: 2, hev1: 2, avc1: 1, avc3: 1, vp8: 0 };
  }
  // Per-session cache for the async display HDR capability check.
  // null = not yet probed, true/false = probe result. Stays null when no
  // HDR AV1 entry exists to probe, so subsequent videos can retry.
  let _hdrDisplayCache = null;
  // Screen can actually show HDR (not just decode+tone-map). Chromium reports
  // mediaCapabilities.supported=true on many SDR panels because it tone-maps;
  // that produces washed-out PQ when we still pick the HDR ladder. Prefer the
  // CSS dynamic-range signal when the engine exposes it.
  UVP.Utils.screenSupportsHdr = function() {
    try {
      if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
        if (window.matchMedia('(dynamic-range: high)').matches) return true;
        // No high dynamic-range → treat as SDR screen even if decode works.
        if (window.matchMedia('(dynamic-range: standard)').matches) return false;
      }
    } catch (e) {}
    // Unknown (older engines without the media query) → do not block on screen.
    return null;
  };
  UVP.Utils.canDisplayHdr = async function(codec, width, height, fps, transfer, container) {
    // Screen gate first: if the panel is known-SDR, never select HDR ladders.
    const screenHdr = UVP.Utils.screenSupportsHdr();
    if (screenHdr === false) return false;
    // If mediaCapabilities is unavailable, fall back to screen signal only.
    // screenHdr === true → allow HDR; screenHdr === null → allow (legacy).
    if (!navigator.mediaCapabilities) return screenHdr !== false;
    try {
      // Race the probe against a 3s timeout — a hung decodingInfo (GPU driver
      // bug, etc.) must not block the MPD build path and prevent failToMuxed
      // recovery. On timeout, keep HDR only when the screen also claims HDR
      // (or is unknown); never default to HDR on a known-SDR panel.
      // NOTE: the W3C TransferFunction enum is 'srgb' | 'pq' | 'hlg' — the raw
      // CICP name 'smpte2084' is NOT valid and makes decodingInfo throw (which
      // silently turned this gate into an always-true no-op). hdrMetadataType
      // 'smpteSt2086' (static HDR10 metadata) only applies to PQ; HLG carries
      // no static metadata, so it must be omitted for HLG.
      const videoCfg = {
        contentType: (container || 'video/mp4') + '; codecs="' + codec + '"',
        width: width, height: height,
        bitrate: 10000000,
        framerate: fps || 30,
        bitsPerChannel: 10,
        colorGamut: 'rec2020',
        transferFunction: transfer === 'hlg' ? 'hlg' : 'pq'
      };
      if (transfer !== 'hlg') videoCfg.hdrMetadataType = 'smpteSt2086';
      const probe = navigator.mediaCapabilities.decodingInfo({
        type: 'media-source',
        video: videoCfg
      });
      const timer = new Promise(function(resolve) { setTimeout(function() { resolve(null); }, 3000); });
      const result = await Promise.race([probe, timer]);
      if (result === null) return screenHdr !== false; // timeout: screen wins
      // Decode support alone is not enough on SDR panels (tone-map path).
      // Require supported AND (screen HDR or unknown screen).
      return !!result.supported && screenHdr !== false;
    } catch (e) { return screenHdr !== false; }
  };
  UVP.MPD.pickVideoFamily = async function(videoEntries) {
    ...
    // Detect HDR in the pool to select the right codec preference.
    const hasHdr = videoEntries.some(function(x) { return isHdrFormat(x.info); });
    // HDR display capability gate.
    // 1) Screen: matchMedia('(dynamic-range: high)') — known-SDR panels force SDR.
    // 2) Decode: mediaCapabilities.decodingInfo — unsupported codecs drop HDR.
    // Chromium often reports decode-supported on SDR panels via tone-map; the
    // screen gate stops washed-out PQ selection in that case.
    let hdrOk = false;
    if (hasHdr) {
      if (_hdrDisplayCache === null) {
        // Find the top HDR representation to probe. Prefer AV1 (the primary
        // HDR path); fall back to any HDR entry (e.g. vp09.02) when no AV1
        // HDR rendition exists so the probe still reflects the actual content.
        let hdrEntry = null;
        const pickTopHdr = function(pred) {
          for (const list of byFamily.values()) {
            for (const entry of list) {
              if (!isHdrFormat(entry.info) || !pred(entry)) continue;
              if (!hdrEntry || (entry.info.height || 0) > (hdrEntry.info.height || 0)) hdrEntry = entry;
            }
          }
        };
        pickTopHdr(function(entry) { return /av01|av1/i.test(entry.info.mimeType || ''); });
        if (!hdrEntry) pickTopHdr(function() { return true; });
        if (hdrEntry) {
          const codecStr = UVP.MPD.codecsOf(hdrEntry.info.mimeType, hdrEntry.info.height) || 'av01.0.12M.10';
          const container = (hdrEntry.info.mimeType || '').split(';')[0].trim() || 'video/mp4';
          _hdrDisplayCache = await UVP.Utils.canDisplayHdr(codecStr, hdrEntry.info.width || 3840, hdrEntry.info.height || 2160, hdrEntry.info.fps || 30, hdrTransferType(hdrEntry.info), container);
        }
        // No HDR entry to probe — leave _hdrDisplayCache null so the next
        // HDR video with usable HDR entries can retry. hdrOk stays false.
      }
      hdrOk = _hdrDisplayCache !== false;
      if (DEBUG) {
        try {
          console.log('[UVP] HDR ladder decision:', {
            hasHdr: true,
            hdrOk: hdrOk,
            screenHdr: UVP.Utils.screenSupportsHdr(),
            cache: _hdrDisplayCache
          });
        } catch (e) {}
      }
    }
    // Keep each codec family transfer-homogeneous. One AdaptationSet maps to
    // one SourceBuffer; mixing PQ and gamma renditions in it produces
    // mis-mapped output when ABR crosses the boundary. With HDR cleared,
    // HDR-dominant families keep HDR renditions only (YouTube's HDR ladder is
    // complete 144p-2160p); without HDR support, HDR renditions are dropped.
    for (const [fam, list] of byFamily) {
      const famHasHdr = list.some(function(x) { return isHdrFormat(x.info); });
      const filtered = list.filter(function(x) {
        const h = isHdrFormat(x.info);
        return hdrOk ? (famHasHdr ? h : true) : !h;
      });
      if (filtered.length) byFamily.set(fam, filtered);
      else byFamily.delete(fam);
    }
    const codecPref = getMpdCodecPref(hdrOk);
    ...
    for (const list of byFamily.values()) {
      ...
      const key = [top.info.height || 0, top.info.fps || 0, codecPref[UVP.MPD.codecFamily(top.info.mimeType)] || 0];
      if (!bestKey ||
          key[0] > bestKey[0] ||
  ...
  UVP.MPD.buildYouTubeMpd = async function(entries, durationMs) {
    try {
      if (!Array.isArray(entries) || !entries.length) return null;
      const usable = entries.filter(UVP.MPD.usableEntry);
      if (!usable.length) return null;
      const videoReps = await UVP.MPD.pickVideoFamily(usable.filter(x => !UVP.MPD.isAudioEntry(x)));
      ...
      const rep = (x) => {
        ...
        // HDR SupplementalProperty descriptors: CICP color metadata so dash.js
        // and the browser know the content is HDR (BT.2020 + PQ/HLG).
        // SupplementalProperty (not EssentialProperty): dash.js drops
        // Representations whose EssentialProperty it doesn't understand
        // (dash.js #4384). Values must be numeric CICP — YouTube's colorInfo
        // string enums are normalized via the cicp maps.
        let hdrProps = '';
        const hdrType = hdrTransferType(i);
        if (hdrType) {
          const ci = i.colorInfo || {};
          const tcValue = cicpValue(ci.transferCharacteristics, CICP_TRANSFER_MAP) || (hdrType === 'pq' ? '16' : '18');
          const primaries = cicpValue(ci.primaries, CICP_PRIMARIES_MAP) || '9';
          const matrix = cicpValue(ci.matrixCoefficients, CICP_MATRIX_MAP) || '9';
          hdrProps =
            '<SupplementalProperty schemeIdUri="urn:mpeg:mpegB:cicp:TransferCharacteristics" value="' + tcValue + '"/>' +
            '<SupplementalProperty schemeIdUri="urn:mpeg:mpegB:cicp:ColorPrimaries" value="' + primaries + '"/>' +
      ...
      // The video family is transfer-homogeneous (pickVideoFamily filters), so
      // the first representation's colorInfo describes the whole AdaptationSet.
      // AdaptationSet-level CICP descriptors are the DVB-DASH signaling point.
      let videoHdrProps = '';
      const videoHdrType = hdrTransferType(videoReps[0].info);
      if (videoHdrType) {
        const vci = videoReps[0].info.colorInfo || {};
        const vTc = cicpValue(vci.transferCharacteristics, CICP_TRANSFER_MAP) || (videoHdrType === 'pq' ? '16' : '18');
        const vCp = cicpValue(vci.primaries, CICP_PRIMARIES_MAP) || '9';
        const vMc = cicpValue(vci.matrixCoefficients, CICP_MATRIX_MAP) || '9';
        videoHdrProps =
          '<SupplementalProperty schemeIdUri="urn:mpeg:mpegB:cicp:TransferCharacteristics" value="' + vTc + '"/>' +
          '<SupplementalProperty schemeIdUri="urn:mpeg:mpegB:cicp:ColorPrimaries" value="' + vCp + '"/>' +
          '<SupplementalProperty schemeIdUri="urn:mpeg:mpegB:cicp:MatrixCoefficients" value="' + vMc + '"/>';
      }
      parts.push('<AdaptationSet id="1" contentType="video" mimeType="' + UVP.MPD.xmlEscape(videoMime) + '" codecs="' + UVP.MPD.xmlEscape(videoCodecs) + '"' + videoFps + ' maxPlayoutRate="1" segmentAlignment="true" subsegmentAlignment="true" subsegmentStartsWithSAP="1" startWithSAP="1">' + videoHdrProps);
  ...
  UVP.MPD.absolutizeExternalMpd = function(xml, mpdUrl) {
    let text = String(xml || '');
    if (!text || !mpdUrl) return text;
    text = text.replace(/<BaseURL>([\s\S]*?)<\/BaseURL>/gi, (m, inner) => {
      const v = String(inner || '').trim();
      if (!v || /^https?:\/\//i.test(v) || /^data:/i.test(v)) return m;
      try { return '<BaseURL>' + new URL(v, mpdUrl).href + '</BaseURL>'; } catch (e) { return m; }
    });
    if (!/<BaseURL>/i.test(text)) {
      let dir = mpdUrl;
      try { dir = new URL('.', mpdUrl).href; } catch (e) { dir = mpdUrl.replace(/\/[^\/?#]*([?#].*)?$/, '/'); }
      text = text.replace(/<Period(\s|>)/i, '<BaseURL>' + dir + '</BaseURL><Period$1');
    }
    return text;
  };

  UVP.MPD.fetchExternalMpdText = async function(mpdUrl) {
    const tryText = async (fn) => {
      try {
        const body = await fn();
        const text = String(body || '').replace(/^\uFEFF/, '').trim();
        if (text && /<MPD[\s>]/i.test(text.slice(0, 4096))) return text;
      } catch (e) {}
      return null;
    };
    const viaFetch = tryText(async () => {
      const r = await origFetch(mpdUrl, { credentials: 'omit', mode: 'cors' });
      if (!r || !r.ok) throw new Error('http ' + (r && r.status));
      return r.text();
    });
    let text = await viaFetch;
    if (text) return text;
    return tryText(() => UVP.Network.requestCrossOriginWithRetry(
      mpdUrl, 'text', null,
      { 'Accept': 'application/dash+xml,application/xml,text/xml,*/*' },
      1, CONFIG.manifestTimeoutMs, null
    ));
  };

  // ==================== BAKED-IN BUNDLE EVALUATOR ====================
  // Libraries arrive as raw JS bytes (CDN fetch in Mini, baked base64 in Full).
  ...
  async function verifyAndEvalBundle(rawPayload, integrity, globalProp, name) {
    ...
    if (integrity && integrity.indexOf('sha384-') === 0) {
      if (!(typeof crypto !== 'undefined' && crypto.subtle && typeof crypto.subtle.digest === 'function')) {
        throw new Error(name + ' integrity check unavailable (no SubtleCrypto)');
      }
    ...
    // player bundles don't divert into module.exports, leaving the
  ...
  // ==================== EXTERNAL PLAYER LIBRARIES (Mini) ====================
  // hls.min.js v1.7.2 (618,946 bytes) and dash.all.min.js v5.2.1 (801,049
  // bytes) are fetched on first HLS/DASH playback from public CDNs, then
  // SHA-384 verified and page-realm eval'd by verifyAndEvalBundle — the same
  // path as the Full (baked) build. No @require (that delays document-start
  // and breaks the interception wrappers). No baked-in library bytes.
  const HLS_BUNDLE_URLS = [
    'https://cdn.jsdelivr.net/npm/hls.js@1.7.2/dist/hls.min.js',
    'https://unpkg.com/hls.js@1.7.2/dist/hls.min.js',
    'https://fastly.jsdelivr.net/npm/hls.js@1.7.2/dist/hls.min.js'
  ];
  const DASH_BUNDLE_URLS = [
    'https://cdn.jsdelivr.net/npm/dashjs@5.2.1/dist/modern/umd/dash.all.min.js',
    'https://unpkg.com/dashjs@5.2.1/dist/modern/umd/dash.all.min.js',
    'https://fastly.jsdelivr.net/npm/dashjs@5.2.1/dist/modern/umd/dash.all.min.js'
  ];
  ...
  const _bundleLoadPromises = Object.create(null);

  async function fetchExternalBundle(urls, integrity, globalProp, name) {
    let lastErr = null;
    for (let i = 0; i < urls.length; i++) {
      const url = urls[i];
      try {
        const payload = await UVP.Network.requestCrossOriginWithRetry(
          url, 'arraybuffer', null, { 'Accept': '*/*' }, 2, 60000, null
        );
        return await verifyAndEvalBundle(payload, integrity, globalProp, name);
      } catch (e) {
        lastErr = e;
        if (DEBUG) console.warn('[UVP] ' + name + ' fetch failed from ' + url + ':', e);
      }
    }
    throw lastErr || new Error(name + ' failed to load from all CDNs');
  }

  function loadExternalBundle(urls, integrity, globalProp, name) {
    if (_bundleLoadPromises[globalProp]) return _bundleLoadPromises[globalProp];
    const p = fetchExternalBundle(urls, integrity, globalProp, name).catch((e) => {
      delete _bundleLoadPromises[globalProp];
      throw e;
    });
    _bundleLoadPromises[globalProp] = p;
    return p;
  }
  ...
  //   - Fallback: on page-transport failure the host's classified failure
  //     counters increment in the persistent origin-scoped registry
  //     (GM_getValue/GM_setValue, survives reloads). Transport selection uses
  //     a Beta-smoothed, half-life-decayed confidence score:
  //     >= 0.8 → page-transport, < 0.3 (with CORS-type evidence) → GM_xhr,
  //     between → probe. A single transient failure does NOT flip the
  //     transport, a 403 storm never pins GM, and gm hosts self-heal via a
  //     throttled re-probe. The resilient engine (requestCrossOriginWithRetry: GM_xhr
  //     CORS bypass, Range-resume, retry backoff, Retry-After, header-strip
  //     retry) serves loads for hosts classified as 'gm'.
  ...
  UVP.PlaybackTransport = {
    // ---- Unified Persistent Host Registry (v2) ----
    // Single persistent registry (GM_getValue/GM_setValue, survives reloads).
    // v2 changes:
    //   - Keys are origin-scoped ('pageOrigin|host'): CORS is granted per
    //     Origin — a CDN that ACAO-allows site A may still block site B, so a
    //     bare-host score misclassifies across sites. Legacy bare-host keys
    //     are dropped on load (scores rebuild within 3 loads; TTL is 1h).
    //   - Outcomes are recorded per transport (pageOk/gmOk) and failures are
    //     classified by kind: statusless failures (TypeError/abort/stall) are
    //     CORS-or-network 'opaque' evidence — the ONLY kind that may pin GM;
    //     HTTP-statused failures mean the server was reached (auth/server
    //     trouble) and must never pin GM (a 403 storm ≠ missing CORS).
    //   - Confidence is Beta(1,1)-smoothed: (pageOk+1)/(total+2), and counters
    //     decay with a half-life so stale evidence fades back toward 'probe'.
    //   - gm classifications self-heal: one page re-probe per hostReprobeMs.
    _hostRegistry: null,  // Map<'origin|host', registryEntry>, lazily loaded from GM storage
    _registryDirty: false, // true if changes need flushing to GM storage
    _registryFlushScheduled: false,
    _registryUnloadHooked: false, // true once the pagehide/beforeunload flush hook is installed

    _regKey(host) {
      try { return location.origin + '|' + host; } catch (e) { return '|' + host; }
    },

    _getHostRegistry() {
      if (this._hostRegistry !== null) return this._hostRegistry;
      this._hostRegistry = new Map();
      try {
        const raw = GM_getValue('uvp-host-registry', null);
        if (raw && typeof raw === 'string') {
          const parsed = JSON.parse(raw);
          if (parsed && typeof parsed === 'object') {
            // Prune expired entries and legacy bare-host keys on load.
            // Entries for OTHER page origins stay in the map — the flush
            // writes the whole map back, so dropping them here would erase
            // their learning. Lookups are keyed by the current origin.
            const now = Date.now();
            const ttl = CONFIG.hostRegistryTtlMs;
            for (const [key, entry] of Object.entries(parsed)) {
              if (key.indexOf('|') < 0) continue; // legacy v1 bare-host key — drop
              if (entry && typeof entry === 'object' && (now - (entry.lastSeen || 0)) < ttl) {
                this._hostRegistry.set(key, entry);
              }
            }
          }
        }
      } catch (e) { /* corrupt or missing — start fresh */ }
      return this._hostRegistry;
    },

    _flushHostRegistry() {
      // Debounce flushes — batch multiple changes into one GM_setValue call
      if (this._registryFlushScheduled) return;
      this._registryFlushScheduled = true;
      this._registryDirty = true;
      // The debounced flush dies with the page — hook unload once so the final
      // segment-load updates survive a tab close/navigation within the window.
      if (!this._registryUnloadHooked) {
        this._registryUnloadHooked = true;
        const flushNow = () => { try { this._flushHostRegistryNow(); } catch (e) {} };
        window.addEventListener('pagehide', flushNow);
        window.addEventListener('beforeunload', flushNow);
      }
      setTimeout(() => {
        this._registryFlushScheduled = false;
        this._flushHostRegistryNow();
      }, CONFIG.hostRegistryFlushMs); // configurable debounce
    },

    _flushHostRegistryNow() {
      if (!this._registryDirty || !this._hostRegistry) return;
      this._registryDirty = false;
      try {
        const obj = {};
        for (const [host, entry] of this._hostRegistry) {
          obj[host] = entry;
        }
        GM_setValue('uvp-host-registry', JSON.stringify(obj));
      } catch (e) { /* GM storage might be unavailable (AdGuard) */ }
    },

    _getHostEntry(host) {
      if (!host) return null;
      const reg = this._getHostRegistry();
      const key = this._regKey(host);
      const entry = reg.get(key);
      if (!entry) return null;
      // TTL check
      if (Date.now() - (entry.lastSeen || 0) >= CONFIG.hostRegistryTtlMs) {
        reg.delete(key);
        // Persist the prune — otherwise the expired entry reloads next session.
        this._flushHostRegistry();
        return null;
      }
      return entry;
    },

    _recordHostEntry(host, updates) {
      if (!host) return;
      const reg = this._getHostRegistry();
      const key = this._regKey(host);
      const existing = reg.get(key) || {
        cors: 'unknown',
        server: null,
        preferredTransport: 'probe',
        pageOk: 0,          // page-transport successes
        gmOk: 0, gmFail: 0, // GM_xhr outcomes (informational + unpin logic)
        failOpaque: 0,      // page failures WITHOUT an HTTP status (TypeError/abort/stall) — the only CORS evidence
        failHttp: 0,        // page failures WITH an HTTP status — server reached; never CORS evidence
        lastFailKind: null,
        lastSuccessAt: 0,
        lastFailAt: 0,
        lastProbeAt: 0,     // last gm-mode page re-probe stamp (throttles self-healing)
        acceptsRanges: null,
        cdnType: null,
        lastSeen: 0,
      };
      const updated = Object.assign({}, existing, updates, { lastSeen: Date.now() });
      reg.set(key, updated);
      this._registryDirty = true;
      this._flushHostRegistry();
    },

    // Time-decayed outcome counters. Old evidence fades with a half-life
    // (hostScoreHalfLifeMs) so a host whose behavior changed migrates back
    // toward 'probe' without waiting out the full registry TTL.
    _decayedCounts(entry, now) {
      let s = entry.pageOk || 0;
      let fo = entry.failOpaque || 0;
      let fh = entry.failHttp || 0;
      const half = CONFIG.hostScoreHalfLifeMs || 0;
      if (half > 0 && entry.lastSeen) {
        const f = Math.pow(0.5, Math.max(0, now - entry.lastSeen) / half);
        s *= f; fo *= f; fh *= f;
      }
      return { s: s, fo: fo, fh: fh, total: s + fo + fh };
    },

    // Pure transport-mode computation (no side effects — safe for logging).
    //   >= hostScorePageThreshold (Bayesian) → page-transport fast path
    //   < hostScoreGmThreshold AND opaque-failure dominance → GM_xhr
    //   otherwise → probe (page first, GM fallback)
    // 'gm' additionally requires CORS-type evidence: an observed missing ACAO
    // header, or statusless failures dominating statused ones. HTTP-statused
    // failures (403 storms, 5xx) can demote 'page' to 'probe' but NEVER to
    // 'gm' — the server was reached, so GM_xhr (same URL, worse Referer)
    // would fail identically.
    _computeHostMode(entry, now) {
      if (!entry) return 'probe'; // unknown host — probe (existing behavior)
      // If we've observed missing CORS headers, skip page-transport entirely
      if (entry.cors === 'missing') return 'gm';
      const c = this._decayedCounts(entry, now || Date.now());
      if (c.total < 3) return 'probe'; // not enough data — probe
      // Beta(1,1) smoothing: 2/2 and 200/200 no longer both score 1.0
      const score = (c.s + 1) / (c.total + 2);
      const gmEvidence = c.fo >= (CONFIG.hostOpaqueGmMinFails || 2) && c.fo > c.fh;
      if (score >= CONFIG.hostScorePageThreshold && !gmEvidence) return 'page';
      if (score < CONFIG.hostScoreGmThreshold && gmEvidence) return 'gm';
      return 'probe';
    },

    // Decision entry point for loaders. Like _computeHostMode, plus gm
    // self-healing: a gm-classified host is allowed ONE page-transport
    // re-probe per hostReprobeMs window. lastProbeAt is stamped and persisted
    // BEFORE returning so concurrent segment loads (and other tabs) stay on
    // GM for the window — exactly one probe flies.
    hostTransportMode(host) {
      const entry = this._getHostEntry(host);
      const mode = this._computeHostMode(entry, Date.now());
      if (mode !== 'gm' || !entry) return mode;
      const now = Date.now();
      if (now - (entry.lastProbeAt || 0) >= (CONFIG.hostReprobeMs || 0)) {
        this._recordHostEntry(host, { lastProbeAt: now });
        return 'probe';
      }
      return 'gm';
    },
    ...
    // ---- Outcome recording ----
    // Page transport resolved: proof CORS works for this (origin, host).
    recordPageOutcome(host, ok, err) {
      if (!host) return;
      const entry = this._getHostEntry(host);
      if (ok) {
        this._recordHostEntry(host, {
          pageOk: ((entry && entry.pageOk) || 0) + 1,
          cors: 'present', // a resolved cross-origin fetch IS the ACAO proof
          lastSuccessAt: Date.now(),
        });
        return;
      }
      // Classify the failure: an HTTP status means the server was reached —
      // auth/token/server trouble, NOT missing CORS. Only statusless failures
      // (TypeError/abort/stall) count as page-transport-can't-work evidence.
      const hasStatus = !!(err && typeof err.status === 'number' && err.status > 0);
      this._recordHostEntry(host, {
        failOpaque: ((entry && entry.failOpaque) || 0) + (hasStatus ? 0 : 1),
        failHttp: ((entry && entry.failHttp) || 0) + (hasStatus ? 1 : 0),
        lastFailKind: hasStatus ? 'http' : 'opaque',
        lastFailAt: Date.now(),
      });
    },

    // GM_xhr resolved: informational. GM_xhr failed: the gm classification is
    // no longer trustworthy (old behavior: the GM-only pin dropped on GM
    // failure) — zero the re-probe stamp so the next load re-probes
    // page-transport at once instead of waiting out hostReprobeMs.
    recordGmOutcome(host, ok) {
      if (!host) return;
      const entry = this._getHostEntry(host);
      if (ok) {
        this._recordHostEntry(host, { gmOk: ((entry && entry.gmOk) || 0) + 1, lastSuccessAt: Date.now() });
        return;
      }
      this._recordHostEntry(host, {
        gmFail: ((entry && entry.gmFail) || 0) + 1,
        lastProbeAt: 0,
        lastFailAt: Date.now(),
      });
    },

    // ---- API-compatible methods (same signatures, registry-backed) ----
    // Side-effect-free mode query (no re-probe stamping) — safe for logging
    // paths. Loaders must use hostTransportMode() for the actual decision.
    isHostGmOnly(host) {
      if (!host) return false;
      return this._computeHostMode(this._getHostEntry(host), Date.now()) === 'gm';
    },

    // Backward-compat wrappers — delegate to the classified outcome recorders.
    markHostGmOnly(host, err) { this.recordPageOutcome(host, false, err); },
    clearHostGmOnly(host) { this.recordPageOutcome(host, true); },

    // Replaces getHostProfile — returns the registry entry
    getHostProfile(host) {
      return this._getHostEntry(host);
    },

    // Replaces recordHostProfile — now captures more headers and records to the persistent registry
    recordHostProfile(host, headers, corsHint) {
      if (!host) return;
      try {
        const hdrs = headers || {};
        const existing = this._getHostEntry(host);
        let cors = (existing && existing.cors) || 'unknown';
        if (corsHint === 'present' || corsHint === 'missing') {
          cors = corsHint;
        } else if (Object.prototype.hasOwnProperty.call(hdrs, 'access-control-allow-origin')) {
          const acao = hdrs['access-control-allow-origin'];
          cors = (acao == null || acao === '' || acao === 'null') ? 'missing' : 'present';
        }
        const server = hdrs['server'] || (existing && existing.server) || null;
        const acceptRanges = hdrs['accept-ranges'] || (existing && existing.acceptsRanges) || null;
        const cdnType = hdrs['x-cache'] || hdrs['x-cdn'] || hdrs['via'] || (existing && existing.cdnType) || null;
        this._recordHostEntry(host, { cors, server, acceptsRanges: acceptRanges, cdnType });
      } catch (e) {}
    },

    // Replaces hostPrefersGm — side-effect-free like isHostGmOnly
    hostPrefersGm(host) {
      return this.isHostGmOnly(host);
    },

    // Replaces clearHostProfile — clears a specific host entry (current origin)
    clearHostProfile(host) {
      if (!host) return;
      const reg = this._getHostRegistry();
      const key = this._regKey(host);
      if (reg.has(key)) {
    ...
    // Registry snapshot for debugging (mirrors SegmentCache.getMetrics).
    // Pure read — no re-probe stamping, no flushes.
    getMetrics() {
      const reg = this._getHostRegistry();
      const now = Date.now();
      const modes = { page: 0, probe: 0, gm: 0 };
      const hosts = [];
      for (const [key, entry] of reg) {
        const mode = this._computeHostMode(entry, now);
        modes[mode]++;
        const loads = (entry.pageOk || 0) + (entry.failOpaque || 0) + (entry.failHttp || 0) + (entry.gmOk || 0) + (entry.gmFail || 0);
        hosts.push({
          key: key,
          mode: mode,
          cors: entry.cors,
          pageOk: entry.pageOk || 0,
          failOpaque: entry.failOpaque || 0,
          failHttp: entry.failHttp || 0,
          gmOk: entry.gmOk || 0,
          gmFail: entry.gmFail || 0,
          lastFailKind: entry.lastFailKind || null,
          loads: loads,
          ageMin: Math.round((now - (entry.lastSeen || now)) / 60000)
        });
      }
      hosts.sort((a, b) => b.loads - a.loads);
      return { entries: reg.size, modes: modes, hosts: hosts.slice(0, 20) };
    },
    // ---- Manifest Cache (VOD only; avoids re-fetching unchanged playlists) ----
    // During recovery and level switches, hls.js re-requests the same manifest.
    // A 5s in-memory cache eliminates the redundant fetch. Live manifests are
    // never cached (sliding window changes between requests).
    _manifestCache: new Map(),
    getManifestCache(key) {
      if (!key) return null;
      const e = this._manifestCache.get(key);
      if (!e) return null;
      if (Date.now() - e.at >= CONFIG.manifestCacheTtlMs) {
        this._manifestCache.delete(key);
        return null;
      }
      return e;
    },
    setManifestCache(key, data, headers) {
      if (!key || !data) return;
      try {
        this._manifestCache.set(key, { data, headers: headers || {}, at: Date.now() });
      } catch (e) {}
    },
    clearManifestCache() {
      this._manifestCache.clear();
    },
    // Log-only URL redaction. Never used to build a request URL.
    // Only the listed query keys are masked; the rest of the URL is kept so
    // host/path/resource-type remain diagnosable.
    redactUrl(url) {
      if (!url || typeof url !== 'string') return String(url || '');
      try {
        const u = new URL(url, location.href);
        for (const key of ['token', 'sig', 'signature', 'expire', 'expires', 'auth']) {
    ...
    classifyBody(data, type) {
      if (type === 'text') {
        const text = (data == null) ? '' : String(data);
        const trimmed = text.replace(/^\uFEFF/, '').trimStart();
        if (trimmed.startsWith('#EXTM3U')) return 'hls-playlist';
        if (/<MPD[\s>]/i.test(trimmed.slice(0, 2048))) return 'dash-mpd';
        return 'text/unknown';
      }
      try {
        const raw = (data && data.buffer instanceof ArrayBuffer && typeof data.byteLength === 'number')
          ? data
          : data;
        const bytes = (raw && raw.byteLength != null) ? new Uint8Array(raw) : new Uint8Array(0);
        let textPrefix = '';
        try { textPrefix = new TextDecoder().decode(bytes.slice(0, 32)); } catch (e) { textPrefix = ''; }
        return {
          firstBytes: Array.from(bytes.slice(0, 16)).map((b) => b.toString(16).padStart(2, '0')).join(' '),
          textPrefix
        };
      } catch (e) {
        return { firstBytes: '', textPrefix: '' };
      }
    },
    hlsResourceType(context, type) {
      const t = (context && context.type) || '';
      if (t === 'key') return 'key';
      if (t === 'frag' || t === 'part') return 'fragment';
      if (t === 'manifest' || t === 'level' || t === 'audioTrack' || t === 'subtitleTrack') return 'playlist';
      return (type === 'arraybuffer') ? 'fragment' : 'playlist';
    },
    dashResourceType(request, url) {
      const t = String((request && request.type) || '');
      if (/MPD|Manifest|XLink/i.test(t) || /\.(?:mpd|m3u8)(?:[?#]|$)/i.test(url || '')) return 'mpd';
      if (/Initialization/i.test(t)) return 'init';
      if (/Index|SIDX|sidx/i.test(t)) return 'index';
      if (/Media|Fragment|Bitstream/i.test(t)) return 'fragment';
      return (request && request.responseType === 'text') ? 'mpd' : 'fragment';
    },
    // Metadata documents must not be treated as media. Keys may be text or
    // raw bytes. Fragments that are clearly playlists/HTML are invalid.
    isPlausiblePlaybackBody(data, type, resourceType) {
      const kind = this.classifyBody(data, type);
      if (resourceType === 'playlist') return kind === 'hls-playlist';
      if (resourceType === 'mpd') return kind === 'dash-mpd';
      if (resourceType === 'key') {
        if (type === 'arraybuffer') return !!(data && data.byteLength > 0);
        return typeof data === 'string' && data.length > 0;
      }
      if (resourceType === 'fragment' || resourceType === 'init' || resourceType === 'index') {
        if (type === 'arraybuffer') {
          const len = (data && data.byteLength) || 0;
          if (len <= 0) return false;
          const prefix = (kind && kind.textPrefix) || '';
          if (prefix.startsWith('#EXTM3U')) return false;
          if (/<MPD[\s>]/i.test(prefix)) return false;
          if (/^\s*<!DOCTYPE/i.test(prefix) || /^\s*<html/i.test(prefix)) return false;
          return true;
        }
        return data != null;
      }
    ...
    // Throughput sample from a fallback transfer. GM_xhr / cache hits often
    // report first-byte and last-byte in the same millisecond, which hls.js
    // and dash.js treat as infinite bandwidth → ABR slams the top rung.
    // A hung CORS XHR whose clock is reused for the fallback does the
    // opposite (bytes / 20s hang → bottom rung). Floor the window and cap
    // the result so one bad timer cannot pin the ladder.
    measuredBitrateBps(bytes, startMs, firstMs, endMs) {
      const size = Number(bytes) || 0;
      const end = Number(endMs) || 0;
      const start = Number(startMs) || 0;
      const first = Number(firstMs) || 0;
      if (size <= 0 || end <= 0) return 0;
      const MIN_MS = 80;
      const firstWin = (first > 0 && end > first) ? (end - first) : 0;
      const totalWin = (start > 0 && end > start) ? (end - start) : 0;
      const ms = firstWin >= MIN_MS ? firstWin : Math.max(MIN_MS, totalWin, firstWin);
      const bps = (size * 8000) / ms;
      return Math.max(1, Math.min(150000000, bps));
    },
    logTransportDiag(diag) {
      try {
        const safe = {};
        const keys = ['phase', 'transport', 'resourceType', 'method', 'status', 'originalUrl', 'rewrittenUrl', 'finalUrl', 'responseType', 'retryDecision', 'host', 'hostPinned', 'timeout', 'rangePresent', 'bodyClass'];
        for (const k of keys) {
          if (diag[k] == null) continue;
          if (k === 'originalUrl' || k === 'rewrittenUrl' || k === 'finalUrl') safe[k] = this.redactUrl(diag[k]);
          else safe[k] = diag[k];
        }
        console.warn('[UVP] transport', safe);
      } catch (e) {}
    },
    ...
    makeLoader(HlsLib, tokenCtx) {
      ...
      class UvpEngineLoader extends HlsLib.DefaultConfig.loader {
        constructor(cfg) { super(cfg); this._uvpAbortFn = null; this._uvpSettled = true; this._uvpProfile = null; }
        ...
        load(context, config, callbacks) {
          // Cache hit check (VOD only).
          if (CONFIG.enablePlaybackPrefetch && !state.overlayIsLive && context.responseType === 'arraybuffer') {
            try {
              const _rt = PT.hlsResourceType(context, 'arraybuffer');
              if (_rt === 'key') { /* DRM/encryption key — never cache */ }
              else {
                const _ck = PT.rewriteUrl(context.url, tokenCtx);
                const _cd = UVP.SegmentCache.get(_ck);
                if (_cd) {
                  if (DEBUG) console.log('[UVP] SegmentCache HIT:', PT.redactUrl(_ck));
                  this._uvpSettled = true;
                  if (!this.stats) this.stats = { aborted: false, loaded: _cd.byteLength, retry: 0, total: _cd.byteLength, chunkCount: 1, bwEstimate: 0, loading: { start: 0, first: 0, end: 0 }, parsing: { start: 0, end: 0 }, buffering: { start: 0, first: 0, end: 0 } };
                  const _ct = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
                  // Zero-duration cache hits look like infinite bandwidth to
                  // hls.js AbrController. Stamp a window that yields the
                  // startup estimate so a hit does not yank ABR to 4K.
                  const _hitBps = CONFIG.abrHlsStartupEstimateBps || 1500000;
                  const _hitMs = Math.max(80, (_cd.byteLength * 8000) / _hitBps);
                  this.stats.loading.end = this.stats.tload = _ct;
                  this.stats.loading.start = this.stats.loading.first = this.stats.tfirst = _ct - _hitMs;
                  this.stats.bwEstimate = _hitBps;
                  callbacks.onSuccess({ url: _ck, data: _cd.slice(0), code: 200, headers: {} }, this.stats, context, { status: 200, readyState: 4, getResponseHeader: () => null });
                  return;
                }
              }
            } catch (e) { if (DEBUG) console.warn('[UVP] SegmentCache hit-check error:', e); }
          }
          ...
          if (DEBUG && finalUrl !== originalUrl) {
            try {
              console.debug('[UVP] HLS rewrite', {
                original: PT.redactUrl(originalUrl),
                rewritten: PT.redactUrl(finalUrl),
                master: PT.redactUrl(tokenCtx && tokenCtx.masterUrl)
              });
            } catch (e) {}
          }
          const type = (context.responseType === 'arraybuffer') ? 'arraybuffer' : 'text';
          const resourceType = PT.hlsResourceType(context, type);
          const isManifestClass = (resourceType === 'playlist');
          const self = this;
          ...
          const startFallback = (originalCallbacks) => {
            ...
            const retries = self._uvpProfile ? self._uvpProfile.retries : (isManifestClass ? 1 : 2);
            const timeoutMs = self._uvpProfile ? self._uvpProfile.timeoutMs : (isManifestClass ? CONFIG.manifestTimeoutMs : CONFIG.segTimeoutMs);
            const headers = {};
            try {
              if (context.headers) {
                for (const k in context.headers) {
                  if (Object.prototype.hasOwnProperty.call(context.headers, k)) headers[k] = context.headers[k];
                }
              }
            } catch (e) {}
            try {
              if ((context.rangeStart != null || context.rangeEnd != null) && !headers['Range'] && !headers['range']) {
                headers['Range'] = 'bytes=' + (context.rangeStart || 0) + '-' + (context.rangeEnd ? (context.rangeEnd - 1) : '');
              }
            } catch (e) {}
            const finishSuccess = (data, headerMap) => {
              if (!scope.check()) return;
              // Store in cache (VOD only, skip keys/DRM). has() prevents double-store.
              if (CONFIG.enablePlaybackPrefetch && !state.overlayIsLive && type === 'arraybuffer' && data && data.byteLength && resourceType !== 'key') {
                try { if (!UVP.SegmentCache.has(finalUrl)) UVP.SegmentCache.set(finalUrl, data.slice(0)); } catch (e) { if (DEBUG) console.warn('[UVP] SegmentCache store error:', e); }
              }
              if (!PT.isPlausiblePlaybackBody(data, type, resourceType)) {
                const bodyClass = PT.classifyBody(data, type);
                PT.logTransportDiag({
                  phase: 3, transport: 'gm-or-page', resourceType, method: 'GET', status: 200,
                  originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
                  retryDecision: 'reject-invalid-body', host: _host || '',
                  hostPinned: PT.isHostGmOnly(_host), timeout: timeoutMs,
                  rangePresent: !!(headers['Range'] || headers['range']),
                  bodyClass: typeof bodyClass === 'string' ? bodyClass : ((bodyClass && bodyClass.textPrefix) || '')
                });
                finishError(Object.assign(new Error('invalid playback body'), { status: 200 }));
                return;
              }
              const end = now();
              ...
              const _bps = PT.measuredBitrateBps(stats.total, stats.loading.start, stats.loading.first, stats.loading.end);
              if (_bps > 0) {
                stats.bwEstimate = _bps;
                // hls.js AbrController recomputes from tload-tfirst. Keep that
                // window consistent with the floored estimate so a same-ms
                // GM_xhr progress burst cannot become Infinity bps.
                const _needMs = (stats.total * 8000) / _bps;
                if (!(stats.loading.end > stats.loading.first) || (stats.loading.end - stats.loading.first) < _needMs) {
                  stats.loading.first = stats.tfirst = stats.loading.end - _needMs;
                  if (!stats.loading.start || stats.loading.start > stats.loading.first) stats.loading.start = stats.loading.first;
                }
              }
              const hm = headerMap || {};
            ...
            const finishError = (err) => {
              ...
              PT.logTransportDiag({
                phase: 3, transport: 'gm-or-page', resourceType, method: 'GET', status,
                originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
                retryDecision: /stalled|timeout/i.test(String(msg || '')) ? 'timeout' : 'fail',
                host: _host || '', hostPinned: PT.isHostGmOnly(_host), timeout: timeoutMs,
                rangePresent: !!(headers['Range'] || headers['range'])
              });
            ...
            const runEngine = () => {
              ...
              const runGmEngine = () => {
                UVP.Network.requestCrossOriginWithRetry(finalUrl, type, progress, headers, retries, timeoutMs, scope, onGmResponse)
                  .then((data) => { PT.recordGmOutcome(_host, true); finishSuccess(data, gmHdrs); })
                  .catch((err) => {
                    // GM_xhr failed too — the gm classification is no longer
                    // trustworthy; recordGmOutcome(false) zeroes the re-probe
                    // stamp so the next load re-probes page-transport at once
                    // (GM_xhr's wrong Referer 403s on some CDNs; a transient
                    // page-fetch failure must not lock the host onto a dead
                    // transport).
                    PT.recordGmOutcome(_host, false);
                    finishError(err);
                  });
              };
              // Single stamped decision: gm-classified hosts may be re-probed
              // (one page attempt per hostReprobeMs) — never evaluate the mode
              // twice per load or the stamp flip-flops mid-expression.
              if (PT.hostTransportMode(_host) === 'gm') {
                runGmEngine();
              } else {
                UVP.Network.requestPageContext(finalUrl, type, headers, progress, scope, onPageResponse)
                  .then((data) => {
                    PT.recordPageOutcome(_host, true);
                    finishSuccess(data, gmHdrs);
                  })
                  .catch((err) => {
                    PT.logTransportDiag({
                      phase: 2, transport: 'page', resourceType, method: 'GET',
                      status: (err && typeof err.status === 'number') ? err.status : 0,
                      originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
                      retryDecision: 'phase3-gm', host: _host || '', hostPinned: false,
                      timeout: timeoutMs, rangePresent: !!(headers['Range'] || headers['range'])
                    });
                    // Classified failure: only statusless (CORS-type) errors
                    // count toward a gm classification.
                    PT.recordPageOutcome(_host, false, err);
                    runGmEngine();
                  });
              }
            };
          ...
          callbacks.onSuccess = (response, stats, context, networkDetails) => {
            ...
            if (!fallbackStarted && !PT.isPlausiblePlaybackBody(data, type, resourceType)) {
              const bodyClass = PT.classifyBody(data, type);
              PT.logTransportDiag({
                phase: 1, transport: 'xhr', resourceType, method: 'GET',
                status: (response && response.code) || 200,
                originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
                retryDecision: 'phase2-page', host: '', hostPinned: false,
                timeout: isManifestClass ? CONFIG.manifestTimeoutMs : CONFIG.segTimeoutMs,
                rangePresent: !!(context && (context.rangeStart != null || context.rangeEnd != null)),
                bodyClass: typeof bodyClass === 'string' ? bodyClass : ((bodyClass && bodyClass.textPrefix) || '')
              });
              startFallback(fallbackCbs);
              return;
            }
            self._uvpSettled = true;
            self._uvpAbortFn = null;
          ...
          callbacks.onError = (error, ctx, networkDetails, stats) => {
            if (!fallbackStarted) {
              PT.logTransportDiag({
                ...
                retryDecision: 'phase2-page', host: '', hostPinned: false,
                timeout: isManifestClass ? CONFIG.manifestTimeoutMs : CONFIG.segTimeoutMs,
                rangePresent: !!(ctx && (ctx.rangeStart != null || ctx.rangeEnd != null))
              });
              startFallback(fallbackCbs);
              return;
            }
          ...
          if (callbacks.onTimeout) {
            callbacks.onTimeout = (stats, ctx, networkDetails) => {
              if (!fallbackStarted) {
                PT.logTransportDiag({
                  phase: 1, transport: 'xhr', resourceType, method: 'GET', status: 0,
                  originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
                  retryDecision: 'phase2-page-timeout', host: '', hostPinned: false,
                  timeout: isManifestClass ? CONFIG.manifestTimeoutMs : CONFIG.segTimeoutMs,
                  rangePresent: !!(ctx && (ctx.rangeStart != null || ctx.rangeEnd != null))
                });
                startFallback(fallbackCbs);
          ...
          callbacks.onProgress = (stats, ctx, data, networkDetails) => {
            UVP.Overlay.noteEngineActivity();
            if (origOnProgress) origOnProgress(stats, ctx, data, networkDetails);
          };
          UVP.Overlay.noteEngineActivity();
          try {
            ...
            PT.logTransportDiag({
              phase: 1, transport: 'xhr', resourceType, method: 'GET', status: 0,
              originalUrl, rewrittenUrl: finalUrl, finalUrl, responseType: type,
              retryDecision: 'phase2-page-exception', host: '', hostPinned: false,
              timeout: isManifestClass ? CONFIG.manifestTimeoutMs : CONFIG.segTimeoutMs,
              rangePresent: !!(context && (context.rangeStart != null || context.rangeEnd != null))
            });
            startFallback(fallbackCbs);
    ...
    // Specialized loaders wrap the base UvpEngineLoader (full native-XHR →
    // page-transport → GM_xhr fallback chain) with resource-specific tuning.
    // Each sets _uvpProfile to override the default retry/timeout values inside
    // startFallback; when _uvpProfile is null the base behavior is unchanged.
    // ManifestLoader adds an in-memory manifest cache (VOD only) so recovery
    // and level switches don't re-fetch unchanged playlists.
    makeManifestLoader(HlsLib, tokenCtx) {
      const BaseLoader = this.makeLoader(HlsLib, tokenCtx);
      const PT = this;
      class UvpManifestLoader extends BaseLoader {
        constructor(cfg) { super(cfg); this._uvpProfile = { retries: 3, timeoutMs: CONFIG.manifestTimeoutMs }; }
        load(context, config, callbacks) {
          // Manifest cache: VOD only (live manifests change with the sliding window)
          if (!state.overlayIsLive) {
            try {
              const cacheKey = PT.rewriteUrl(context.url, tokenCtx);
              const cached = PT.getManifestCache(cacheKey);
              if (cached) {
                if (DEBUG) console.log('[UVP] Manifest cache HIT:', PT.redactUrl(cacheKey));
                this._uvpSettled = true;
                this.context = context;
                try { this.callbacks = callbacks; } catch (e) {}
                const now = ((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now());
                ...
                this.stats.loading.start = this.stats.loading.first = this.stats.loading.end = now;
                this.stats.tfirst = this.stats.tload = now;
                this.stats.loaded = this.stats.total = (typeof cached.data === 'string') ? cached.data.length : (cached.data && cached.data.byteLength) || 0;
                callbacks.onSuccess(
                  { url: cacheKey, data: cached.data, code: 200, headers: cached.headers },
                  this.stats, context,
                  { status: 200, readyState: 4, getResponseHeader: (name) => cached.headers[name.toLowerCase()] || null }
                );
                return;
              }
              // Wrap onSuccess to cache the manifest after a successful fetch
              const origSuccess = callbacks.onSuccess;
              callbacks.onSuccess = function (response, stats, ctx, details) {
                if (response && response.data) {
                  PT.setManifestCache(cacheKey, response.data, response.headers || {});
                }
                if (origSuccess) origSuccess(response, stats, ctx, details);
              };
            } catch (e) {}
          }
      ...
      return UvpManifestLoader;
    },
    makeFragmentLoader(HlsLib, tokenCtx) {
      const BaseLoader = this.makeLoader(HlsLib, tokenCtx);
      class UvpFragmentLoader extends BaseLoader {
        constructor(cfg) { super(cfg); this._uvpProfile = { retries: CONFIG.segMaxRetries, timeoutMs: CONFIG.segTimeoutMs }; }
      }
      return UvpFragmentLoader;
    },
    makeKeyLoader(HlsLib, tokenCtx) {
      const BaseLoader = this.makeLoader(HlsLib, tokenCtx);
      class UvpKeyLoader extends BaseLoader {
        // Keys are tiny (16-32B for AES-128). Short timeout + 1 retry → fast-fail
        // so recovery escalates immediately instead of waiting through segTimeoutMs.
        constructor(cfg) { super(cfg); this._uvpProfile = { retries: 1, timeoutMs: CONFIG.keyTimeoutMs }; }
      }
      return UvpKeyLoader;
    },
    // XHRLoader child for FactoryMaker.extend('XHRLoader', fn, true).
    // Raw constructor only — NOT getClassFactory(): FactoryMaker's override
    // merge creates the stock instance and copies onto it only properties
    // that already exist on it. A factory returns { create }, which the stock
    // instance lacks, so nothing would be copied — the extension would be a
    // silent no-op. Capture the stock functions before that assign or load
    // recurses.
    makeDashXHRLoader(DashLib) {
      function UvpDashXHRLoader() {
        const parent = this && this.parent;
        if (!parent) {
          if (DEBUG) console.warn('[UVP] XHRLoader wrap: no this.parent (extend hook invalid)');
          return {};
        }
        const parentLoad = parent.load;
        const parentAbort = parent.abort;
        const parentGetXhr = parent.getXhr;
        const parentReset = parent.reset;
        const parentResetInitialSettings = parent.resetInitialSettings;
        if (typeof parentLoad !== 'function') {
          if (DEBUG) console.warn('[UVP] XHRLoader wrap: parent.load missing');
          return {};
        }
        const _fallbackScopes = new Set();
        const abortFallbackScope = (scope) => {
          if (!scope) return;
          _fallbackScopes.delete(scope);
          try { scope.abort(); } catch (e) {}
        };
        const abortAllFallbacks = () => {
          for (const scope of Array.from(_fallbackScopes)) abortFallbackScope(scope);
        };
        // Transport death only. A 403/404 is an HTTP answer — dash.js
        // already retries those. Re-running page+GM on every 4xx storms
        // the CDN and can pin the host to GM from a real client error.
        const isNativeTransportFailure = (status) =>
          status == null || status === 0 || status === 408 || status === 429 || (status >= 500 && status < 600);
        // Per-resource timeout/retry profiles for the fallback engine:
        // dash.js routes every resource type through this one XHRLoader, so
        // the tuning is selected dynamically per request via dashResourceType().
        // Unknown types fall back to the fragment profile.
        const _resourceProfiles = {
          mpd:      { retries: CONFIG.dashMpdRetries,     timeoutMs: CONFIG.dashMpdTimeoutMs },
          init:     { retries: CONFIG.dashInitRetries,    timeoutMs: CONFIG.dashInitTimeoutMs },
          index:    { retries: CONFIG.dashIndexRetries,   timeoutMs: CONFIG.dashIndexTimeoutMs },
          fragment: { retries: CONFIG.dashSegmentRetries, timeoutMs: CONFIG.dashSegmentTimeoutMs }
        };
        return {
          load: function (commonMediaRequest, commonMediaResponse) {
            try {
              // Request issued — not bytes. isDashInGrace treats this as fill evidence after seekStartTime.
              if (state.dashActivity) state.dashActivity.lastLoaderTime = Date.now();
            } catch (e) {}
            // Transport fallback: wrap customData.onloadend to retry via
            // page-fetch → GM_xhr when native XHR dies (status 0 / timeout /
            // 408 / 429 / 5xx). dash.js HTTPLoader stays in charge of traces
            // and addHttpRequest; this wrap only substitutes the body.
            const cd = commonMediaRequest && commonMediaRequest.customData;
            if (cd && typeof cd.onloadend === 'function') {
              const origOnLoadEnd = cd._uvpOrigOnLoadEnd || cd.onloadend;
              cd._uvpOrigOnLoadEnd = origOnLoadEnd;
              const url = commonMediaRequest.url || '';
              const responseType = commonMediaRequest.responseType || 'arraybuffer';
              const type = responseType === 'arraybuffer' ? 'arraybuffer' : 'text';
              const reqHeaders = {};
              if (commonMediaRequest.headers) {
                for (const k in commonMediaRequest.headers) {
                  if (commonMediaRequest.headers[k]) reqHeaders[k] = commonMediaRequest.headers[k];
                }
              }
              const dashReq = cd.request || {};
              if (dashReq.range && !reqHeaders['Range'] && !reqHeaders['range']) {
                reqHeaders['Range'] = 'bytes=' + dashReq.range;
              }
              const resourceType = UVP.PlaybackTransport.dashResourceType(dashReq, url);
              const profile = _resourceProfiles[resourceType] || _resourceProfiles.fragment;
              const timeoutMs = (commonMediaRequest.timeout || 0) > 0
                ? commonMediaRequest.timeout
                : profile.timeoutMs;
              cd.onloadend = function () {
                const status = commonMediaResponse && commonMediaResponse.status;
                if (isNativeTransportFailure(status) && url) {
                  const scope = UVP.PlaybackTransport.makeScope();
                  _fallbackScopes.add(scope);
                  cd._uvpFallbackScope = scope;
                  console.warn('[UVP] XHRLoader transport fallback:', UVP.PlaybackTransport.redactUrl(url), 'status:', status);
                  const fallbackStartedAt = Date.now();
                  let lastProgressLoaded = 0;
                  // Reset HTTPLoader's lastTraceTime so the failed native XHR
                  // (CORS hang can be seconds) is not counted as download time.
                  // event.time=1 makes the discarded interval 1ms, not the hang.
                  if (cd.onprogress) {
                    try { cd.onprogress({ loaded: 0, total: 0, lengthComputable: false, time: 1 }); } catch (e) {}
                  }
                  const onProgress = (loaded, total) => {
                    if (!scope.check()) return;
                    lastProgressLoaded = loaded || 0;
                    if (cd.onprogress) {
                      try { cd.onprogress({ loaded, total, lengthComputable: total > 0 }); } catch (e) {}
                    }
                  };
                  const deliverFallback = (data, meta) => {
                    if (!scope.check()) return;
                    const headers = (meta && meta.headers) || {};
                    let outStatus = (meta && typeof meta.status === 'number' && meta.status > 0) ? meta.status : 200;
                    if (outStatus < 200 || outStatus >= 300) outStatus = 200;
                    const bytes = (data && data.byteLength) || (typeof data === 'string' ? data.length : lastProgressLoaded) || 0;
                    // GM_xhr often fires a single onprogress at completion.
                    // Stamp a final trace whose duration is the real fallback
                    // window so dash.js ThroughputRule sees kbps, not Infinity.
                    if (cd.onprogress && bytes > 0) {
                      const elapsed = Math.max(80, Date.now() - fallbackStartedAt);
                      try {
                        cd.onprogress({
                          loaded: bytes,
                          total: bytes,
                    ...
                    abortFallbackScope(scope);
                    if (cd._uvpFallbackScope === scope) cd._uvpFallbackScope = null;
                    if (commonMediaResponse) {
                      commonMediaResponse.url = url;
                      commonMediaResponse.status = outStatus;
                      commonMediaResponse.statusText = outStatus === 206 ? 'Partial Content' : 'OK';
                      commonMediaResponse.headers = headers;
                      commonMediaResponse.data = data;
                      try {
                        if (!commonMediaResponse.resourceTiming) commonMediaResponse.resourceTiming = {};
                        const rt = commonMediaResponse.resourceTiming;
                        const nowPerf = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
                        const elapsed = Math.max(80, Date.now() - fallbackStartedAt);
                        rt.encodedBodySize = bytes;
                        rt.responseEnd = nowPerf;
                        if (rt.startTime == null) rt.startTime = nowPerf - elapsed;
                        if (rt.responseStart == null) rt.responseStart = rt.startTime;
                        rt.duration = elapsed;
                      } catch (e) {}
                    }
                    if (DEBUG) console.debug('[UVP] XHRLoader transport fallback succeeded:', UVP.PlaybackTransport.redactUrl(url), outStatus);
                    try { origOnLoadEnd(); } catch (e) {}
                  };
                  const failFallback = () => {
                    if (!scope.check()) return;
                    abortFallbackScope(scope);
                    if (cd._uvpFallbackScope === scope) cd._uvpFallbackScope = null;
                    console.warn('[UVP] XHRLoader transport fallback failed:', UVP.PlaybackTransport.redactUrl(url));
                    try { origOnLoadEnd(); } catch (e) {}
                  };
                  const runGmEngine = () => {
                    if (!scope.check()) return;
                    const gmMeta = { status: 200, headers: {} };
                    UVP.Network.requestCrossOriginWithRetry(url, type, onProgress, reqHeaders, profile.retries, timeoutMs, scope, (res) => {
                      if (res && typeof res.status === 'number') gmMeta.status = res.status;
                      gmMeta.headers = UVP.Utils.parseResponseHeaders(res);
                    })
                      .then((data) => {
                        if (!scope.check()) return;
                        if (!UVP.PlaybackTransport.isPlausiblePlaybackBody(data, type, resourceType)) {
                          failFallback();
                        } else {
                          UVP.PlaybackTransport.recordGmOutcome(host, true);
                          deliverFallback(data, gmMeta);
                        }
                      })
                      .catch(() => { UVP.PlaybackTransport.recordGmOutcome(host, false); failFallback(); });
                  };
                  let host = '';
                  try { host = new URL(url).hostname; } catch (e) {}
                  // Single stamped decision (gm hosts may re-probe page once
                  // per hostReprobeMs — a second evaluation would see the
                  // stamp and flip back to gm).
                  if (UVP.PlaybackTransport.hostTransportMode(host) === 'gm') {
                    runGmEngine();
                  } else {
                    const pageMeta = { status: 200, headers: {} };
                    UVP.Network.requestPageContext(url, type, reqHeaders, onProgress, scope, (res) => {
                      if (res && typeof res.status === 'number') pageMeta.status = res.status;
                      pageMeta.headers = UVP.Utils.parseResponseHeaders(res);
                    })
                      .then((data) => {
                        if (!scope.check()) return;
                        // Transport resolved — that alone is CORS proof. An
                        // implausible body is a content problem, not transport
                        // evidence, so it records success and falls to GM.
                        UVP.PlaybackTransport.recordPageOutcome(host, true);
                        if (!UVP.PlaybackTransport.isPlausiblePlaybackBody(data, type, resourceType)) {
                          runGmEngine();
                        } else {
                          deliverFallback(data, pageMeta);
                        }
                      })
                      .catch((err) => {
                        if (!scope.check()) return;
                        UVP.PlaybackTransport.recordPageOutcome(host, false, err);
                        runGmEngine();
                      });
                  }
                  return; // Fallback will call origOnLoadEnd
                }
                try { origOnLoadEnd(); } catch (e) {}
              };
            }
            if (DEBUG) {
              try {
                const req = commonMediaRequest || {};
                const dashReq = req.customData && req.customData.request;
                const act = state.dashActivity;
                const now = Date.now();
                const seekStart = act && act.seekStartTime || 0;
                console.debug('[UVP] XHRLoader wrap load', {
                  method: req.method || 'GET',
                  resourceType: UVP.PlaybackTransport.dashResourceType(dashReq, req.url),
                  url: UVP.PlaybackTransport.redactUrl(req.url),
                  timeout: req.timeout || 0,
                  fillSettled: !!(act && act.fillSettled),
                  seekAgeMs: seekStart ? now - seekStart : -1,
                  afterSeek: !!(act && act.lastLoaderTime && act.lastLoaderTime >= seekStart && seekStart > 0),
                  hasParentLoad: typeof parentLoad === 'function'
                });
              } catch (e) {}
            }
            const result = parentLoad.call(parent, commonMediaRequest, commonMediaResponse);
            if (cd && typeof cd.abort === 'function' && !cd._uvpAbortWrapped) {
              const origCdAbort = cd.abort;
              cd._uvpAbortWrapped = true;
              cd.abort = function () {
                abortFallbackScope(cd._uvpFallbackScope);
                cd._uvpFallbackScope = null;
                return origCdAbort.call(this);
              };
            }
            return result;
          },
          abort: function () {
            if (DEBUG) console.debug('[UVP] XHRLoader wrap abort');
            abortAllFallbacks();
            if (typeof parentAbort === 'function') return parentAbort.apply(parent, arguments);
          },
          getXhr: function () {
            return typeof parentGetXhr === 'function' ? parentGetXhr.call(parent) : null;
          },
          reset: function () {
            if (typeof parentReset === 'function') return parentReset.apply(parent, arguments);
          },
          resetInitialSettings: function () {
            if (typeof parentResetInitialSettings === 'function') {
              return parentResetInitialSettings.apply(parent, arguments);
            }
          }
        };
      }
      UvpDashXHRLoader.__dashjs_factory_name = 'XHRLoader';
      return UvpDashXHRLoader;
    },
  ...
  UVP.Overlay.loadHlsScript = function() {
    ...
    return loadExternalBundle(HLS_BUNDLE_URLS, BAKED_HLS_INTEGRITY, 'Hls', 'hls.js');
  };
  ...
  UVP.Overlay.loadDashScript = function() {
    ...
    return loadExternalBundle(DASH_BUNDLE_URLS, BAKED_DASH_INTEGRITY, 'dashjs', 'dash.js');
  };
  ...
  UVP.Overlay.startDashPlayback = async function(options) {
    ...
    try {
      ...
      // dash.js XHRLoader wrap (parent-delegate): the stock HTTPLoader stays
      // in charge — request traces, dashMetrics.addHttpRequest (bandwidth
      // estimation), retry engine — while this wrap adds the page-fetch →
      // GM_xhr CORS fallback for googlevideo Range GETs. Do NOT extend
      // 'HTTPLoader' itself: replacing it removes all throughput metrics.
      try {
        const CustomXHRLoader = UVP.PlaybackTransport.makeDashXHRLoader(DashLib);
        player.extend('XHRLoader', CustomXHRLoader, true);
        if (DEBUG) console.debug('[UVP] XHRLoader wrap attached (parent-delegate)');
      } catch (e) {
        if (DEBUG) console.warn('[UVP] dash.js XHRLoader extension failed (using stock loader):', e);
      }

      const _playQMax = UVP.Utils.getPlaybackQuality() === 'max';
      const vodSettings = {
        streaming: {
          buffer: {
            // false: replace at the next segment boundary. true splices the
            // new rung into already-buffered media at the playhead — on
            // YouTube SegmentBase that is a decoder flush and a 1–N frame jump.
            fastSwitchEnabled: false,
            // Under max-pin the top rung needs more runway so the playhead
            // never outruns the buffer (primary source of visible "holes").
            bufferTimeDefault: _playQMax ? 30 : 24,
            bufferTimeAtTopQuality: _playQMax ? 48 : 36,
            bufferTimeAtTopQualityLongForm: _playQMax ? 72 : 60,
            longFormContentDurationThreshold: 600,
            bufferToKeep: _playQMax ? 36 : 30,
          ...
          abr: {
            ...
            rules: {
              ...
              // Abandon mid-segment leaves a hole at the playhead; the next
              // rung then starts with a gap → frame jump / stall recovery.
            ...
            // Portal cap off. ABR = throughput only.
            limitBitrateByPortal: false,
            usePixelRatioInLimitBitrateByPortal: false
          },
          retryAttempts: {
          ...
          // enabled:false alone is not enough on dash.js 5.2.x: an MPD
          // ServiceDescription.PlaybackRate still drives 0.5x–1.5x catch-up
          // (slow-mo, then chipmunk, plus AAC reset clicks). NaN min/max is
          // the documented off switch and beats the MPD values. Hard-coded
          // liveDelay:12 also made low-latency streams look "ahead" of a
          // 12s target and slam the rate down — honor the MPD SPD instead.
          delay: {
            liveDelay: NaN,
      ...
      // Live DASH: same idea as HLS applyLiveHlsSyncNoSkip.
      // Delay = MPD SPD (or dash.js default fragment count). Never a fixed
      // liveDelay — a hardcoded 12s made LL streams look "ahead" and catch-up
      // slammed playbackRate. Catch-up stays off: enabled + maxDrift is the
      // DASH seek-to-edge race. Buffer targets are stock 18/30, bounded
      // long-form 30 (stock 60) and back-buffer 20 (stock 20).
      const liveSettings = {
        streaming: {
          buffer: {
            fastSwitchEnabled: false,
            ...
            longFormContentDurationThreshold: 86400,
            bufferToKeep: 20,
            bufferPruningInterval: 10,
            flushBufferAfterPlayback: true
          },
          delay: {
            liveDelay: NaN,
            liveDelayFragmentCount: NaN,
      ...
      const EVT_QUOTA = events.QUOTA_EXCEEDED || 'quotaExceeded';
      const EVT_BUF_CLEARED = events.BUFFER_CLEARED || 'bufferCleared';
      const EVT_Q_REQ = events.QUALITY_CHANGE_REQUESTED || 'qualityChangeRequested';
      const EVT_Q_REN = events.QUALITY_CHANGE_RENDERED || 'qualityChangeRendered';
      ...
      state.enginePlaybackReady = false;
      if (!state.engineWaitStartedAt) state.engineWaitStartedAt = Date.now();
      ...
      const applyCatchupOff = () => {
        // Re-assert after STREAM_INITIALIZED: dash.js applies MPD
        // ServiceDescription (PlaybackRate / Latency) during parse, which
        // can reopen catch-up after the constructor-time updateSettings.
        try {
          player.updateSettings({
            streaming: {
              delay: liveSettings.streaming.delay,
              liveCatchup: liveSettings.streaming.liveCatchup
            }
          });
        } catch (e) {
          if (DEBUG) console.warn('[UVP] DASH catch-up off rejected:', e);
        }
        pinDashRate();
      };
      const applyLiveSettings = () => {
        UVP.Overlay.enterLiveMode('dash-dynamic');
        applyCatchupOff();
      ...
      const onSeeking = () => {
        ...
        UVP.Overlay.debugDashSeekSnap('seeking');
      };
      const onSeeked = () => {
        activity.targetSeekTime = video.currentTime || 0;
        if (UVP.Overlay.getBufferedAhead(video) > 0.5 && video.readyState >= 3) {
          ...
          UVP.Overlay.markDashFillSettled();
          UVP.Overlay.clearDashQuotaLatchIfFilled();
        }
        UVP.Overlay.debugDashSeekSnap('seeked');
      };
      const onWaiting = () => {
        ...
        state.stallCount = 0;
        state.lastVideoTime = video.currentTime || 0;
        UVP.Overlay.debugDashSeekSnap('waiting');
      };
      const onPlaying = () => {
        activity.seeking = false;
        ...
        UVP.Overlay.markDashFillSettled();
        UVP.Overlay.clearDashQuotaLatchIfFilled();
        UVP.Overlay.debugDashSeekSnap('playing');
      };
      const onTimeUpdate = () => {
        if (video.readyState >= 3 && !video.paused) {
          activity.seeking = false;
          activity.waiting = false;
          if (UVP.Overlay.getBufferedAhead(video) > 0.5) {
            UVP.Overlay.markDashFillSettled();
            UVP.Overlay.clearDashQuotaLatchIfFilled();
          }
        }
      ...
      const onRateChange = () => {
        if (!UVP.Overlay.isLivePlayback()) return;
        if (video.playbackRate === 1) return;
        if (DEBUG) console.log('[UVP] DASH live playbackRate clamped from', video.playbackRate);
        pinDashRate();
      };
      ...
      const isStructuralError = (code, msg, type) => {
        ...
        // Download/CORS misses (10/11) and HTML5 MEDIA_ERR_* (3/4) are
        // retried by dash.js + the XHRLoader wrap. Only unrecoverable parse /
        // capability failures should tear the session down on the first event.
        if (code === 23 || code === 31 || code === 32 || code === 34 || code === 35) return true;
        return /MANIFEST_LOADER_PARSING_ERROR|MANIFEST_EMPTY|CAPABILITY_ERROR|MEDIA_SOURCE_NOT_SUPPORTED/i.test(text);
      };
      ...
      player.on(EVT_STREAM_INIT, (evt) => {
        ...
        UVP.Overlay.applyDashQualityMode(player);
        recordProgress('streamInit');
        UVP.Overlay.markEngineReady('dash-stream-init');
      });
      player.on(EVT_BUF_LOADED, () => {
        if (sessionCurrent()) {
          recordProgress('bufferLoaded');
          UVP.Overlay.markEngineReady('dash-buffer-loaded');
        }
      });
      player.on(EVT_FRAG_LOADED, () => {
        if (sessionCurrent()) {
          lastFragmentFetch.at = Date.now();
          recordProgress('fragmentLoaded');
          UVP.Overlay.markEngineReady('dash-fragment-loaded');
        }
      });
      player.on(EVT_FRAG_PROG, () => {
        if (sessionCurrent()) {
          lastFragmentFetch.at = Date.now();
          recordProgress('fragmentProgress');
          UVP.Overlay.markEngineReady('dash-fragment-progress');
        }
      });
      try {
        player.on(EVT_Q_REQ, () => { if (sessionCurrent()) UVP.Overlay.noteAbrSwitch(); });
        player.on(EVT_Q_REN, () => { if (sessionCurrent()) UVP.Overlay.noteAbrSwitch(); });
      } catch (e) {}
      try {
        player.on(EVT_QUOTA, () => {
          if (sessionCurrent()) {
            UVP.Overlay.noteDashQuotaClear('quotaExceeded');
            UVP.Overlay.handleBufferQuota('dash-evt-quota');
          }
        });
      } catch (e) {}
      try {
        player.on(EVT_BUF_CLEARED, (evt) => {
          if (sessionCurrent() && evt && evt.quotaExceeded) {
            UVP.Overlay.noteDashQuotaClear('bufferCleared');
            UVP.Overlay.handleBufferQuota('dash-evt-bufferCleared');
          }
        });
      } catch (e) {}
      ...
      player.on(EVT_ERROR, (evt) => {
        ...
        if (code === 22 || /QuotaExceeded|QUOTA_EXCEEDED|quota exceed/i.test(String(type) + ' ' + String(msg))) {
          UVP.Overlay.noteDashQuotaClear('error');
          UVP.Overlay.handleBufferQuota('dash-quota-error');
          return;
        }
        scheduleRecovery(
      ...
      video.addEventListener('canplay', () => {
        ...
        if (active) {
          ...
          if (active.lastBufferedAhead > UVP.Overlay.dashPlayMinBufferSec() && active.recoveryTimer) {
            clearTimeout(active.recoveryTimer);
            active.recoveryTimer = null;
            active.pendingError = false;
          }
        }
        if (UVP.Overlay.getBufferedAhead(video) >= 0.15) UVP.Overlay.markEngineReady('dash-canplay');
        else UVP.Overlay.applyDeferredSeek(video);
        UVP.Overlay.maybeResumeWhenBuffered(video);
      }, { once: true });

      // HDR pipeline validation: use requestVideoFrameCallback to read the
      // actual colorSpace the browser decoded. metadata.colorSpace is a
      // VideoColorSpace OBJECT ({ primaries, transfer, matrix, fullRange }),
      // not a string — comparing it to 'bt709' is always true. BT.2020
      // primaries or a PQ/HLG transfer confirms the HDR path is live.
      if ('requestVideoFrameCallback' in video) {
        video.requestVideoFrameCallback(function onFrame(now, metadata) {
          if (!sessionCurrent()) return;
          const cs = metadata && metadata.colorSpace;
          if (cs) {
            const isHdrOut = cs.transfer === 'pq' || cs.transfer === 'hlg' || cs.primaries === 'bt2020';
            if (DEBUG) console.log('[UVP] ' + (isHdrOut ? 'HDR pipeline active' : 'SDR output') + ' — colorSpace: ' + cs.primaries + '/' + cs.transfer + '/' + cs.matrix + (cs.fullRange ? ' full' : ' limited'));
          }
        });
      }
  ...
  UVP.Overlay.isLivePlayback = function() {
    if (state.overlayIsLive || ytPlayerData.isLive) return true;
    const u = state.overlayUrl;
    const info = u ? ytFmtMap.get(u) : null;
    return !!(info && info.isLive);
  };

  UVP.Overlay.pickLiveFailoverUrl = function(failedUrl) {
    const isDashUrl = (u) => {
      if (!u) return false;
      if (UVP.Utils.isDashManifest(u)) return true;
      const info = ytFmtMap.get(u);
      return !!(info && info.isDash);
    };
    // DASH just failed. Returning another MPD here re-enters playVideo's
    // DASH branch and failToMuxed. The live caller charges recoveryAttempts
    // before playVideo so a flapping HLS switch cannot run unbounded.
    if (UVP.Extractors.isYouTube()) {
      try {
        const pick = UVP.Extractors.pickYouTubeFormats();
        if (pick && pick.video && pick.video !== failedUrl && !isDashUrl(pick.video)) return pick.video;
      } catch (e) {}
      for (const [u, info] of ytFmtMap) {
        if (u !== failedUrl && info && info.isLive && info.isHls) return u;
      }
    }
    try {
      const fb = UVP.Utils.pickPlaybackFallback(failedUrl);
      if (fb && fb !== failedUrl && !isDashUrl(fb) && (UVP.Utils.isHlsManifest(fb) || UVP.Utils.isDirectMedia(fb))) return fb;
    } catch (e) {}
    return null;
  };

  UVP.Overlay.applyHlsBufferPolicy = function(live) {
    const hls = state.hls;
    if (!hls || !hls.config) return;
    const cfg = hls.config;
    if (live) {
      cfg.maxBufferLength = CONFIG.abrHlsLiveMaxBufferSec;
      cfg.maxMaxBufferLength = CONFIG.abrHlsLiveMaxMaxBufferSec;
      cfg.backBufferLength = CONFIG.abrHlsLiveBackBufferSec;
      cfg.liveMaxBackBufferLength = CONFIG.abrHlsLiveBackBufferSec;
      cfg.maxBufferSize = CONFIG.abrHlsMaxBufferBytesByTier[UVP.Utils.getDeviceMemoryTier()];
      UVP.Overlay.applyLiveHlsSyncNoSkip(cfg);
    } else {
      const _tier = UVP.Utils.getDeviceMemoryTier();
      cfg.maxBufferLength = CONFIG.abrHlsMaxBufferLengthByTier[_tier];
      cfg.maxMaxBufferLength = CONFIG.abrHlsMaxMaxBufferLengthByTier[_tier];
      cfg.backBufferLength = CONFIG.abrHlsBackBufferSecByTier[_tier];
      cfg.liveMaxBackBufferLength = CONFIG.abrHlsLiveBackBufferSec;
    }
    if (DEBUG) console.log('[UVP] HLS buffer policy:', { live, maxBufferLength: cfg.maxBufferLength, maxMaxBufferLength: cfg.maxMaxBufferLength, backBufferLength: cfg.backBufferLength, maxBufferSize: cfg.maxBufferSize, tier: UVP.Utils.getDeviceMemoryTier() });
  };
  // Stock live shape: sync = 3 target-durations (hls.js default
  // liveSyncDurationCount). Do not set liveSyncDuration (seconds) — that
  // overrides HOLD-BACK and fights the buffer if it is shorter than
  // maxBufferLength. Do not set liveMaxLatencyDuration or
  // liveMaxLatencyDurationCount — hls.js API forbids them alongside
  // liveSyncDurationCount, and the default Infinity already prevents
  // edge-seeking (the root cause of the YT live breakage in this build).
  // hls.js API constraint: liveMaxLatencyDuration requires liveSyncDuration
  // to be defined simultaneously, and cannot coexist with the count-based
  // variants (liveSyncDurationCount / liveMaxLatencyDurationCount). Setting
  // all three violates the config validator and breaks the LatencyController
  // — the root cause of broken YouTube live in this build. Keep only
  // liveSyncDurationCount (hls.js's own default) and let the engine manage
  // maxLatency via its default liveMaxLatencyDurationCount=Infinity.
  // Reduces buffer targets on QuotaExceededError with 15s cooldown, restores
  // on seek or 30s. Does NOT trigger BUFFER_FLUSHING — hls.js 1.7.2 handles
  // back-buffer eviction internally (PR #7749); manual flush causes recursion
  // (PR #7776).
  UVP.Overlay.handleBufferQuota = function(reason) {
    if (state.bufferQuotaCooldown && Date.now() - state.bufferQuotaCooldown < 15000) return;
    state.bufferQuotaCooldown = Date.now();
    var tier = UVP.Utils.getDeviceMemoryTier();
    if (DEBUG) {
      var heap = (performance && performance.memory) ? (performance.memory.usedJSHeapSize >> 20) + 'MB' : 'n/a';
      console.warn('[UVP] Buffer quota exceeded:', reason, 'tier:', tier, 'heap:', heap,
        'deviceMemory:', navigator.deviceMemory || 'n/a');
    }
    if (state.hls && state.hls.config) {
      var cfg = state.hls.config;
      var _isLive = state.overlayIsLive;
      var _floorLen = _isLive ? 30 : 10;
      var _floorMax = _isLive ? 60 : 20;
      var _floorBytes = _isLive ? 40 << 20 : 10 << 20;
      cfg.maxBufferLength = Math.max(_floorLen, Math.floor(cfg.maxBufferLength * 0.6));
      cfg.maxMaxBufferLength = Math.max(_floorMax, Math.floor(cfg.maxMaxBufferLength * 0.6));
      cfg.maxBufferSize = Math.max(_floorBytes, Math.floor(cfg.maxBufferSize * 0.6));
      if (DEBUG) console.log('[UVP] handleBufferQuota — HLS config reduced:', { live: _isLive, maxBufferLength: cfg.maxBufferLength, maxMaxBufferLength: cfg.maxMaxBufferLength, maxBufferSize: cfg.maxBufferSize, reason: reason });
    } else if (state.dashPlayer) {
      try {
        var s = state.dashPlayer.getSettings();
        var cur = s.streaming.buffer.bufferTimeDefault || 24;
        state.dashPlayer.updateSettings({
          streaming: { buffer: {
            bufferTimeDefault: Math.max(6, Math.floor(cur * 0.6))
          }}
        });
      } catch (e) {}
    }
    var restore = function() {
      state.bufferQuotaCooldown = 0;
      if (state.video) {
        try { state.video.removeEventListener('seeked', restore); } catch (e) {}
      }
      if (state.hls) UVP.Overlay.applyHlsBufferPolicy(state.overlayIsLive);
      if (state.dashPlayer) {
        try {
          var t = UVP.Utils.getDeviceMemoryTier();
          state.dashPlayer.updateSettings({ streaming: { buffer: {
            bufferTimeDefault: state.overlayIsLive
              ? CONFIG.abrDashLiveBufferTimeDefaultByTier[t]
              : CONFIG.abrDashBufferTimeDefaultByTier[t]
          }}});
        } catch (e) {}
      }
    };
    if (state.video) state.video.addEventListener('seeked', restore, { once: true });
    setTimeout(restore, state.overlayIsLive ? 10000 : 30000);
  };

  UVP.Overlay.applyLiveHlsSyncNoSkip = function(cfg) {
    if (!cfg) return;
    cfg.liveSyncDurationCount = 3;
  };
  // Chaturbate: short CMAF window. Sync still 3 segments (~3–6s). Forward
  // buffer 8s / cap 12s stays above that so ABR has runway without a skip.
  UVP.Overlay.applyChaturbateLiveHls = function() {
    if (!UVP.Extractors.isChaturbate() || !state.hls || !state.hls.config) return;
    const cfg = state.hls.config;
    cfg.maxBufferLength = 8;
    cfg.maxMaxBufferLength = 12;
    cfg.backBufferLength = 8;
    cfg.liveMaxBackBufferLength = 8;
    UVP.Overlay.applyLiveHlsSyncNoSkip(cfg);
  };
  UVP.Overlay.enterLiveMode = function(reason) {
    state.overlayIsLive = true;
    state.liveDetectReason = reason || 'live';
    UVP.Overlay.applyHlsBufferPolicy(true);
    UVP.Overlay.applyChaturbateLiveHls();
    state.bufferHold = false;
    if (UVP.Extractors.isYouTube()) ytPlayerData.isLive = true;
    state.deferredSeekTo = null;
    if (state.pendingSeekHandler && state.video) {
      try { state.video.removeEventListener('loadedmetadata', state.pendingSeekHandler); } catch (e) {}
      state.pendingSeekHandler = null;
    }
    if (state.video) {
      try { state.video.loop = false; state.video.removeAttribute('loop'); } catch (e) {}
    }
    ...
    // Flags + live HLS buffer cap. Warm = HLS FRAG_BUFFERED or dashActivity.engineReady.
    if (DEBUG) console.log('[UVP] enterLiveMode:', reason || 'live');
  };
  // HLS LEVEL_LOADED can set live on a VOD playlist that omitted ENDLIST
  // on a truncated first parse. PLAYLIST-TYPE:VOD means the list is finite.
  UVP.Overlay.exitHlsVodMisdetect = function() {
    if (state.liveDetectReason !== 'hls-level-loaded') return;
    state.overlayIsLive = false;
    state.liveDetectReason = null;
    const saveBtn = state.overlay && state.overlay.shadowRoot
      ? state.overlay.shadowRoot.querySelector('.uvp-save')
      : null;
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.textContent = state.downloadText || 'Save';
    }
    UVP.Overlay.applyHlsBufferPolicy(false);
    if (DEBUG) console.log('[UVP] exitHlsVodMisdetect: PLAYLIST-TYPE VOD');
  };

  UVP.Overlay.isEngineWarm = function() {
    if (state.dashPlayer && state.dashActivity) return !!state.dashActivity.engineReady;
    if (state.hls) return !!state.enginePlaybackReady;
    return !!(state.video && state.video.readyState >= 1);
  };
  UVP.Overlay.attachWaitCapMs = function() {
    if (UVP.Overlay.isLivePlayback()) return CONFIG.liveEngineWaitMs || 12000;
    return CONFIG.hlsStartupBufferGraceMs || 20000;
  };
  UVP.Overlay.isFirstSegmentFlowing = function(video) {
    return !!(state.hls && video && !video.buffered.length && lastFragmentFetch.at > 0 &&
      Date.now() - lastFragmentFetch.at < (CONFIG.hlsStartupBufferGraceMs || 20000));
  };
  // Cold attach with recent loader activity is not a dead engine.
  // HLS Max first fragments can exceed attachWaitCapMs. Event stamps expire
  // at 20s; networkState=2 (LOADING) after a real engine request may wait
  // attachLoadingMaxMs because playlist/extensionless URLs never match .ts/.m4s.
  UVP.Overlay.isAttachActivityFlowing = function(video) {
    video = video || state.video;
    if (!video || video.error) return false;
    if (!(state.hls || state.dashPlayer)) return false;
    const slack = CONFIG.hlsStartupBufferGraceMs || 20000;
    const loadingMax = CONFIG.attachLoadingMaxMs || 120000;
    const now = Date.now();
    if (lastFragmentFetch.at > 0 && now - lastFragmentFetch.at < slack) return true;
    const act = state.dashActivity;
    if (act && act.lastProgressTime && now - act.lastProgressTime < slack) return true;
    if (lastFragmentFetch.at > 0 && video.networkState === 2 && now - lastFragmentFetch.at < loadingMax) return true;
    return false;
  };

  UVP.Overlay.dashPlayMinBufferSec = function() {
    const n = Number(CONFIG.dashPlayMinBufferSec);
    return (Number.isFinite(n) && n > 0) ? n : 1.5;
  };

  // t=0 init buffer is not the resume playhead. Do not play() until the
  // current playhead itself has dashPlayMinBufferSec buffered.
  UVP.Overlay.maybeResumeWhenBuffered = function(video) {
    video = video || state.video;
    if (!video || video.error) return false;
    if (state.wasPaused) {
      state.bufferHold = false;
    ...
    if (state.deferredSeekTo != null) {
      UVP.Overlay.applyDeferredSeek(video);
      if (state.deferredSeekTo != null) return false;
    }
    if (UVP.Overlay.isLivePlayback()) {
      state.bufferHold = false;
    ...
    if (!video.paused) {
      state.bufferHold = false;
      if (UVP.Overlay.getBufferedAhead(video) > 0.5) UVP.Overlay.markDashFillSettled();
      return true;
    }
    const minBuf = state.dashPlayer ? UVP.Overlay.dashPlayMinBufferSec() : 0.15;
    if (UVP.Overlay.getBufferedAhead(video) < minBuf) {
      state.bufferHold = true;
      return false;
    }
    state.bufferHold = false;
    UVP.Overlay.resumeOrPauseVideo(video);
  ...
  UVP.Overlay.isBufferHoldProgressing = function(video) {
    if (!state.bufferHold || !video || video.error) return false;
    const slack = CONFIG.dashProgressExtendMs || 8000;
    const now = Date.now();
    if (lastFragmentFetch.at > 0 && now - lastFragmentFetch.at < slack) return true;
    const act = state.dashActivity;
    if (act && act.lastProgressTime && now - act.lastProgressTime < slack) return true;
    ...
    const minBuf = state.dashPlayer ? UVP.Overlay.dashPlayMinBufferSec() : 0.15;
    if (UVP.Overlay.getBufferedAhead(video) >= minBuf) return true;
    return false;
  };

  UVP.Overlay.applyDeferredSeek = function(video) {
    video = video || state.video;
    const seekTo = state.deferredSeekTo;
    if (!video || seekTo == null || !isFinite(seekTo) || seekTo <= 0) return false;
    if (video.error) { state.deferredSeekTo = null; return false; }
    if (UVP.Overlay.isLivePlayback()) { state.deferredSeekTo = null; return false; }
    if (video.readyState < 1) return false;
    const hasStart = UVP.Overlay.getBufferedAhead(video, 0) >= 0.15;
    const hasPlayhead = UVP.Overlay.getBufferedAhead(video) >= 0.15;
    if (!hasStart && !hasPlayhead) return false;
    state.deferredSeekTo = null;
    if (state.dashActivity) {
      state.dashActivity.seeking = true;
      state.dashActivity.seekStartTime = Date.now();
      state.dashActivity.targetSeekTime = seekTo;
    }
    state.suppressPlaybackStateEvents = true;
    try { if (!video.paused) video.pause(); } catch (e) {}
    state.suppressPlaybackStateEvents = false;
    try { video.currentTime = seekTo; } catch (e) { if (DEBUG) console.warn('[UVP] deferred seek failed:', e); }
    if (DEBUG) console.log('[UVP] deferred seek applied:', seekTo);
    return true;
  };

  UVP.Overlay.markEngineReady = function(tag) {
    if (state.dashActivity) state.dashActivity.engineReady = true;
    state.enginePlaybackReady = true;
    state.engineWaitStartedAt = 0;
    if (DEBUG) console.log('[UVP] engine ready:', tag || '');
    UVP.Overlay.applyDeferredSeek(state.video);
    UVP.Overlay.maybeResumeWhenBuffered(state.video);
  };

  // First playhead media only. Not STREAM_INIT / wrap MPD / engineReady.
  UVP.Overlay.noteDashQuotaClear = function(src) {
    const act = state.dashActivity;
    if (!act) return;
    act.quotaClearAt = Date.now();
    if (DEBUG) UVP.Overlay.debugDashSeekSnap('quota-clear:' + (src || ''));
  };

  UVP.Overlay.clearDashQuotaLatchIfFilled = function() {
    const act = state.dashActivity;
    const video = state.video;
    if (!act || !act.quotaClearAt) return;
    if (video && UVP.Overlay.getBufferedAhead(video) > 0.5 && video.readyState >= 3) {
      act.quotaClearAt = 0;
      if (DEBUG) UVP.Overlay.debugDashSeekSnap('quota-clear-resolved');
    }
  };

  UVP.Overlay.isDashQuotaClearing = function(now) {
    const act = state.dashActivity;
    if (!act || !state.dashPlayer || !act.quotaClearAt) return false;
    const t = now || Date.now();
    if (t - act.quotaClearAt >= (CONFIG.dashQuotaClearGraceMs || 15000)) return false;
    const video = state.video;
    if (video && UVP.Overlay.getBufferedAhead(video) > 0.5 && video.readyState >= 3) return false;
    return true;
  };

  UVP.Overlay.markDashFillSettled = function() {
    const act = state.dashActivity;
    if (act && !act.fillSettled) {
      act.fillSettled = true;
      UVP.Overlay.debugDashSeekSnap('fillSettled');
    }
  };

  UVP.Overlay.debugDashSeekSnap = function(reason) {
    if (!DEBUG) return;
    try {
      const video = state.video;
      const act = state.dashActivity;
      const now = Date.now();
      const seekStart = (act && act.seekStartTime) || 0;
      const evidenceAt = UVP.Overlay.dashFillEvidenceAt(act);
      console.debug('[UVP] dash-seek', reason, {
        fillSettled: !!(act && act.fillSettled),
        actSeeking: !!(act && act.seeking),
        actWaiting: !!(act && act.waiting),
        videoSeeking: !!(video && video.seeking),
        paused: !!(video && video.paused),
        readyState: video ? video.readyState : -1,
        networkState: video ? video.networkState : -1,
        currentTime: video ? Number(video.currentTime) || 0 : -1,
        ahead: video ? UVP.Overlay.getBufferedAhead(video) : -1,
        bufferHold: !!state.bufferHold,
        wasPaused: !!state.wasPaused,
        stallCount: state.stallCount || 0,
        seekAgeMs: seekStart ? now - seekStart : -1,
        loaderAgeMs: (act && act.lastLoaderTime) ? now - act.lastLoaderTime : -1,
        progressAgeMs: (act && act.lastProgressTime) ? now - act.lastProgressTime : -1,
        progressTag: (act && act.lastProgressTag) || '',
        evidenceAfterSeek: !!(act && seekStart > 0 && evidenceAt >= seekStart && evidenceAt > 0),
        quotaClearAgeMs: (act && act.quotaClearAt) ? now - act.quotaClearAt : -1,
        inGrace: UVP.Overlay.isDashInGrace(now),
        engineReady: !!(act && act.engineReady),
        targetSeekTime: act ? act.targetSeekTime : null
      });
    } catch (e) {}
  };

  // Max of DASH loader / progress / fragment stamps. Callers must compare against seekStartTime.
  UVP.Overlay.dashFillEvidenceAt = function(act) {
    act = act || state.dashActivity;
    const loader = (act && act.lastLoaderTime) ? act.lastLoaderTime : 0;
    const progress = (act && act.lastProgressTime) ? act.lastProgressTime : 0;
    const frag = lastFragmentFetch.at || 0;
    return Math.max(loader, progress, frag);
  };

  // !fillSettled: 5.0.36 attach grace. fillSettled: evidence + request budget.
  UVP.Overlay.isDashInGrace = function(now) {
    const act = state.dashActivity;
    if (!act || !state.dashPlayer) return false;
    const t = now || Date.now();
    if (UVP.Overlay.isDashQuotaClearing(t)) return true;
    if (!act.fillSettled) {
    ...
    const video = state.video;
    const intent = !!(act.seeking || act.waiting || (video && video.seeking));
    if (!intent) return false;
    const seekStart = act.seekStartTime || 0;
    const maxGrace = CONFIG.dashSeekMaxGraceMs || 45000;
    if (seekStart > 0 && (t - seekStart) >= maxGrace) return false;
    const evidenceAt = UVP.Overlay.dashFillEvidenceAt(act);
    const extend = CONFIG.dashProgressExtendMs || 8000;
    if (evidenceAt >= seekStart && evidenceAt > 0 && (t - evidenceAt) < extend) return true;
    if (video && UVP.Overlay.getBufferedAhead(video) > 0.5 && video.readyState >= 3) return false;
    const budget = CONFIG.dashSeekRequestBudgetMs || 8000;
    if (seekStart > 0 && (t - seekStart) < budget) return true;
    return false;
  };

  // Nested-feed scroll passthrough. The overlay is position:fixed, so a
  // vertical finger-drag that would scroll the document on most sites does
  // nothing on RedGifs (body overflow is hidden; the real scroller is
  // .previewFeed / #scrollableDiv). Forward wheel + vertical touch drags
  // onto that scroller so the feed still moves under the overlay. Other
  // sites keep native document scrolling and are not intercepted.
  let _pageScrollRootCache = { el: null, at: 0, had: false };
  UVP.Extractors.getPageScrollRoot = function() {
    if (!UVP.Extractors.isRedgifs()) return null;
    const now = performance.now();
    if (_pageScrollRootCache.had && now - _pageScrollRootCache.at < 500) return _pageScrollRootCache.el;
    const el = UVP.Extractors._findPageScrollRoot();
    _pageScrollRootCache = { el: el, at: now, had: true };
    return el;
  };
  UVP.Extractors._isVerticallyScrollable = function(el) {
    if (!el || el.nodeType !== 1) return false;
    try {
      if (el.scrollHeight <= el.clientHeight + 4) return false;
      const oy = getComputedStyle(el).overflowY;
      return oy === 'auto' || oy === 'scroll' || oy === 'overlay' || oy === 'hidden' || oy === 'clip';
    } catch (e) { return false; }
  };
  UVP.Extractors._findPageScrollRoot = function() {
    const named = [
      document.querySelector('.previewFeed'),
      document.querySelector('#scrollableDiv'),
      document.querySelector('.watchPage'),
      document.querySelector('[class*="previewFeed"]')
    ];
    for (let i = 0; i < named.length; i++) {
      if (UVP.Extractors._isVerticallyScrollable(named[i])) return named[i];
    }
    let n = state.targetVideo;
    if (!n) {
      const active = document.querySelector('.GifPreview_isActive, .GifPreview.GifPreview_isActive');
      n = active || null;
    }
    while (n && n !== document.body && n !== document.documentElement) {
      if (UVP.Extractors._isVerticallyScrollable(n)) return n;
      n = n.parentElement;
    }
    return named[0] || named[1] || null;
  };

  UVP.Overlay.scrollPageBy = function(dx, dy) {
    if (document.fullscreenElement) return false;
    const root = UVP.Extractors.getPageScrollRoot();
    if (!root) return false;
    const prevTop = root.scrollTop;
    const prevLeft = root.scrollLeft;
    if (dy) root.scrollTop += dy;
    if (dx) root.scrollLeft += dx;
    return root.scrollTop !== prevTop || root.scrollLeft !== prevLeft;
  };

  UVP.Overlay._rgStepFeed = function(direction) {
    if (!direction || document.fullscreenElement) return false;
    const active = document.querySelector('.GifPreview.GifPreview_isActive, .GifPreview_isActive');
    let target = null;
    if (active) {
      target = direction > 0 ? active.nextElementSibling : active.previousElementSibling;
      while (target && !(target.classList && (target.classList.contains('GifPreview') || target.hasAttribute('data-feed-item-id')))) {
        target = direction > 0 ? target.nextElementSibling : target.previousElementSibling;
      }
    } else {
      const items = document.querySelectorAll('.GifPreview, [data-feed-item-id]');
      ...
      const idx = best + (direction > 0 ? 1 : -1);
      if (idx >= 0 && idx < items.length) target = items[idx];
    }
  ...
  UVP.Overlay.bindScrollPassthrough = function(overlay) {
    if (!overlay || overlay._uvpScrollPass) return;
    const sr = overlay.shadowRoot;
    if (!sr) return;
    overlay._uvpScrollPass = true;

    // Cover the video (not native controls / UVP chrome) so the gesture
    // cannot be swallowed by the <video controls> widget. Taps still
    // toggle playback; vertical drags scroll the RedGifs feed.
    if (UVP.Extractors.isRedgifs()) {
      const hit = document.createElement('div');
      hit.className = 'uvp-scroll-hit';
      hit.setAttribute('aria-hidden', 'true');
      hit.style.cssText = 'position:absolute;left:0;right:0;top:0;bottom:72px;z-index:2;touch-action:pan-y;background:transparent;';
      sr.appendChild(hit);
    ...
    const CHROME = '.uvp-close, .uvp-save, .uvp-pip, .uvp-progress-track';
    const CONTROL_PX = 72;
    const SLOP = 12;
    const STEP_PX = 80;
    let startX = 0, startY = 0, lastY = 0, lastX = 0;
    let tracking = false, scrolling = false, locked = false, acc = 0;

    const fromChrome = (e) => {
      const t = e.target;
      return !!(t && t.closest && t.closest(CHROME));
    };
    const inControls = (clientY) => {
      const r = overlay.getBoundingClientRect();
      return clientY > r.bottom - CONTROL_PX;
    };
    const onWheel = (e) => {
      if (!UVP.Extractors.isRedgifs() || document.fullscreenElement) return;
      if (fromChrome(e)) return;
      if (UVP.Overlay.scrollPageBy(e.deltaX, e.deltaY)) {
        e.preventDefault();
        e.stopPropagation();
      } else if (Math.abs(e.deltaY) >= 40 && UVP.Overlay._rgStepFeed(e.deltaY > 0 ? 1 : -1)) {
        e.preventDefault();
        e.stopPropagation();
      }
    };
    const onStart = (e) => {
      ...
      if (!t) return;
      if (inControls(t.clientY)) return;
      tracking = true; scrolling = false; locked = false; acc = 0;
    ...
    const onMove = (e) => {
      ...
      const delta = -dy;
      acc += delta;
      let moved = UVP.Overlay.scrollPageBy(0, delta);
      if (!moved && Math.abs(acc) >= STEP_PX) {
        moved = UVP.Overlay._rgStepFeed(acc > 0 ? 1 : -1);
        acc = 0;
      }
      if (moved || scrolling) {
        e.preventDefault();
    ...
    const onEnd = (e) => {
      const hitTap = tracking && !locked && e.target && e.target.classList && e.target.classList.contains('uvp-scroll-hit');
      if (hitTap && state.video) {
        try { if (state.video.paused) state.video.play(); else state.video.pause(); } catch (err) {}
      }
      tracking = false; scrolling = false; locked = false; acc = 0;
    };

    sr.addEventListener('wheel', onWheel, { passive: false, capture: true });
    sr.addEventListener('touchstart', onStart, { passive: true, capture: true });
    sr.addEventListener('touchmove', onMove, { passive: false, capture: true });
    sr.addEventListener('touchend', onEnd, { passive: true, capture: true });
    sr.addEventListener('touchcancel', onEnd, { passive: true, capture: true });
  };
  ...
  // PiP on this <video>. hardResetVideoEl replaces the element — re-bind there too.
  //
  // ENTER: hide the overlay box; the same <video> keeps playing in the system PiP window.
  //
  // LEAVE — the browser does not say why. Paths:
  //   stale (no overlay / this <video> is no longer state.video) → stop
  //   Path A  pipExitByToggle (our button) → restoreOrClose now
  //   Path B  otherwise wait pipExitDecisionMs
  //     overlay gone / video replaced / entered PiP again → stop
  //     video.paused → PiP-window X → closeOverlay
  //     video playing → restoreOrClose
  //       back-to-tab and other-tab steal of the one PiP slot both look like this
  //       (Chrome does not pause on steal)
  //
  // restoreOrClose (no mini-player — PiP was the floater):
  //   site target on-screen in THIS tab → show overlay on it (two audios if another tab now holds PiP)
  //   target off-screen or missing → closeOverlay
  UVP.Overlay.bindPipEvents = function(vid) {
    if (!vid) return;
    ...
    vid.addEventListener('leavepictureinpicture', () => {
      ...
      const exitedByToggle = state.pipExitByToggle;
      state.pipExitByToggle = false;
      state.isDetachedPip = false;
      if (!state.overlay || state.video !== vid) return;
      const restoreOrClose = () => {
        if (!state.overlay || state.video !== vid) return;
        // Same target chain as startOverlaySync.
        const inDom = state.targetVideo && document.body.contains(state.targetVideo);
        let target = state.targetVideo;
        let rect = inDom ? target.getBoundingClientRect() : null;
        // YouTube: sync to stable player container (same as startOverlaySync).
        if (inDom && UVP.Extractors.isYouTube()) {
          const container = target.closest('#movie_player, .html5-video-player, ytd-player');
          if (container) rect = container.getBoundingClientRect();
        }
        const vw = window.innerWidth, vh = window.innerHeight;
        // Re-resolve ONLY if the original target is no longer in DOM.
        if (!inDom) {
          if (state._activePlaybackSource && document.body.contains(state._activePlaybackSource)) {
            target = state._activePlaybackSource;
            rect = target.getBoundingClientRect();
          } else if (state.overlayUrl || state._activePoster) {
            const vids = UVP.Utils.findAllVideos ? UVP.Utils.findAllVideos(document) : Array.from(document.querySelectorAll('video'));
            const sameVid = vids.find(v => v !== state.video && document.body.contains(v) && (
              (state.overlayUrl && (v.src === state.overlayUrl || v.currentSrc === state.overlayUrl)) ||
              (state._activePoster && v.poster && v.poster === state._activePoster)
            ));
            if (sameVid) { target = sameVid; rect = sameVid.getBoundingClientRect(); }
            else { target = null; }
          } else { target = null; }
        }
        // Recycled-element guard (same as startOverlaySync).
        if (target && rect && !UVP.Overlay.isActivePlaybackElement(target)) { rect = null; }
        // RedGifs RG-id guard (same as startOverlaySync).
        if (target && UVP.Extractors.isRedgifs() && state._activeRgId) {
          const curRgId = UVP.Extractors.getRgIdFromElement(target);
          if (curRgId && curRgId !== state._activeRgId) { rect = null; }
        }
        // Visibility check — same isOutOfView formula as startOverlaySync.
        const isNowInDom = target && document.body.contains(target);
        const isOutOfView = !isNowInDom || !rect || rect.bottom <= 40 || rect.top >= vh - 40 || rect.right <= 40 || rect.left >= vw - 40 || rect.width < 20 || rect.height < 20;
        // On-screen → put the overlay box back. Off-screen/missing → close (not mini).
        if (target && !isOutOfView && state.overlay) {
          state.targetVideo = target;
          state.isMiniPlayer = false;
      ...
      if (exitedByToggle) { restoreOrClose(); return; } // Path A
      // Path B: X pauses after this event; playing means back-to-tab or other-tab steal.
      setTimeout(() => {
        if (!state.overlay || state.video !== vid || state.isDetachedPip || document.pictureInPictureElement) return;
        if (vid.paused) { UVP.Overlay.closeOverlay(false); return; }
        restoreOrClose();
      }, CONFIG.pipExitDecisionMs);
    });
  };
  ...
  UVP.Overlay.hardResetVideoEl = function() {
    ...
    // Release the old element before detach: a still-playing detached <video>
    // keeps decoding/downloading invisibly, and browsers pin playing media
    // against GC (mirrors closeOverlay's pause + src-clear teardown).
    try { oldVideo.pause(); oldVideo.removeAttribute('src'); oldVideo.load(); } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
    if (oldVideo.parentNode) oldVideo.parentNode.replaceChild(newVideo, oldVideo);
    state.video = newVideo;
    state.enginePlaybackReady = false;
    state.engineWaitStartedAt = Date.now();
    state.engineEscalated = false;
    state.bufferHold = false;
    state.lastVideoTime = 0;
    state.stallCount = 0;
    UVP.Overlay.bindVideoEvents(newVideo);
    UVP.Overlay.bindPlaybackIntent(newVideo, resetGeneration);
    UVP.Overlay.bindPipEvents(newVideo);
    // The openOverlay 'playing' listener that clears suppressPlaybackStateEvents
    // died with the old element — re-arm it for this generation.
    newVideo.addEventListener('playing', () => {
      if (state.overlayGeneration === resetGeneration) state.suppressPlaybackStateEvents = false;
    }, { once: true });
  ...
  UVP.Overlay.bindPlaybackIntent = function(video, generation) {
    state.videoPlayHandler = () => { if (!state.suppressPlaybackStateEvents && state.video === video && state.overlayGeneration === generation) { state.wasPaused = false; state.bufferHold = false; } };
    state.videoPauseHandler = () => { if (!state.suppressPlaybackStateEvents && state.video === video && state.overlayGeneration === generation) { state.wasPaused = true; state.bufferHold = false; } };
    // Abort in-flight prefetch on seek (out-of-window requests)
    state.videoSeekHandler = () => { if (CONFIG.enablePlaybackPrefetch) UVP.SegmentCache.abortInFlight(); };
    video.addEventListener('play', state.videoPlayHandler);
    video.addEventListener('pause', state.videoPauseHandler);
    video.addEventListener('seeking', state.videoSeekHandler);
  };
  ...
  UVP.Overlay.playVideo = function(url, video, seekTo) {
    ...
    if (UVP.Extractors.isYouTube()) {
      ...
      console.log('[UVP] YT MSE play', {
        live: !!(ytPlayerData.isLive || (_ytInfo && _ytInfo.isLive)),
        hls: !!_isHls,
        dash: !!_isDash,
        host,
        pathN: /\/n\/[A-Za-z0-9_-]+\//.test(url),
        queryN: /[?&]n=/.test(url),
        rewritten: ytNsigRewrites.has(url),
        deciphered: ytDecipheredUrls.has(url),
        hasN: _ytInfo ? _ytInfo.hasN : null,
        client: _ytInfo ? (_ytInfo.client || '') : ''
      });
    }
    // Room streams are live. Tag before the engine so Save/loop/seek-restore
    // stay off even if LEVEL_LOADED never fires (native HLS fallback).
    if (UVP.Extractors.isChaturbate() && _isHls) UVP.Overlay.enterLiveMode('chaturbate');
    const liveSeekBlocked = !!((_ytInfo && _ytInfo.isLive) || state.overlayIsLive || ytPlayerData.isLive);
    if (liveSeekBlocked) {
      state.overlayIsLive = true;
      state.deferredSeekTo = null;
      if (state.video) {
    ...
    const wantSeek = !!(seekTo && isFinite(seekTo) && seekTo > 0 && !liveSeekBlocked);
    state.deferredSeekTo = wantSeek ? seekTo : null;
    state.bufferHold = false;
    state.enginePlaybackReady = false;
    state.engineWaitStartedAt = Date.now();
    state.engineEscalated = false;
    lastFragmentFetch.at = 0;
    state.stallCount = 0;
    state.lastVideoTime = (video && video.currentTime) || 0;
    // Native progressive can seek on loadedmetadata. MSE engines must NOT:
    // DASH SegmentBase has no timeline until the sidx exists, and an early
    // seek on a 1h file is what burned the stall/recovery budget. HLS YouTube
    // VOD uses startPosition (loads the target fragment directly); generic
    // HLS applies deferredSeekTo after FRAG_BUFFERED.
    const youtubeDash = UVP.Extractors.isYouTube() && !state.overlayIsLive && !ytPlayerData.isLive && (
      ytFmtMap.size > 0 || (_ytInfo && _ytInfo.initRange && _ytInfo.indexRange)
    );
    const mseEngine = (_isHls && UVP.Overlay.shouldUseHlsJs(video)) || _isDash || youtubeDash;
    if (wantSeek && !mseEngine) {
    ...
    if (_isHls && UVP.Overlay.shouldUseHlsJs(video)) {
      UVP.Overlay.loadHlsScript().then(HlsLib => {
        ...
        // Worker ON unconditionally: hls.js 1.7.2 self-heals on TT-enforced
        // sites (worker constructor throws -> inline transmuxer fallback, at
        // creation AND on runtime worker errors), so keying it off the mere
        // presence of the trustedTypes API (every Chrome/Edge since 83) only
        // threw away off-main-thread transmuxing on the entire non-enforcing web.
        const _dp = UVP.Utils.getDeviceProfile();
        const hlsOpts = {
          maxBufferLength: 60, maxMaxBufferLength: 120, enableWorker: _dp.cpuClass !== 'low',
          // lowLatencyMode is deliberately NOT set: hls.js's default config and
          // its own LL-HLS playlist detection govern low-latency/live handling
          // (partial loading, latency catchup). Forcing it overrides the
          // engine's live logic instead of working with it.
          // Engine-owned ABR: configure hls.js, never pin. startLevel -1 lets
          // hls.js pick the initial level from its bandwidth estimate; EWMA
          // factors stay stock.
          startLevel: -1,
          abrEwmaFastVoD: 3.0,
          abrEwmaSlowVoD: 9.0,
          ...
          // Tiny MSE holes at a level switch look like a freeze/jump. Stock
          // 0.1s is tight for YouTube/CDN boundary drift. 0.8 covers larger
          // SegmentBase / discontinuity gaps without letting real under-runs
          // hide for too long (nudgeMaxRetry still limits stall recovery).
          maxBufferHole: 0.8,
          nudgeMaxRetry: 8,
          // Prefer contiguous appends; avoid splicing a new rung into the
          // playhead (creates 1–N frame holes on YouTube SegmentBase).
          // 'max': climb faster / larger starve budget. 'auto': stock hls.js ABR.
          ...(UVP.Utils.getPlaybackQuality() === 'max' ? {
            // Native decode-overload guard (one-way level cap on
            // sustained FPS drops until the next stream load).
            capLevelOnFPSDrop: true,
            abrBandWidthFactor: 0.95,
            abrBandWidthUpFactor: CONFIG.abrHlsUpSwitchFactor,
            abrEwmaDefaultEstimate: CONFIG.abrHlsStartupEstimateBps,
            // Larger starve budget under max-pin: top rung fills slower; a
            // short starvation delay used to trigger demotion → mid-ladder
            // hole → recovery loop.
            maxStarvationDelay: Math.max(CONFIG.abrHlsMaxStarvationDelaySec, 12),
            maxLoadingDelay: Math.max(CONFIG.abrHlsMaxLoadingDelaySec, 6)
          } : {}),
          // Bounded memory: stock backBufferLength=Infinity grows the back
          // buffer unbounded on long VOD; stock maxBufferSize=60MB truncates
          // the 60s forward target at 4K to ~12-15s of runway.
          // liveMaxBackBufferLength is set ONLY in the live override block
          // below (liveSeekBlocked || state.overlayIsLive). Setting it in the
          // base config is a no-op for VOD but risks confusing the engine's
          // buffer management on the live→VOD transition path.
          backBufferLength: CONFIG.abrHlsBackBufferSec,
          maxBufferSize: CONFIG.abrHlsMaxBufferBytes,
          ...
          // liveDurationInfinity: hls.js sets MediaSource.duration to Infinity
          // for live broadcasts when this is true, enabling native Live UI
          // controls (Chrome shows "LIVE" instead of 0/growing-number).
          // No-op for VOD — hls.js only applies it when it detects live
          // internally (#EXT-X-ENDLIST absent).
          liveDurationInfinity: true,
        };
        // startPosition: only set for known VOD (YouTube non-live, where
        // _ytInfo.isLive === false). For generic HLS, let hls.js use its
        // default (-1): live streams start at the live edge, VOD at 0.
        // The LEVEL_LOADED listener below detects live and applies live UI
        // (overlayIsLive, disabled Save). The loadedmetadata seek handler
        // (above) handles VOD seekTo restore.
        if (!liveSeekBlocked && !state.overlayIsLive && _ytInfo && !_ytInfo.isLive) {
          hlsOpts.startPosition = initialPos;
          // Engine will start at this position; do not also deferred-seek.
          state.deferredSeekTo = null;
        }
        ...
        // Token-propagating loader: only activated when the master playlist URL
        // carries CDN security tokens (query params) that must be merged into
        // child URLs (variant playlists, segments, AES keys on the same origin).
        // The custom UvpEngineLoader intercepts ONLY when token propagation is
        // needed. It tries native XhrLoader FIRST (correct Referer, proper
        // stats/abort/timeout), falling back to the custom transport
        // (page-transport fetch → GM_xhr CORS bypass) on native failure.
        // Always attach the PlaybackTransport loader. rewriteUrl no-ops
        // without masterQuery, so query-less masters and path-token CDNs
        // get the same native-XHR → page-fetch → GM_xhr fallback chain.
        if (HlsLib.DefaultConfig && HlsLib.DefaultConfig.loader) {
          const tokenCtx = { masterUrl: url, masterQuery, masterOrigin };
          hlsOpts.pLoader = UVP.PlaybackTransport.makeManifestLoader(HlsLib, tokenCtx);
          hlsOpts.fLoader = UVP.PlaybackTransport.makeFragmentLoader(HlsLib, tokenCtx);
          hlsOpts.kLoader = UVP.PlaybackTransport.makeKeyLoader(HlsLib, tokenCtx);
        }
        if (liveSeekBlocked || state.overlayIsLive) {
          hlsOpts.maxBufferLength = CONFIG.abrHlsLiveMaxBufferSec;
          hlsOpts.maxMaxBufferLength = CONFIG.abrHlsLiveMaxMaxBufferSec;
          hlsOpts.backBufferLength = CONFIG.abrHlsLiveBackBufferSec;
          hlsOpts.liveMaxBackBufferLength = CONFIG.abrHlsLiveBackBufferSec;
          UVP.Overlay.applyLiveHlsSyncNoSkip(hlsOpts);
        }
        if (UVP.Extractors.isChaturbate()) {
          hlsOpts.maxBufferLength = 8;
          hlsOpts.maxMaxBufferLength = 12;
          hlsOpts.backBufferLength = 8;
          hlsOpts.liveMaxBackBufferLength = 8;
          UVP.Overlay.applyLiveHlsSyncNoSkip(hlsOpts);
        }
        if (CONFIG.enablePlaybackPrefetch) {
          const _pcMax = CONFIG.abrHlsMaxBufferBytesByTier[_dp.memClass];
          UVP.MultiTab.setCacheMaxBytes(_pcMax);
          UVP.MultiTab.activate();
          UVP.SegmentCache.init(_pcMax);
          if (DEBUG) console.log('[UVP] SegmentCache initialized, maxBytes:', _pcMax, '(memClass:', _dp.memClass, 'netClass:', _dp.netClass, ')');
        }
        const hls = new HlsLib(hlsOpts);
        state.hls = hls;
        if (liveSeekBlocked || state.overlayIsLive) UVP.Overlay.applyHlsBufferPolicy(true);
        UVP.Overlay.applyChaturbateLiveHls();
        lastFragmentFetch.at = Date.now();
        state.engineWaitStartedAt = Date.now();
        // Bounded fatal error recovery, per the official hls.js guidance:
        // unbounded instant startLoad()/recoverMediaError() calls can loop
        // forever. The budget counts fatal recoveries that produced NO buffered
        // fragment since the last one (a FRAG_BUFFERED listener resets it —
        // a recovery that yields playback progress proves itself). Escalate
        // to the overlay hard-recovery path (which re-extracts a fresh URL
        // and rebuilds the player) after CONFIG.hlsMaxRecoveries fruitless
        // attempts, regardless of how far apart they are. The old 5s window
        // reset the count whenever recoveries were merely spaced >5s apart,
        // and the loader's transport chain (native XHR → page fetch → GM_xhr
        // retries) routinely takes longer than that between fatals — a
        // fatal → startLoad() → chain fails → fatal cycle never escalated.
        const hlsRecovery = { count: 0 };
        ...
        hls.on(HlsLib.Events.ERROR, (event, data) => {
          ...
          // Intercept non-fatal quota errors before the fatal gate.
          if (HlsLib.ErrorDetails) {
            var _qd = data.details;
            if (_qd === HlsLib.ErrorDetails.BUFFER_FULL_ERROR || _qd === HlsLib.ErrorDetails.BUFFER_APPEND_ERROR) {
              UVP.Overlay.handleBufferQuota('hls-' + (_qd === HlsLib.ErrorDetails.BUFFER_FULL_ERROR ? 'buffer-full' : 'buffer-append'));
              if (!data.fatal) return;
            }
          }
          if (!data.fatal) return;
          if (hlsRecovery.count >= CONFIG.hlsMaxRecoveries) {
            if (DEBUG) console.warn('[UVP] HLS.js instant recovery budget exhausted (no fragment appended between attempts) — escalating to overlay recovery');
            escalateRecovery();
            return;
          }
          hlsRecovery.count++;
        ...
        // Successful playback progress restores the instant-recovery budget:
        // FRAG_BUFFERED fires only when a fragment has been fully downloaded,
        // remuxed and appended — proof the last recovery worked (or that none
        // was needed). Without it, a fatal/recover cycle that never buffers
        // anything would never exhaust its budget; with it, only fruitless
        // recoveries accumulate toward escalation.
        hls.on(HlsLib.Events.FRAG_BUFFERED, (event, data) => {
          hlsRecovery.count = 0;
          lastFragmentFetch.at = Date.now();
          if (state.overlay === overlayForPlayback && state.video === video && state.overlayGeneration === playGeneration) {
            UVP.Overlay.markEngineReady('hls-frag-buffered');
          }
          // Ahead-window prefetch (VOD only).
          if (CONFIG.enablePlaybackPrefetch && !state.overlayIsLive && data && data.frag && hls.levels) {
            try {
              const pf = data.frag;
              const lvl = hls.levels[pf.level];
              if (lvl && lvl.details && lvl.details.fragments) {
                const frags = lvl.details.fragments;
                let fIdx = -1;
                for (let i = 0; i < frags.length; i++) { if (frags[i].sn === pf.sn) { fIdx = i; break; } }
                if (fIdx >= 0) {
                  const aheadN = Math.min(UVP.MultiTab.getPrefetchDepth(), frags.length - fIdx - 1);
                  const _gen = UVP.SegmentCache.getGeneration();
                  // Tracks issued-but-not-yet-acquired requests (semaphore is async).
                  let _pfIssued = 0;
                  for (let j = 1; j <= aheadN; j++) {
                    const nf = frags[fIdx + j];
                    if (!nf || !nf.url) continue;
                    const nUrl = UVP.PlaybackTransport.rewriteUrl(nf.url, { masterUrl: url, masterQuery, masterOrigin });
                    if (UVP.SegmentCache.has(nUrl)) continue;
                    if (UVP.SegmentCache._bytes > UVP.SegmentCache.maxBytes * 0.8) break;
                    // Connection-budget guard: leave ≥1 slot for hls.js fragment loading.
                    let _pfHost = 'global';
                    try { _pfHost = new URL(nUrl).hostname || 'global'; } catch (e) {}
                    const _pfActive = UVP.Network.getActiveHostRequests(_pfHost) + _pfIssued;
                    if (_pfActive >= CONFIG.perHostConcurrency - 1) break;
                    _pfIssued++;
                    const ctrl = (typeof AbortController === 'function') ? new AbortController() : null;
                    if (ctrl) UVP.SegmentCache.trackInFlight(nUrl, ctrl);
                    const pfScope = { aborted: false, abort() { this.aborted = true; if (ctrl) { try { ctrl.abort(); } catch (e) {} } }, check() { return !this.aborted; } };
                    UVP.Network.requestCrossOriginWithRetry(nUrl, 'arraybuffer', null, {}, 1, 0, pfScope, null)
                      .then((buf) => {
                        if (_gen !== UVP.SegmentCache.getGeneration()) return;
                        if (buf && buf.byteLength) UVP.SegmentCache.set(nUrl, buf);
                      })
                      .catch(() => {})
                      .then(() => UVP.SegmentCache.untrackInFlight(nUrl));
                  }
                }
              }
            } catch (e) { if (DEBUG) console.warn('[UVP] prefetch error:', e); }
            if (DEBUG && (UVP.SegmentCache._hits + UVP.SegmentCache._misses) % 10 === 0) {
              console.log('[UVP] SegmentCache metrics:', UVP.SegmentCache.getMetrics());
            }
          }
        });
        const stampHlsActivity = () => { UVP.Overlay.noteEngineActivity(); };
        ['FRAG_LOADING', 'FRAG_LOAD_PROGRESS', 'FRAG_LOADED', 'MANIFEST_LOADING', 'LEVEL_LOADING'].forEach((name) => {
          const ev = HlsLib.Events && HlsLib.Events[name];
          if (ev) hls.on(ev, stampHlsActivity);
        });
        // Level switch → evict stale-level segments + abort old-level prefetch.
        if (HlsLib.Events && HlsLib.Events.LEVEL_SWITCHING) {
          hls.on(HlsLib.Events.LEVEL_SWITCHING, (event, data) => {
            UVP.Overlay.noteAbrSwitch();
            if (CONFIG.enablePlaybackPrefetch && data && data.level != null) {
              try {
                const lvlUrl = hls.levels && hls.levels[data.level] && hls.levels[data.level].url;
                if (lvlUrl) UVP.SegmentCache.evictByLevelPrefix(lvlUrl);
                UVP.SegmentCache.abortInFlight();
              } catch (e) {}
            }
          });
        }
        // Max-pin guard: hls.js can still demote under starvation; re-assert
        // nextLevel/loadLevel on the segment edge so the ladder does not leave
        // a mid-rung hole while recovering.
        if (HlsLib.Events && HlsLib.Events.LEVEL_SWITCHED) {
          hls.on(HlsLib.Events.LEVEL_SWITCHED, () => {
            if (state.overlay !== overlayForPlayback || state.video !== video || state.overlayGeneration !== playGeneration) return;
            if (UVP.Utils.getPlaybackQuality() === 'max') UVP.Overlay.applyHlsQualityMode(hls, 'max');
          });
        }
        hls.loadSource(url);
        ...
        hls.on(HlsLib.Events.MANIFEST_PARSED, (event, data) => {
          if (state.overlay !== overlayForPlayback || state.video !== video || state.overlayGeneration !== playGeneration) return;
          UVP.Overlay.applyHlsQualityMode(hls);
          UVP.Overlay.resumeOrPauseVideo(video);
        });
        // Live detection: hls.js fires LEVEL_LOADED after parsing each
        // variant playlist. data.details.live is the definitive signal
        // (true when #EXT-X-ENDLIST is absent — live or truncated VOD).
        // When live is detected, set overlayIsLive (drives no-seek-restore,
        // no-save, no-state-persistence) and disable the Save button.
        // YouTube live is already tagged via ytFmtMap before playback starts,
        // so this listener fires harmlessly with live=true (already set).
        hls.on(HlsLib.Events.LEVEL_LOADED, (event, data) => {
          if (state.overlay !== overlayForPlayback || state.video !== video || state.overlayGeneration !== playGeneration) return;
          const details = data && data.details;
          if (!details) return;
          const playlistType = details.type ? String(details.type).toUpperCase() : '';
          // PLAYLIST-TYPE:VOD is finite media. details.live can still be
          // true on a truncated first parse (no ENDLIST yet).
          if (playlistType === 'VOD') {
            UVP.Overlay.exitHlsVodMisdetect();
            return;
          }
          if (details.live === true) {
            const wasLive = state.overlayIsLive;
            UVP.Overlay.enterLiveMode('hls-level-loaded');
            if (!wasLive && DEBUG) console.log('[UVP] HLS live stream detected via LEVEL_LOADED — live edge start, Save disabled');
          }
          // Live eviction: remove segments no longer in the sliding window.
          if (CONFIG.enablePlaybackPrefetch && details.live === true && details.fragments && UVP.SegmentCache._cache.size > 0) {
            try {
              const validUrls = new Set();
              for (const f of details.fragments) {
                if (f && f.url) validUrls.add(UVP.PlaybackTransport.rewriteUrl(f.url, { masterUrl: url, masterQuery, masterOrigin }));
              }
              UVP.SegmentCache.evictByUrlSet(validUrls);
            } catch (e) {}
          }
        });
      }).catch((hlsErr) => {
        ...
        // YouTube: route through failToMuxed (itag 18/22 progressive) instead
        // of video.src = url which is a dead MSE stream on Chromium.
        if (UVP.Extractors.isYouTube()) {
          if (state.dashFailover) { state.dashFailover('hls load: ' + (hlsErr && hlsErr.message ? hlsErr.message : hlsErr)); return; }
          const picked = UVP.Extractors.pickYouTubeMuxedUrl();
          if (picked) {
            try { video.src = picked; } catch (e) {}
            ...
            UVP.Overlay.showToast('HLS engine failed — playing progressive fallback');
            return;
          }
          // Live: no muxed progressive URL (format URLs expired). Native HLS
          // (video.src = manifest) is dead on Chromium — no canplay, no
          // recovery, silent stall. Trigger overlay recovery to re-extract
          // a fresh manifest and retry hls.js, bounded by maxRecoveryAttempts.
          if (UVP.Overlay.isLivePlayback()) {
            UVP.Overlay.showToast('HLS engine unavailable — recovering live stream', 4000);
            if (state.overlay && state.video === video && state.overlayGeneration === playGeneration) UVP.Overlay.triggerRecovery(true);
            return;
          }
        }
        // Generic HLS: toast + native src fallback
        UVP.Overlay.showToast('HLS engine unavailable — trying native playback', 4000);
        // try/catch matches the sibling fallback paths (muxed pick above, DASH
        // failToMuxed below): a throwing src setter on a locked-down page must
        // not escape this catch chain — the canplay listener still binds so a
        // cached/already-set src can still resume.
      ...
      // DASH. Live: HLS/direct failover, never muxed itag. VOD: muxed / safe fallback.
      const failToMuxed = (why) => {
        console.warn('[UVP] DASH playback failed over:', why);
        if (!state.overlay || state.video !== video || state.overlayGeneration !== playGeneration) return;
        state.engineWaitStartedAt = 0;
        state.engineEscalated = true;
        if (UVP.Overlay.isLivePlayback()) {
          UVP.Overlay.destroyDashSession();
          state.dashFailover = null;
          const liveFb = UVP.Overlay.pickLiveFailoverUrl(url);
          const liveFbIsDash = !!(liveFb && (UVP.Utils.isDashManifest(liveFb) || (ytFmtMap.get(liveFb) && ytFmtMap.get(liveFb).isDash)));
          if (liveFb && !liveFbIsDash) {
            ...
            if (state.recoveryAttempts > CONFIG.maxRecoveryAttempts) {
              ...
              try { sessionStorage.removeItem('uvp-player'); } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
              if (UVP.StateMachine.can('FAILED')) UVP.StateMachine.transition('FAILED', 'live DASH failover exhausted');
              UVP.Overlay.closeOverlay(false);
              return;
            }
            state.overlayUrl = liveFb;
            UVP.Overlay.showToast('Live DASH failed — switching stream', 4000);
            UVP.Overlay.playVideo(liveFb, video, 0);
            return;
          }
          UVP.Overlay.showToast('Live DASH failed — recovering', 4000);
          UVP.Overlay.triggerRecovery(true);
          return;
        }
        UVP.Overlay.destroyDashSession();
      ...
      if (typeof window.MediaSource === 'function' && typeof window.MediaSource.isTypeSupported === 'function') {
        const prepareAndStartDash = async () => {
          let externalMpdUrl = _isDash ? url : null;
          let mpdText = null;
          let mpdVideoReps = null;
          let dynamicHint = !!((_ytInfo && _ytInfo.isLive) || state.overlayIsLive || ytPlayerData.isLive);

          if (externalMpdUrl) {
            try {
              const fetched = await UVP.MPD.fetchExternalMpdText(externalMpdUrl);
              if (fetched) {
                const head = fetched.slice(0, 4000);
                const looksLive = /type\s*=\s*["']dynamic["']/i.test(head) ||
                  /isoff-live/i.test(head) ||
                  (/availabilityStartTime/i.test(head) && !/mediaPresentationDuration/i.test(head));
                if (looksLive) {
                  dynamicHint = true;
                  UVP.Overlay.enterLiveMode('external-mpd-dynamic');
                  // Keep the network MPD URL. A blob snapshot cannot refresh
                  // and the live window expires after a few seconds.
                } else {
                  mpdText = UVP.MPD.absolutizeExternalMpd(fetched, externalMpdUrl);
                  externalMpdUrl = null;
                }
              }
            } catch (e) {
              if (DEBUG) console.warn('[UVP] external MPD prefetch failed:', e);
            }
          }

          if (!_isDash && !mpdText) {
            // ytFormatsCurrent() can be true on muxed/locked-only rows.
            // DASH needs deciphered SegmentBase ranges before buildYouTubeMpd
            // or the ladder is empty and we failToMuxed too early.
            const dashMapReady = () => [...ytFmtMap.entries()].some(([u, info]) => {
              try { return UVP.MPD.usableEntry({ u: u, info: info || {} }); } catch (e) { return false; }
            });
            const mapWaitMs = Math.max(Number(CONFIG.playDecipherWaitMs) || 0, 6000);
            const mapWaitStart = Date.now();
            let _dashReExtracted = false;
            while (!dashMapReady() && (Date.now() - mapWaitStart) < mapWaitMs) {
              let _deciphered = 0;
              try { _deciphered = await UVP.Extractors.ensureYtNDeciphered(ytRouteGeneration); } catch (e) {}
              if (!state.overlay || state.video !== video || state.overlayGeneration !== playGeneration) return;
              if (dashMapReady()) break;
              // If n-decipher made no progress, the adaptive formats may not
              // have arrived yet (async clone.json().then() from the
              // intercepted /youtubei/v1/player response) or the current
              // formats may lack initRange/indexRange (mobile client URLs).
              // Force a fresh extraction from InnerTube clients to fetch
              // adaptive formats with SegmentBase ranges. This bypasses the
              // ytFormatsCurrent() short-circuit (which considers itag=18
              // muxed as "clean video" and stops early — but DASH needs
              // adaptive, not muxed).
              if (!_dashReExtracted && _deciphered === 0) {
                _dashReExtracted = true;
                if (DEBUG) {
                  const _e = [...ytFmtMap.entries()];
                  console.warn('[UVP] dashMapReady stuck — re-extracting. ytFmtMap:', {
                    total: _e.length,
                    hasN: _e.filter(([u, i]) => i && (i.hasN === true || (i.hasN !== false && /[?&]n=/.test(u)))).length,
                    hasRanges: _e.filter(([, i]) => i && i.initRange && i.indexRange).length,
                    deciphered: _e.filter(([u]) => ytDecipheredUrls.has(u)).length,
                    usable: _e.filter(([u, i]) => { try { return UVP.MPD.usableEntry({ u, info: i || {} }); } catch (e) { return false; } }).length,
                    itags: _e.map(([, i]) => i && i.itag).filter(Boolean)
                  });
                }
                // Invalidate format freshness so extractYouTubeUrls does a
                // full InnerTube client walk instead of short-circuiting.
                ytPlayerData.fmtTs = 0;
                try { await UVP.Extractors.extractYouTubeUrls(); } catch (e) {}
                if (!state.overlay || state.video !== video || state.overlayGeneration !== playGeneration) return;
                if (dashMapReady()) break;
                // Re-decipher any new URLs with n= params from the fresh extraction
            ...
            let built = null;
            try {
              built = await UVP.MPD.buildYouTubeMpd(
            ...
            // buildYouTubeMpd is now async (HDR display probe) — re-check
            // session validity after the await yield point before proceeding.
            if (!state.overlay || state.video !== video || state.overlayGeneration !== playGeneration) return;
            mpdText = built.mpd;
            mpdVideoReps = Array.isArray(built.videoReps) ? built.videoReps : null;
          }
      ...
      // try/catch matches the sibling fallback paths: a throwing src setter on
      // a locked-down page must not surface in openOverlay mid-setup; the
      // canplay listener still binds so a cached/already-set src can resume.
      try { video.src = url; } catch (e) {}
      video.addEventListener('canplay', () => { if (state.video === video && state.overlayGeneration === playGeneration) UVP.Overlay.resumeOrPauseVideo(video); }, { once: true });
    }
  }
  ...
  UVP.Overlay.recoverPlayer = function() {
    ...
    if (state.recoveryAttempts > CONFIG.maxRecoveryAttempts) {
      ...
      console.warn('[UVP] Max recovery attempts reached. Closing overlay. Disabling button for this site (session).', {
        live: UVP.Overlay.isLivePlayback(),
        hls: !!state.hls,
        dash: !!state.dashPlayer,
        warm: UVP.Overlay.isEngineWarm(),
        overlayHost: (function() { try { return state.overlayUrl ? new URL(state.overlayUrl).host : ''; } catch (e) { return ''; } })()
      });
      state.sitePlayerFailed = true;
      try { sessionStorage.setItem('uvp-failed', '1'); } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
      try { sessionStorage.removeItem('uvp-player'); } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
    ...
    let attemptIdx = -1;
    // True only when playback is actually running. Attach / empty post-reset
    // is not healthy; shouldDeferRecoveryReset keeps the poll without a reset.
    function isVideoHealthy() {
      const v = state.video;
      if (!v) return false;
      if (v.error) { if (DEBUG) console.log('[UVP] isVideoHealthy: FALSE — v.error:', v.error); return false; }
      if (!UVP.Overlay.isEngineWarm()) {
        if (DEBUG) console.log('[UVP] isVideoHealthy: FALSE — engine not warm');
        return false;
      }
      ...
      if (state.bufferHold && UVP.Overlay.isBufferHoldProgressing(v)) {
        if (DEBUG) console.log('[UVP] isVideoHealthy: TRUE (buffer hold filling)');
        return true;
      }
      if (state.bufferHold && !UVP.Overlay.isBufferHoldProgressing(v)) {
        if (DEBUG) console.log('[UVP] isVideoHealthy: FALSE — buffer hold frozen');
        return false;
      }
      ...
      if (!v.paused && !v.ended) {
        const delta = Math.abs((v.currentTime || 0) - (state.lastVideoTime || 0));
        const jumpSec = CONFIG.playheadJumpSec || 15;
        if (v.seeking) { if (DEBUG) console.log('[UVP] isVideoHealthy: TRUE (video.seeking active)'); return true; }
        if (delta < 0.1) { if (DEBUG) console.log('[UVP] isVideoHealthy: FALSE — playback time is not advancing'); return false; }
        if (delta > jumpSec) { if (DEBUG) console.log('[UVP] isVideoHealthy: FALSE — playhead discontinuity'); return false; }
      }
      if (DEBUG) console.log('[UVP] isVideoHealthy: TRUE');
      return true;
    }
    function shouldDeferRecoveryReset() {
      const v = state.video;
      if (!v || v.error) return false;
      if (UVP.Overlay.isDashInGrace()) return true;
      if (UVP.Overlay.isAttachActivityFlowing(v)) return true;
      if (UVP.Overlay.isFirstSegmentFlowing(v)) return true;
      const waited = state.engineWaitStartedAt ? (Date.now() - state.engineWaitStartedAt) : Infinity;
      const attachCap = UVP.Overlay.attachWaitCapMs();
      if ((state.hls || state.dashPlayer) && !UVP.Overlay.isEngineWarm() && !state.engineEscalated && waited < attachCap) return true;
      if (v.readyState === 0 && waited < 2000 && !state.engineEscalated) return true;
      const src = v.currentSrc || v.src || '';
      if (!src && !state.hls && !state.dashPlayer && waited < 2000 && !state.engineEscalated) return true;
      const jump = Math.abs((v.currentTime || 0) - (state.lastVideoTime || 0));
      if (jump > (CONFIG.playheadJumpSec || 15) && waited < attachCap && !state.engineEscalated) return true;
      if (v.paused && !state.wasPaused && waited < attachCap && !state.engineEscalated) return true;
      return false;
    }
    function checkAndRecover() {
      ...
      if (shouldDeferRecoveryReset()) {
        if (DEBUG) console.log('[UVP] checkAndRecover — deferred: attach/startup in progress (attempt ' + attemptIdx + ' of ' + delays.length + ')');
      ...
      const replay = async (resetVideo, resetGeneration) => {
        ...
        if (UVP.Extractors.isYouTube() && UVP.Extractors.getYtVideoId() && !UVP.Overlay.isLivePlayback()) {
          ...
          // Live: do NOT drop the durable HLS/DASH manifest — a refresh race
          // leaves only 5s SABR window URLs and the overlay dies.
          ytFmtMap.clear();
          ytDecipheredUrls.clear();
          ytNsigRewrites.clear();
          for (const u of [...capturedUrls]) {
        ...
        } else if (UVP.Extractors.isYouTube() && UVP.Extractors.getYtVideoId() && UVP.Overlay.isLivePlayback()) {
          // Keep the durable live map. Re-pick HLS/DASH so recovery does not
          // replay a dead SABR window URL that landed in overlayUrl.
          try {
            const freshLive = UVP.Extractors.pickYouTubeFormats();
            if (freshLive && freshLive.video) replayUrl = freshLive.video;
          } catch (e) { if (DEBUG) console.warn('[UVP] YouTube live re-pick failed:', e); }
        }
        ...
        if (state.overlay !== recoveryOverlay || state.video !== resetVideo || state.overlayGeneration !== resetGeneration) return;
        // Generic-site candidate ladder: with ranked alternates available,
        // recovery advances to the next untried candidate instead of
        // replaying the same dead URL until the attempt budget burns out.
        // (Dedicated-extractor sites keep their own recovery paths above.)
        if (!UVP.Extractors.isYouTube() && !UVP.Extractors.isPornhub() &&
            Array.isArray(state.playbackCandidates) && state.playbackCandidates.length > 1) {
          if (!state.playbackFailedCandidates) state.playbackFailedCandidates = new Set();
          if (replayUrl) state.playbackFailedCandidates.add(replayUrl);
          const alt = state.playbackCandidates.find(c => !state.playbackFailedCandidates.has(c));
          if (alt && alt !== replayUrl) {
            if (DEBUG) console.log('[UVP] Recovery advancing to next playback candidate:', alt);
            replayUrl = alt;
          }
        }
  ...
  UVP.Overlay.triggerRecovery = function(force) {
    if (DEBUG) console.log('[UVP] triggerRecovery called — force:', force, 'recoveryTimer active:', !!state.recoveryTimer, 'overlay:', !!state.overlay, 'video:', !!state.video, 'video error:', state.video ? state.video.error : 'no video');
    const live = UVP.Overlay.isLivePlayback();
    const hasEngine = !!(state.hls || state.dashPlayer);
    const hardError = !!(state.video && state.video.error);
    // Live + engine + no video.error: do not rebuild. t=0 empty buffer is attach, not a stall.
    if (live && hasEngine && !hardError) {
      if (DEBUG) console.log('[UVP] triggerRecovery skipped — live engine attached');
      return;
    }
    if (hasEngine && !UVP.Overlay.isEngineWarm() && !hardError) {
      if (UVP.Overlay.isAttachActivityFlowing(state.video)) {
        if (DEBUG) console.log('[UVP] triggerRecovery deferred — MSE engine not warm yet');
        return;
      }
      const waited = state.engineWaitStartedAt ? (Date.now() - state.engineWaitStartedAt) : Infinity;
      const cap = UVP.Overlay.attachWaitCapMs();
      if (!state.engineEscalated && waited < cap) {
        if (DEBUG) console.log('[UVP] triggerRecovery deferred — MSE engine not warm yet');
        return;
      }
    }
    if (state.recoveryTimer && !force) return;
    if (force && state.recoveryTimer) { clearTimeout(state.recoveryTimer); state.recoveryTimer = null; }
    UVP.Overlay.recoverPlayer();
  }
  ...
  UVP.Overlay.setupVideoHealthMonitoring = function() {
        ...
        if (act) {
          ...
          if (!act.recoveryTimer) {
            ...
            act.recoveryTimer = setTimeout(function checkNativeDashError() {
              ...
              if (UVP.Overlay.getBufferedAhead(recoveryVideo) > UVP.Overlay.dashPlayMinBufferSec() && recoveryVideo.readyState >= 3) { recoveryActivity.pendingError = false; return; }
              if (UVP.Overlay.isDashInGrace() || !recoveryActivity.engineReady) {
    ...
    state.healthTimer = setInterval(() => {
      ...
      const video = state.video;
      if (video.seeking || UVP.Overlay.isDashInGrace() || UVP.Overlay.isAbrSwitching()) {
        state.stallCount = 0;
        state.lastVideoTime = video.currentTime;
        return;
      }
      if (UVP.Overlay.isLivePlayback()) {
        // Live health does not stall-escalate. Engine retries the window.
        state.stallCount = 0;
        state.lastVideoTime = video.currentTime;
        return;
      }
      if (!UVP.Overlay.isEngineWarm()) {
        state.stallCount = 0;
        state.lastVideoTime = video.currentTime;
        if (UVP.Overlay.isAttachActivityFlowing(video) || UVP.Overlay.isFirstSegmentFlowing(video)) return;
        if (!state.engineEscalated && (state.dashPlayer || state.hls) && state.engineWaitStartedAt &&
            (Date.now() - state.engineWaitStartedAt) >= UVP.Overlay.attachWaitCapMs()) {
          state.engineEscalated = true;
          if (DEBUG) console.warn('[UVP] MSE engine never initialized — escalating');
          if (state.dashPlayer && typeof state.dashFailover === 'function') {
            state.dashFailover('engine never initialized');
          } else {
            UVP.Overlay.recoverPlayer();
          }
        }
      ...
      if (video.paused || video.ended) { state.stallCount = 0; state.lastVideoTime = video.currentTime; return; }
      ...
      if (delta < 0.1) {
        if (DEBUG && state.dashPlayer && state.dashActivity && state.dashActivity.fillSettled && state.stallCount === 0) {
          UVP.Overlay.debugDashSeekSnap('health-stall-start');
        }
        state.stallCount++;
        if (state.stallCount >= CONFIG.stallThresholdSec) {
          state.stallCount = 0;
          if (DEBUG) {
            console.log('[UVP] STALL DETECTED — stallCount threshold reached, currentTime:', video.currentTime);
            UVP.Overlay.debugDashSeekSnap('health-stall-threshold');
          }
          UVP.Overlay.triggerRecovery(true);
        }
  ...
  // Block a site video's play() while the overlay owns playback (Android
  // audio-focus guard). The shadow is self-liveness-checking: it stays active
  // only while ownerOverlay is the live overlay; on any later call it removes
  // itself and delegates to the original prototype play(). No registry is
  // needed — the block can never outlive its overlay, even for elements
  // removed from the DOM or living in closed shadow roots.
  UVP.Overlay.blockSiteVideoPlay = function(v, ownerOverlay) {
    if (!v || v.tagName !== 'VIDEO' || typeof v.play !== 'function') return;
    if (v.__uvpPlayBlocked) return;
    try {
      v.play = function () {
        if (state.overlay === ownerOverlay) return Promise.resolve();
        try { delete v.play; } catch (e) {}
        return HTMLMediaElement.prototype.play.call(v);
      };
      v.__uvpPlayBlocked = true;
    } catch (e) { if (DEBUG) console.warn('[UVP] play() block failed:', e); }
  };
  ...
  UVP.Overlay.openOverlay = function(url, opts) {
    ...
    // Create the overlay node first so pauseIntruderHandler does not close
    // over `overlay` in the TDZ. Site players often play() again during the
    // pause sweep and can fire 'playing' before a later const would init.
    ...
    pauseIntruderHandler = (e) => {
      ...
      // Android audio-focus guard: also block its play() for this overlay's
      // lifetime (see blockSiteVideoPlay) so aggressive restart loops can't
      // re-steal media focus after the first pause.
      try { UVP.Overlay.blockSiteVideoPlay(v, overlay); } catch (err) {}
    };
    ...
    shadowStyle.textContent = `video { width: 100%; height: 100%; object-fit: contain; background: #000; touch-action: pan-y; } .uvp-close { position: absolute; top: 8px; right: 8px; z-index: 3; min-width: 32px; min-height: 32px; padding: 4px; background: rgba(0,0,0,.65); color: #fff; border: 0; border-radius: 50%; font-size: 16px; line-height: 1; cursor: pointer; backdrop-filter: blur(4px); } .uvp-save { position: absolute; top: 8px; left: 8px; z-index: 3; min-width: 70px; min-height: 32px; padding: 6px 10px; background: rgba(0,0,0,.65); color: #fff; border: 0; border-radius: 6px; font: 600 11px/1 system-ui, Roboto, sans-serif; cursor: pointer; backdrop-filter: blur(4px); transition: all 0.1s ease; white-space: nowrap; text-align: center; } .uvp-close:hover, .uvp-save:not(:disabled):hover { background: rgba(0,0,0,.85); } .uvp-pip { position: absolute; top: 8px; right: 48px; z-index: 3; min-width: 32px; min-height: 32px; padding: 4px; background: rgba(0,0,0,.65); color: #fff; border: 0; border-radius: 50%; font-size: 15px; line-height: 1; cursor: pointer; backdrop-filter: blur(4px); } .uvp-pip:hover { background: rgba(0,0,0,.85); } .uvp-save:disabled { background: rgba(50,50,50,.85); cursor: not-allowed; color: #ccc; } .uvp-progress-track { position: absolute; top: 0; left: 0; right: 0; height: 4px; background: rgba(255,255,255,0.1); z-index: 3; opacity: 0; transition: opacity 0.3s; } .uvp-progress-fill { height: 100%; width: 100%; transform: scaleX(0); transform-origin: left center; background: #19c3ff; box-shadow: 0 0 8px #19c3ff; transition: transform 0.1s linear; } .uvp-close, .uvp-save, .uvp-pip { touch-action: manipulation; }`;
    ...
    // PiP button. Leaving this way is Path A (pipExitByToggle). Entering while
    // some other element in THIS document is in PiP: exit that first, then request ours.
    ...
    UVP.Overlay.bindPipEvents(vid);
    // Double-click the video to toggle fullscreen on the overlay container.
    UVP.Overlay.bindVideoEvents(vid);
    UVP.Overlay.bindScrollPassthrough(overlay);
    if (state.isDownloading) { const svBtn = shadow.querySelector('.uvp-save'); const track = shadow.querySelector('.uvp-progress-track'); const fill = shadow.querySelector('.uvp-progress-fill'); if (svBtn) { svBtn.disabled = false; svBtn.textContent = state.downloadText || '...'; } if (track) track.style.opacity = '1'; if (fill) fill.style.transform = `scaleX(${(Math.max(0, Math.min(100, state.downloadPercent || 0)) / 100).toFixed(4)})`; }
    state.overlayGeneration++;
    ...
    state._activeRgId = (state.targetVideo && UVP.Extractors.isRedgifs())
      ...
      : null;
    // Android audio-focus guard: mobile Chromium pauses the overlay video when
    // another page video calls play() (single-active-media). Site players
    // (video.js etc.) aggressively restart their stream via play() while their
    // video is in the viewport — every restart steals focus and freezes the
    // overlay until the site video is scrolled out of view. Preemptively block
    // play() on the video(s) this overlay replaced — the exact scope
    // pauseIntruderHandler already governs — for this overlay's lifetime; the
    // shadow self-heals once this overlay is gone (see blockSiteVideoPlay).
    try {
      const vidsToBlock = UVP.Utils.findAllVideos ? UVP.Utils.findAllVideos(document) : Array.from(document.querySelectorAll('video'));
      for (const v of vidsToBlock) {
        if (!v || v.tagName !== 'VIDEO') continue;
        if (!(v === state.targetVideo || UVP.Overlay.isSameAsActivePlayback(v))) continue;
        UVP.Overlay.blockSiteVideoPlay(v, overlay);
      }
    } catch (e) { if (DEBUG) console.warn('[UVP] play() block install failed:', e); }
    ...
    state.overlayIsLive = !!(ytPlayerData.isLive || (ytFmtMap.get(url) && ytFmtMap.get(url).isLive));
    state.liveDetectReason = state.overlayIsLive ? 'youtube' : null;
    if (state.overlayIsLive && state.video) {
      try { state.video.loop = false; state.video.removeAttribute('loop'); } catch (e) {}
    }
    ...
    state.recoveryAttempts = 0;
    const uvpBtn = UVP.Overlay.getPlayButton();
    if (uvpBtn) uvpBtn.style.display = 'none';
    UVP.Overlay.startOverlaySync();
    // Pre-validate the playback URL: check token expiry and do a lightweight
    // HEAD probe. If the URL is expired or returns 403/410, refresh formats
    // (YouTube) or show a toast (other sites) before attempting playback.
    // This eliminates the 8-second stall-then-recover cycle for dead URLs.
    // Runs in a background async IIFE so openOverlay stays synchronous.
    (async function() {
      // Identity guard: the validation below is async. If the user closes the
      // overlay (or a newer openOverlay supersedes it) while validation is in
      // flight, playVideo must NOT fire on the new/destroyed overlay's video.
      var ovOverlay = overlay;
      var ovVideo = state.video;
      var ovGeneration = overlayGeneration;
      var overlayStillCurrent = function() {
        return state.overlay === ovOverlay && state.video === ovVideo && state.overlayGeneration === ovGeneration;
      };
      var _preValidate = async function(urlToCheck) {
        // Manifest URLs (HLS/DASH) are durable and self-refreshing — the
        // expire param on a manifest URL is for the live/segment window, not
        // the manifest itself. Pre-validating manifest URLs causes false
        // refreshes: pickYouTubeFormats() returns a VOD adaptive format URL
        // (not the manifest), and playVideo routes it into the DASH-branch
        // buildYouTubeMpd path, which fails with "mpd empty" because live
        // streams have no approxDurationMs.
        if (UVP.Utils.isHlsManifest(urlToCheck) || UVP.Utils.isDashManifest(urlToCheck)) {
          if (DEBUG) console.log('[UVP] Pre-validation: skipping manifest URL');
          return true;
        }
        var _fmtInfo = ytFmtMap.get(urlToCheck);
        if (_fmtInfo && (_fmtInfo.isHls || _fmtInfo.isDash)) {
          if (DEBUG) console.log('[UVP] Pre-validation: skipping format-tagged manifest URL');
          return true;
        }
        // Instant expiry check (no network)
        if (UVP.Utils.isUrlExpired(urlToCheck)) {
          if (DEBUG) console.warn('[UVP] Pre-validation: URL expired, refreshing');
          return false;
        }
        // For YouTube: check if URL is about to expire (< 30s remaining)
        if (UVP.Extractors.isYouTube() && UVP.Utils.isUrlExpiringSoon(urlToCheck, 30000)) {
          if (DEBUG) console.warn('[UVP] Pre-validation: URL expiring soon, refreshing');
          return false;
        }
        // Lightweight HEAD probe (5s timeout, optimistic on CORS failure)
        var result = await UVP.Utils.validatePlaybackUrl(urlToCheck);
        if (!result.ok) {
          if (DEBUG) console.warn('[UVP] Pre-validation failed:', result.status, result.reason);
          return false;
        }
        return true;
      };
      var playbackUrl = url;
      var isValid = await _preValidate(playbackUrl);
      if (!overlayStillCurrent()) return; // overlay closed/replaced during validation
      if (!isValid) {
        // Try to refresh the URL
        if (UVP.Extractors.isYouTube()) {
          try {
            UVP.Overlay.showToast('Refreshing video URL…');
            await UVP.Extractors.extractYouTubeUrls();
            if (!overlayStillCurrent()) return; // refresh awaited — re-verify identity before mutating shared state
            var fresh = UVP.Extractors.pickYouTubeFormats();
            if (fresh && fresh.video) {
              playbackUrl = fresh.video;
              state.targetAudioUrl = fresh.audio || null;
              state.overlayUrl = playbackUrl;
              if (DEBUG) console.log('[UVP] Pre-validation: refreshed URL:', UVP.Utils.urlExpiryMs(playbackUrl) > 0 ? Math.round(UVP.Utils.urlExpiryMs(playbackUrl) / 1000) + 's remaining' : 'no expiry');
            } else {
              UVP.Overlay.showToast('Video URL unavailable');
              // Continue with original URL as last resort
            }
          } catch (e) {
            if (DEBUG) console.warn('[UVP] Pre-validation refresh failed:', e);
          }
        }
        // Non-YouTube: try a fresh extraction as a last resort
        // Don't block — continue with the original URL and let recovery handle it
      }
      if (overlayStillCurrent()) {
        UVP.Overlay.playVideo(playbackUrl, ovVideo, opts.seekTo);
        UVP.Overlay.setupVideoHealthMonitoring();
      }
    })();
  ...
  // ==================== CHATURBATE SUPPORT ====================
  UVP.Extractors.isChaturbate = function() { return /(?:^|\.)chaturbate\.com$/i.test(location.hostname); }
  // Keep the room-context master as returned (live-edge TS or llhls).
  // Rewriting live-edge → live-fhls/playlist_sfm4s reused a token issued for
  // a different path; hls.js then failed and native fallback looked like VOD.
  // Still drop video-only chunklists (no audio renditions, one rung).
  UVP.Extractors.preferChaturbateMaster = function(url) {
    if (!url) return null;
    const u = String(url);
    if (/chunklist|_video_|\baudio_aac_/i.test(u) && !/llhls\.m3u8/i.test(u)) return null;
    return UVP.Utils.assertSafeVideoUrl(u) || u;
  };

  UVP.Extractors.isVidara = function() {
    return /(?:^|\.)(?:vidara\.(?:so|to)|vidwara\.online)$/i.test(location.hostname);
  };
  UVP.Extractors.getVidaraFilecode = function(raw) {
    const s = String(raw || location.href);
    const m = s.match(/\/(?:e|v|embed)\/([A-Za-z0-9_-]{6,})/i);
    return m ? m[1] : null;
  };
  UVP.Extractors.fetchVidaraStream = function(filecode) {
    if (!filecode) return Promise.resolve(null);
    const endpoints = [
      'https://vidara.to/api/stream',
      'https://vidara.so/api/stream'
    ];
    const post = (url) => new Promise((resolve) => {
      const body = JSON.stringify({ filecode: filecode, device: 'desktop' });
      const finish = (text) => {
        try {
          const json = JSON.parse(text || '{}');
          const u = json.streaming_url || json.file || json.source || null;
          resolve(u && UVP.Utils.assertSafeVideoUrl(u));
        } catch (e) { resolve(null); }
      };
      try {
        GM_xmlhttpRequest({
          method: 'POST', url: url,
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'Origin': 'https://vidara.to', 'Referer': 'https://vidara.to/' },
          data: body, responseType: 'text', timeout: 8000,
          onload: (r) => finish(r.response || r.responseText),
    ...
    return (async () => {
      for (const ep of endpoints) {
        const u = await post(ep);
        if (u) return u;
      }
      return null;
    })();
  };
  UVP.Extractors.acceptFrameStreamUrl = function(raw, base) {
    if (!raw) return null;
    const resolved = UVP.Utils.assertSafeVideoUrl(UVP.Utils.resolveSafeUrl(raw, base || location.href));
    if (!resolved) return null;
    if (/youtu\.?be(?:\.com)?\//i.test(resolved)) return null;
    try {
      const path = new URL(resolved).pathname;
      const hasMediaExt = /\.(m3u8|mpd|mp4|m4v|webm)([?#]|$)/i.test(resolved);
      if (!hasMediaExt && /\/(?:e|embed|v|watch|player|dash-player)(?:\/|$)/i.test(path)) return null;
      if (!hasMediaExt && /\.(?:html?|aspx?)([?#]|$)/i.test(path)) return null;
    } catch (e) { return null; }
    if (/\.(m3u8|mpd|mp4|m4v|webm)([?#]|$)/i.test(resolved)) return resolved;
    if (UVP.Utils.isHlsManifest(resolved) || UVP.Utils.isDashManifest(resolved) || UVP.Utils.isDirectMedia(resolved)) return resolved;
    if (/[?&#](?:type|ext|format|scheme)=(?:hls|m3u8|mpd|dash|mp4|webm)/i.test(resolved)) return resolved;
    try {
      if (/\/(?:hls|llhls|dash|manifest|playlist|master)(?:\/|$)/i.test(new URL(resolved).pathname)) return resolved;
    } catch (e) {}
    return null;
  };
  // Parent extract from an <iframe> element: src / query stream URL,
  // same-origin JW / inner <video>, then Vidara filecode. Does not require
  // a UVP instance inside the frame.
  UVP.Extractors.getUrlFromPlayerFrame = async function(iframe) {
    if (!iframe) return null;
    let src = '';
    try { src = iframe.currentSrc || iframe.src || iframe.getAttribute('src') || iframe.getAttribute('data-src') || ''; } catch (e) {}
    if (!src) return null;
    const fromSrc = UVP.Extractors.acceptFrameStreamUrl(src, location.href);
    if (fromSrc) return fromSrc;
    try {
      const u = new URL(src, location.href);
      for (const key of ['src', 'file', 'source', 'url', 'dash', 'hls', 'mpd', 'stream']) {
        const val = u.searchParams.get(key);
        const safe = UVP.Extractors.acceptFrameStreamUrl(val, u.href);
        if (safe) return safe;
      }
    } catch (e) {}
    try {
      const innerJw = UVP.Extractors.extractJWPlayerUrls(iframe.contentWindow);
      const jwSafe = UVP.Extractors.acceptFrameStreamUrl(innerJw, location.href);
      if (jwSafe) return jwSafe;
    } catch (e) {}
    try {
      ...
      if (doc) {
        ...
        if (v) {
          const innerSrc = v.currentSrc || v.src || '';
          const fromEl = UVP.Extractors.acceptFrameStreamUrl(innerSrc, (iframe.src || location.href));
          if (fromEl) return fromEl;
          const inner = UVP.Extractors.getGenericUrl(v);
          const innerSafe = UVP.Extractors.acceptFrameStreamUrl(inner, location.href);
          if (innerSafe) return innerSafe;
        }
      }
    } catch (e) {}
    const code = UVP.Extractors.getVidaraFilecode(src);
    if (code) return UVP.Extractors.fetchVidaraStream(code);
    return null;
  };
  ...
  UVP.Extractors.SITES = [
    {
      name: 'Vidara',
      test: UVP.Extractors.isVidara,
      isSite: UVP.Extractors.isVidara,
      extract: null,
      spa: true,
      getUrl: async function() {
        UVP.Utils.harvestDeclaredMediaUrls();
        const code = UVP.Extractors.getVidaraFilecode(location.href) ||
          UVP.Extractors.getVidaraFilecode((UVP.Overlay.findPlayerIframe() || {}).src);
        if (code) {
          const streamed = await UVP.Extractors.fetchVidaraStream(code);
          if (streamed) return streamed;
        }
        const frame = UVP.Overlay.findPlayerIframe();
        if (frame) {
          const fromFrame = await UVP.Extractors.getUrlFromPlayerFrame(frame);
    ...
    {
      name: 'Pornhub',
      test: UVP.Extractors.isPornhub,
      isSite: UVP.Extractors.isPornhub,
      extract: UVP.Extractors.extractPornhubUrls,
      spa: true,
      getUrl: async function(bestVideo) {
        ...
        if (!url && bestVideo) {
          ...
          if (src && !src.startsWith('blob:') && UVP.Utils.isUsableUrl(src))
            url = UVP.Utils.resolveSafeUrl(src, location.href);
        }
    ...
    {
      name: 'SpankBang',
      test: UVP.Extractors.isSpankBang,
      isSite: UVP.Extractors.isSpankBang,
      extract: UVP.Extractors.extractSpankBangUrls,
      spa: true,
      getUrl: async function(bestVideo) {
    ...
    {
      name: 'XHamster',
      test: UVP.Extractors.isXHamster,
      isSite: UVP.Extractors.isXHamster,
      extract: UVP.Extractors.extractXHamsterUrls,
      spa: true,
      getUrl: function(bestVideo) {
    ...
    {
      name: 'XVideos',
      test: UVP.Extractors.isXVideos,
      isSite: UVP.Extractors.isXVideos,
      extract: UVP.Extractors.extractXVideosUrls,
      spa: true,
      getUrl: function(bestVideo) {
        ...
        // Use bestVideo.src BEFORE pickBestUrl — on search/grid pages this is the
        // correct preview for the clicked card. pickBestUrl() may return a stale
        // preview URL from a different card.
        if (!url && bestVideo) {
          const src = bestVideo.currentSrc || bestVideo.src || '';
          if (src && !src.startsWith('blob:') && UVP.Utils.isUsableUrl(src))
            url = UVP.Utils.resolveSafeUrl(src, location.href);
    ...
    {
      name: 'PHNetwork',
      test: UVP.Extractors.isPHNetwork,
      isSite: UVP.Extractors.isPHNetwork,
      extract: UVP.Extractors.extractPHNetworkUrls,
      spa: true,
      getUrl: function(bestVideo) {
    ...
    {
      name: 'Eporner',
      test: UVP.Extractors.isEporner,
      isSite: UVP.Extractors.isEporner,
      extract: UVP.Extractors.extractEpornerUrls,
      spa: true,
      getUrl: async function(bestVideo) {
    ...
    {
      name: 'YouTube',
      test: UVP.Extractors.isYouTube,
      isSite: UVP.Extractors.isYouTube,
      extract: null,
      spa: false,
      getUrl: async function(bestVideo) {
        // Non-watch pages have no current video id. Null here is not a toast;
        // showNativePlayer still runs generic / iframe / pickBestUrl.
        if (!UVP.Extractors.getYtVideoId()) return null;
    ...
    {
      name: 'PMVHaven',
      test: UVP.Extractors.isPMVHaven,
      isSite: UVP.Extractors.isPMVHaven,
      extract: UVP.Extractors.extractPMVHavenUrls,
      spa: true,
      getUrl: async function(bestVideo) {
        ...
        if (bestVideo) {
          const src = bestVideo.currentSrc || bestVideo.src || '';
          if (src && !src.startsWith('blob:') && UVP.Utils.isUsableUrl(src))
            return UVP.Utils.resolveSafeUrl(src, location.href);
        }
    ...
    {
      name: 'X',
      test: UVP.Extractors.isX,
      isSite: UVP.Extractors.isX,
      extract: UVP.Extractors.extractXUrls,
      spa: true,
      getUrl: async function(bestVideo) {
        ...
        if (tweetId && mediaId) {
          const hasForThisVideo = [...capturedUrls].some(u => u.includes('/' + mediaId + '/') || (/\/tweet_video\//i.test(u) && u.includes(mediaId)));
        ...
        // Scoped pick only. This extractor does not call pickBestUrl.
        // showNativePlayer may still try generic / iframe / pickBestUrl after null.
        if (mediaId) return UVP.Extractors.pickXUrl(mediaId);
        if (bestVideo && bestVideo.poster) {
          try {
            const file = new URL(bestVideo.poster, location.href).pathname.split('/').pop().replace(/\.[a-z0-9]+$/i, '');
            if (file && file.length >= 4) {
              const gifHit = UVP.Utils.extractAllUrls().find(u => {
                if (!/video\.twimg\.com/i.test(u)) return false;
                try {
                  const path = new URL(u).pathname;
                  return path.includes('/tweet_video/' + file) || path.includes('/' + file + '/') || path.split('/').pop().replace(/\.[a-z0-9]+$/i, '') === file;
                } catch (e) { return false; }
              });
              if (gifHit) return gifHit;
            }
          } catch (e) {}
        }
        return null;
    ...
    {
      name: 'Chaturbate',
      test: UVP.Extractors.isChaturbate,
      isSite: UVP.Extractors.isChaturbate,
      extract: null,
      spa: false,
      getUrl: async function(bestVideo) {
        // CMAF/LL-HLS rooms: the site's hls.js MSE player fetches a master
        // playlist (llhls.m3u8?token=<one-time JWE>) whose variants are
        // VIDEO-ONLY chunklists; audio ships as separate renditions
        // (audio_aac_96/128) referenced only from that master. The generic
        // picker's naming scan (master|index|playlist) misses "llhls.m3u8"
        // and lands on the site player's video-only chunklist_..._video_...m3u8,
        // so the overlay plays silent. Master tokens captured at page load
        // are single-use (403 claims_denied on refetch), so mint a FRESH
        // master via the same-origin room context API; the overlay's hls.js
        // then selects video + audio renditions from the full ladder itself.
        let url = null;
        try {
          const room = (location.pathname.match(/^\/([^\/?#]+)\/?/) || [])[1];
          if (room) {
            const ctrl = new AbortController();
            const abortT = setTimeout(() => ctrl.abort(), CONFIG.manifestTimeoutMs);
            const resp = await origFetch('/api/chatvideocontext/' + encodeURIComponent(room) + '/', { credentials: 'include', signal: ctrl.signal });
            clearTimeout(abortT);
            if (resp.ok) {
              const data = await resp.json();
              const src = data && data.hls_source;
              if (src && /\.m3u8([?#]|$)/i.test(src)) {
                url = UVP.Utils.assertSafeVideoUrl(UVP.Utils.resolveSafeUrl(src, location.href));
              }
            }
          }
        } catch (e) { if (DEBUG) console.warn('[UVP] Chaturbate context API failed:', e); }
        url = UVP.Extractors.preferChaturbateMaster(url);
        if (!url && bestVideo) {
          const src = bestVideo.currentSrc || bestVideo.src || '';
          if (src && !src.startsWith('blob:') && UVP.Utils.isUsableUrl(src))
            url = UVP.Extractors.preferChaturbateMaster(UVP.Utils.resolveSafeUrl(src, location.href));
        }
        if (!url) url = UVP.Extractors.preferChaturbateMaster(UVP.Utils.pickBestUrl());
        return url;
      }
    }
  ];

  // ==================== JWPLAYER EXTRACTOR (Global) ====================
  // JW Player file URLs exist only after packer eval (not interceptable).
  // Read jwplayer().getPlaylistItem() after init. Returns a playable file:
  // media extension, sources[].type (hls/dash/mp4/webm), or a path/query
  // stream hint. YouTube/image/audio-only files are not returned.
  UVP.Extractors.extractJWPlayerUrls = function(win) {
    const found = [];
    const normType = (t) => {
      const s = String(t || '').toLowerCase();
      if (/^(hls|m3u8|application\/vnd\.apple\.mpegurl)/.test(s)) return 'hls';
      if (/^(dash|mpd|application\/dash\+xml)/.test(s)) return 'dash';
      if (/^(mp4|m4v|video\/mp4)/.test(s)) return 'mp4';
      if (/^(webm|video\/webm)/.test(s)) return 'webm';
      if (/^(aac|mp3|m4a|audio)/.test(s)) return 'audio';
      return '';
    };
    const add = (file, typeHint) => {
      if (!file || typeof file !== 'string' || !/^https?:\/\//i.test(file)) return;
      const cap = UVP.Utils.captureUrl(file);
      if (cap) found.push({ url: cap, type: normType(typeHint) });
    };
    const hasStreamHint = (u) => {
      if (/[?&#](?:type|ext|format|scheme)=(?:hls|m3u8|mpd|dash|mp4|webm)/i.test(u)) return true;
      try {
        const path = new URL(u).pathname;
        return /\/(?:hls|llhls|dash|manifest|playlist|master)(?:\/|$)/i.test(path);
      } catch (e) { return false; }
    };
    const isPlayableJwFile = (entry) => {
      const u = entry.url;
      if (!UVP.Utils.assertSafeVideoUrl(u)) return false;
      if (/youtu\.?be(?:\.com)?\//i.test(u)) return false;
      if (/\.(?:jpe?g|png|gif|webp|vtt|srt)(?:[?#]|$)/i.test(u)) return false;
      if (entry.type === 'audio') return false;
      if (entry.type === 'hls' || entry.type === 'dash' || entry.type === 'mp4' || entry.type === 'webm') return true;
      if (/\.(m3u8|mpd|mp4|m4v|webm)([?#]|$)/i.test(u)) return true;
      if (UVP.Utils.isHlsManifest(u) || UVP.Utils.isDashManifest(u) || UVP.Utils.isDirectMedia(u)) return true;
      return hasStreamHint(u);
    };
    try {
      const w = win || ((typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window);
      if (!w || typeof w.jwplayer !== 'function') return null;
      const p = w.jwplayer();
      if (!p || typeof p.getPlaylistItem !== 'function') return null;
      const item = p.getPlaylistItem();
      if (item) {
        add(item.file, item.type);
        if (Array.isArray(item.sources)) {
          for (const s of item.sources) { if (s) add(s.file, s.type); }
        }
        if (Array.isArray(item.allSources)) {
          for (const s of item.allSources) { if (s) add(s.file, s.type); }
        }
      }
      if (typeof p.getPlaylist === 'function') {
        try {
          ...
          if (Array.isArray(playlist)) {
            for (const plItem of playlist) {
              if (!plItem) continue;
              add(plItem.file, plItem.type);
              if (Array.isArray(plItem.sources)) {
                for (const s of plItem.sources) { if (s) add(s.file, s.type); }
              }
            }
          }
        } catch (e) {}
    ...
    const playable = found.filter(isPlayableJwFile);
    if (!playable.length) return null;
    const mediaExt = (u) => /\.(m3u8|mpd|mp4|webm)([?#]|$)/i.test(u);
    playable.sort((a, b) => (mediaExt(b.url) - mediaExt(a.url)) || (UVP.Utils.resOf(b.url) - UVP.Utils.resOf(a.url)));
  ...
  UVP.Extractors.getGenericUrl = function(bestVideo) {
    ...
    // JW Player API (extension-less files included).
    if (!targetUrl) {
      const jwHit = UVP.Utils.safeCall(UVP.Extractors.extractJWPlayerUrls, 'extractJWPlayerUrls');
      if (jwHit) return jwHit;
    }
    ...
    // extractor. No poster match → grid-filtered pickBestUrl below.
    ...
    if (!targetUrl && posterSrc) {
      try {
        ...
        const segs = posterUrl.pathname.split('/').filter(Boolean);
        const segments = segs.filter(s => s.length >= 4 && /\d/.test(s));
        const leaf = (segs[segs.length - 1] || '').replace(/\.[a-z0-9]+$/i, '');
        if (leaf.length >= 4 && !segments.includes(leaf)) segments.push(leaf);
        if (segments.length) {
          const allUrls = UVP.Utils.extractAllUrls();
    ...
    // Grid-page guard: pickBestUrl's pool includes previously played videos.
    // Only accept that fallback if it shares identity with THIS card
    // (src/poster token). Otherwise a GIF/card with no captured stream yet
    // would replay the last video — and returning a stale URL also skipped
    // the poke-retry that could have captured the right one.
    const fallback = UVP.Utils.pickBestUrl();
    if (!fallback || !bestVideo) return bestVideo ? null : fallback;
    const tokens = [];
    const addTok = (raw) => {
      if (!raw || /^(blob|data):/i.test(raw)) return;
      try {
        const segs = new URL(raw, location.href).pathname.split('/').filter(Boolean);
        const base = (segs[segs.length - 1] || '').replace(/\.[a-z0-9]+$/i, '');
        if (base.length >= 4) tokens.push(base);
        if (segs.length >= 2 && segs[segs.length - 2].length >= 4) tokens.push(segs[segs.length - 2]);
      } catch (e) {}
    };
    addTok(bestVideo.currentSrc || bestVideo.src || '');
    addTok(bestVideo.poster || '');
    addTok(posterSrc);
    if (!tokens.length) {
      // Blob MSE / no poster: a top-level manifest on this page is usually
      // the current stream. Progressive leftovers are too often another card.
      if (fallback && (UVP.Utils.isHlsManifest(fallback) || UVP.Utils.isDashManifest(fallback))) return fallback;
      return null;
    }
    try {
      const fbPath = new URL(fallback, location.href).pathname;
      if (tokens.some(tok => fbPath.includes('/' + tok + '/') || fbPath.includes(tok))) return fallback;
    } catch (e) {}
    return null;
  };
    ...
    if (state.buttonExtracting) return;
    if (state.overlay && !state.isDetachedPip && !state.isMiniPlayer) return;
    if (state.overlay && state.buttonTargetVideo && UVP.Overlay.isSameAsActivePlayback(state.buttonTargetVideo)) return;
    if (state.overlay && state.buttonTargetFrame && state._activePlaybackFrame === state.buttonTargetFrame) return;
    let url = null;
    const _btn = UVP.Overlay.getPlayButton();
    state.buttonExtracting = true;
    if (_btn) { _btn.classList.add('uvp-loading'); _btn.style.pointerEvents = 'none'; }
    const _restoreBtn = () => {
      state.buttonExtracting = false;
      if (_btn) { _btn.classList.remove('uvp-loading'); _btn.style.pointerEvents = ''; }
    };
    ...
    const playerFrame = state.buttonTargetFrame || (!bestVideo && UVP.Overlay.findPlayerIframe()) || null;
    if (playerFrame) state._activePlaybackFrame = playerFrame;
    UVP.Utils.harvestDeclaredMediaUrls();
    ...
    // Baked-in libraries load on-demand, but an early click can still beat the
    ...
    if (UVP.Extractors.isYouTube() && !UVP.Extractors.ytFormatsCurrent()) {
      ...
      // Live streams need extra headroom: the durable HLS manifest arrives
      // only from the ANDROID InnerTube client (Tier 3), which must complete
      // a full HTTP round-trip. The default 10s gate can expire before that
      // finishes under slow network conditions, causing pickYouTubeUrl() to
      // return null (no durable manifest) → blank overlay or 5s-then-stall.
      const _gateTimeout = ytPlayerData.isLive ? CONFIG.playFormatWaitMs + 5000 : CONFIG.playFormatWaitMs;
      while (Date.now() - _gateStart < _gateTimeout) {
        if (isStale()) { _restoreBtn(); return; }
        if (!_gateToasted) { _gateToasted = true; UVP.Overlay.showToast('Loading video data…', _gateTimeout + 2000); }
    ...
    // Order: site getUrl → generic (JW/poster/grid) → declared m3u8/mpd.
    // YouTube/X hits must belong to this click (acceptClickScopedUrl).
    const takeUrl = (candidate) => UVP.Utils.acceptClickScopedUrl(candidate, bestVideo);
    const declared = (!UVP.Extractors.isYouTube()) ? UVP.Utils.getDeclaredPageStream() : null;
    const site = UVP.Extractors.SITES.find(s => (s.test || s.isSite)());
    if (site) url = takeUrl(await site.getUrl(bestVideo));
    if (!url) url = takeUrl(UVP.Extractors.getGenericUrl(bestVideo));
    if (!url && declared) url = declared;
    ...
    if (!url && bestVideo) {
      ...
      try {
        ...
        try {
          const w = (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;
          jwPresent = typeof w.jwplayer === 'function';
        } catch (e) {}
        const pokeMaxMs = isYtPoke ? 1600 : (jwPresent ? 2200 : 900);
        const pokeStart = Date.now();
        const pokeBaseline = capturedUrls.size;
        const pokeAnchor = lastInterceptedMedia.url;
        while (Date.now() - pokeStart < pokeMaxMs) {
          ...
          if (isYtPoke) {
            ...
            if (capturedUrls.size > pokeBaseline || lastInterceptedMedia.url !== pokeAnchor) break;
          }
        }
    ...
    if (!url) {
      const frame = playerFrame || UVP.Overlay.findPlayerIframe();
      if (frame) {
        try { url = await UVP.Extractors.getUrlFromPlayerFrame(frame); } catch (e) {}
      }
    }
    if (!url) {
      UVP.Utils.harvestDeclaredMediaUrls();
      UVP.Utils.invalidateUrlCache();
      url = UVP.Utils.acceptClickScopedUrl(UVP.Utils.pickBestUrl(), bestVideo);
    }
    ...
    if (UVP.Extractors.isYouTube() && url && /googlevideo\.com/i.test(url)) {
      ...
      // HLS/DASH manifests do not carry an n-param challenge. Skip the
      // n-sig decipher wait and URL rewrite — manifests are durable and
      // hls.js/dash.js manage their own segment URLs.
      if (!(_nInfo && (_nInfo.isHls || _nInfo.isDash))) {
        const _needsN = !ytDecipheredUrls.has(url) && !ytNsigRewrites.has(url) && (
          (_nInfo && _nInfo.hasN === true) || UVP.Extractors.ytUrlHasNChallenge(url)
        );
        if (_needsN) {
          try { UVP.Extractors.ensureYtNDeciphered(ytRouteGeneration); } catch (e) {}
          const _nStart = Date.now();
          while (Date.now() - _nStart < CONFIG.playDecipherWaitMs) {
            if (isStale()) return;
            if (ytDecipheredUrls.has(url) || ytNsigRewrites.has(url)) break;
            await new Promise(r => setTimeout(r, 250));
          }
        }
        if (ytNsigRewrites.has(url)) url = ytNsigRewrites.get(url);
      }
    }

    // Candidate ranking (generic sites only — dedicated extractors keep
    // their own failover paths). Rank the primary against format-class
    // alternates so recovery gets a candidate ladder to walk. NO blocking
    // probe: the previous verifyCandidates() sent a GM_xhr to each
    // candidate before playback, which (a) added up to 10s delay per
    // candidate, (b) got 403 on CDNs that reject GM_xhr probes (wrong
    // referer/cookies — e.g. lulustream), and (c) could trigger CDN
    // rate-limiting that broke the subsequent hls.js playback. The
    // primary stands; recovery advances to alternates on failure.
    if (!site && url) {
      try {
        state.playbackCandidates = UVP.Utils.rankPlaybackCandidates(url);
        state.playbackFailedCandidates = null;
      } catch (e) { if (DEBUG) console.warn('[UVP] Candidate ranking failed:', e); }
    }
  ...
  // FULL TEARDOWN pause — freeze/pagehide only (the browser is freezing or
  // unloading the page). Pauses the overlay video AND the site player and
  // stops HLS segment loading; resumeFromBackground reverses all of it.
  UVP.Boot.pauseForBackground = function() {
    ...
    // while the page is frozen/unloaded (especially Pornhub, which restarts
    // aggressively).
    [state.video, state.targetVideo].forEach(v => {
      if (v) { try { v.pause(); } catch (e) { if (DEBUG) console.warn('[UVP]', e); } }
    });
    state.suppressPlaybackStateEvents = false;
    // Stop HLS.js from fetching segments while torn down; it will be resumed
    // in resumeFromBackground().
    if (state.hls) { try { state.hls.stopLoad(); state.backgroundStoppedLoad = true; } catch (e) { if (DEBUG) console.warn('[UVP]', e); } }
    state.recoveryAttempts = 0;
  };

  // visibilitychange → hidden. Do not pause the overlay video and do not
  // stopLoad HLS: that video is the PiP source, and hiding a tab is how PiP
  // is used. Only pause the site <video> so it cannot double-speak.
  UVP.Boot.pauseForHiddenTab = function() {
    UVP.State.savePlayerState();
    UVP.Overlay.stopButtonSync();
    if (state.targetVideo) {
      state.suppressPlaybackStateEvents = true;
      try { state.targetVideo.pause(); } catch (e) { if (DEBUG) console.warn('[UVP]', e); }
      state.suppressPlaybackStateEvents = false;
    }
  };

  UVP.Boot.resumeFromBackground = function() {
    // Symmetric with pauseForBackground: only restart loading when the
    // teardown pause actually stopped it. A hidden tab keeps loading — an
    // unconditional startLoad() on a live stream that never stopped seeks to
    // the live edge and jumps the playhead.
    if (state.hls && state.backgroundStoppedLoad) { try { state.hls.startLoad(); } catch (e) { if (DEBUG) console.warn('[UVP]', e); } }
    state.backgroundStoppedLoad = false;
    const video = state.video;
    ...
    if (CONFIG.showButton) UVP.Overlay.wakeButtonEngine();
    if (state.overlay && !state.isDetachedPip) UVP.Overlay.startOverlaySync();
    UVP.State.maybeResumeDownload();
  };

  UVP.Boot.onVisibilityChange = function() {
    if (document.hidden) UVP.Boot.pauseForHiddenTab();
    else UVP.Boot.resumeFromBackground();
  };
 
  // ==================== SHARED SITE OBSERVER ====================
  // One MutationObserver + one debounce timer for ALL SPA site extractors.
  // Previously each SPA site got its own observer on document.body subtree,
  // so on a site like YouTube 4-5 simultaneous subtree observers fired on
  // every DOM mutation.  Now a single shared observer dispatches to all
  // registered extractors through one debounce timer.
  UVP.Boot._sharedSiteObs = null;
  UVP.Boot._sharedSiteTimer = null;
  UVP.Boot._siteExtractors = [];
  UVP.Boot.registerSiteExtractor = function(isSiteFn, extractFn) {
    UVP.Boot._siteExtractors.push({ isSiteFn, extractFn });
  };
  UVP.Boot.setupSiteObserver = function(isSiteFn, extractFn) {
    if (!isSiteFn()) return;
    UVP.Boot.registerSiteExtractor(isSiteFn, extractFn);
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => setTimeout(extractFn, CONFIG.siteExtractInitialDelayMs), { once: true });
    else setTimeout(extractFn, CONFIG.siteExtractInitialDelayMs);
    if (!UVP.Boot._sharedSiteObs) {
      UVP.Boot._sharedSiteObs = new MutationObserver(() => {
        if (UVP.Boot._sharedSiteTimer) return;
        UVP.Boot._sharedSiteTimer = setTimeout(() => {
          UVP.Boot._sharedSiteTimer = null;
          UVP.Utils.invalidateInlineScriptCache();
          for (const entry of UVP.Boot._siteExtractors) {
            if (entry.isSiteFn()) { try { entry.extractFn(); } catch (e) { if (DEBUG) console.warn('[UVP] site extractor failed:', e); } }
          }
        }, CONFIG.observerDebounceMs);
      });
      if (document.body) UVP.Boot._sharedSiteObs.observe(document.body, { childList: true, subtree: true });
      else document.addEventListener('DOMContentLoaded', () => { if (UVP.Boot._sharedSiteObs) UVP.Boot._sharedSiteObs.observe(document.body, { childList: true, subtree: true }); }, { once: true });
    }
  }

  // Derived from UVP.Extractors.SITES (spa + extract). Kept as an alias so
  // the test harness and any late readers still see the same list.
  const SPA_NAV_EXTRACTORS = UVP.Extractors.SITES.filter(s => s.spa && typeof s.extract === 'function');
  ...
  UVP.Boot.updateMenuCommands = function() {
    ...
    const playLabel = playQ === 'max' ? 'Max (locked top quality)' : 'Auto (ABR)';
    const id1 = UVP.Boot.registerMenuCommand(`▶ Playback Quality: [${playLabel}] (click to toggle)`, () => {
      ...
      UVP.Overlay.showToast(`UVP playback quality: ${next === 'max' ? 'Max (locked top quality)' : 'Auto (ABR)'}`);
      try {
        if (state.hls) {
          const cfg = state.hls.config;
          if (cfg) {
            const max = next === 'max';
            cfg.abrBandWidthUpFactor = max ? CONFIG.abrHlsUpSwitchFactor : 0.7;
            cfg.abrEwmaDefaultEstimate = max ? CONFIG.abrHlsStartupEstimateBps : 500000;
            cfg.maxStarvationDelay = max ? Math.max(CONFIG.abrHlsMaxStarvationDelaySec, 12) : 4;
            cfg.maxLoadingDelay = max ? Math.max(CONFIG.abrHlsMaxLoadingDelaySec, 6) : 4;
            cfg.capLevelToPlayerSize = false;
            cfg.ignoreDevicePixelRatio = true;
          }
          UVP.Overlay.applyHlsQualityMode(state.hls, next);
        } else if (state.dashPlayer) {
          try {
            UVP.Overlay.applyDashQualityMode(state.dashPlayer, next);
          } catch (e) { if (DEBUG) console.warn('[UVP] dash.js live-tune failed:', e); }
    ...
    const dlLabel = dlQ === 'muxed' ? '720p progressive, else max adaptive' : (dlQ === 'max' ? 'Max 1080p/4K (Video+Audio)' : 'Lowest 360p (Audio+Video)');
    const id2 = UVP.Boot.registerMenuCommand(`💾 Download Format: [${dlLabel}] (click to toggle)`, () => {
      ...
      const nextLabel = next === 'muxed' ? '720p progressive if available, else max adaptive' : (next === 'max' ? 'Max 1080p/4K (Separate Video + Audio)' : 'Lowest 360p (Muxed Audio + Video)');
    ...
    const id4 = UVP.Boot.registerMenuCommand(`🐞 Debug Logging: [${DEBUG ? 'ON' : 'OFF'}] (click to toggle)`, () => {
      DEBUG = !DEBUG;
      try { if (typeof GM_setValue === 'function') GM_setValue('uvp-debug', DEBUG); } catch (e) {}
    ...
    const id5 = UVP.Boot.registerMenuCommand('🔄 Re-scan page for videos', () => {
      ...
      UVP.Utils.invalidateUrlCache();
      UVP.Utils.invalidateInlineScriptCache();
      if (CONFIG.showButton) { UVP.Overlay.ensureButton(); UVP.Overlay.wakeButtonEngine(); }
      UVP.Overlay.showToast('UVP: re-scanning page');
    });
    if (id5 !== false && id5 !== null && id5 !== undefined) _menuCommandIds.push(id5);

    const _prefetchOn = CONFIG.enablePlaybackPrefetch;
    const id6 = UVP.Boot.registerMenuCommand(`⚡ Playback Prefetch: [${_prefetchOn ? 'ON' : 'OFF'}] (click to toggle)`, () => {
      CONFIG.enablePlaybackPrefetch = !CONFIG.enablePlaybackPrefetch;
      try { if (typeof GM_setValue === 'function') GM_setValue('uvp-prefetch', CONFIG.enablePlaybackPrefetch); } catch (e) {}
      if (CONFIG.enablePlaybackPrefetch) { UVP.MultiTab.init(); }
      else { UVP.SegmentCache.clear(); UVP.MultiTab.destroy(); }
      UVP.Overlay.showToast('UVP playback prefetch: ' + (CONFIG.enablePlaybackPrefetch ? 'ON' : 'OFF'));
      UVP.Boot.updateMenuCommands();
    });
    if (id6 !== false && id6 !== null && id6 !== undefined) _menuCommandIds.push(id6);
  };
  ...
  UVP.Boot.boot = function() {
    ...
    // Load persisted prefetch preference
    try {
      if (typeof GM_getValue === 'function') {
        CONFIG.enablePlaybackPrefetch = GM_getValue('uvp-prefetch', CONFIG.enablePlaybackPrefetch);
      }
    } catch (e) { if (DEBUG) console.warn('[UVP] prefetch pref load failed:', e); }
    if (CONFIG.enablePlaybackPrefetch) UVP.MultiTab.init();
    ...
    if (CONFIG.showButton) {
      ...
      setTimeout(() => {
        ...
        UVP.Overlay.ensureButton();
        UVP.Overlay.wakeButtonEngine();
        const nodeHasVideo = (n) => {
          if (!n || n.nodeType !== 1) return false;
          if (n.tagName === 'VIDEO' || n.tagName === 'IFRAME') return true;
          try { return !!(n.querySelector && n.querySelector('video, iframe')); } catch (e) { return false; }
        };
        const onLateVideo = (e) => {
          if (e && e.target && e.target.tagName === 'VIDEO' && e.target !== state.video) UVP.Overlay.scheduleRetarget();
        };
        document.addEventListener('loadedmetadata', onLateVideo, true);
        const videoAppearObs = new MutationObserver((muts) => {
          let found = false;
          for (const m of muts) {
            for (const n of m.addedNodes) {
              if (nodeHasVideo(n)) { found = true; break; }
            }
            if (found) break;
          }
          if (!found) return;
          UVP.Overlay.scheduleRetarget();
        });
        try {
          const root = document.body || document.documentElement;
          if (root) videoAppearObs.observe(root, { childList: true, subtree: true });
        } catch (e) {}
    ...
    if (UVP.Extractors.isRedgifs()) {
      let rgRetargetTimer = null;
      const rgDeriveFromVideo = (video) => {
        if (!video || video.tagName !== 'VIDEO') return;
        if (video.dataset) delete video.dataset.rgId;
        if (video.poster) UVP.Extractors.deriveRgUrlsFromMedia(video.poster);
        const src = video.currentSrc || video.src || '';
        if (src && !src.startsWith('blob:')) UVP.Extractors.deriveRgUrlsFromMedia(src);
      };
      const rgMaybeRetarget = () => {
        if (state.buttonHiddenByScroll) return;
        if (rgRetargetTimer) return;
        rgRetargetTimer = setTimeout(() => {
          rgRetargetTimer = null;
          if (!state.buttonHiddenByScroll) UVP.Overlay.retargetPlaybackUi();
        }, CONFIG.observerDebounceMs);
      };
      const rgObserver = new MutationObserver((mutations) => {
        for (const m of mutations) {
          if (m.type === 'attributes' && m.target.tagName === 'VIDEO') {
            rgDeriveFromVideo(m.target);
            rgMaybeRetarget();
          }
          if (m.type === 'childList') {
            m.addedNodes.forEach(node => {
              if (!node || node.nodeType !== 1) return;
              if (node.tagName === 'VIDEO') rgDeriveFromVideo(node);
              if (node.querySelectorAll) node.querySelectorAll('video').forEach(rgDeriveFromVideo);
            });
            rgMaybeRetarget();
          }
        }
    ...
    // Site observers: every registry entry with spa+extract (YouTube/RedGifs
    // keep their dedicated observers below / above).
    UVP.Extractors.SITES.forEach((s) => {
      if (s.spa && typeof s.extract === 'function') UVP.Boot.setupSiteObserver(s.test, s.extract);
    });
      ...
      const ytObserver = new MutationObserver(() => {
        if (location.href !== ytLastUrl) {
          ...
          if (keepFormats) {
            ...
            ytHasFormats = false; ytFmtMap.clear(); ytDecipheredUrls.clear(); ytNsigRewrites.clear(); ytPlayerData.isLive = false; ytPlayerData.videoId = null; ytPlayerData.fmtTs = 0;
    ...
    // Global JWPlayer extractor: runs on every site (not just one domain).
    // Many file-host and tube templates use JWPlayer with packer-obfuscated
    // inline scripts. The file URL exists after the packer decodes
    // and JWPlayer initializes, so we poll with a short delay + MutationObserver.
    const _jwDelay = CONFIG.siteExtractInitialDelayMs + 1000; // 1.5s after DOM ready
    setTimeout(() => {
      UVP.Utils.safeCall(UVP.Extractors.extractJWPlayerUrls, 'extractJWPlayerUrls');
      let _jwTimer = null;
      const _jwObserver = new MutationObserver(() => {
      ...
      if (document.body) _jwObserver.observe(document.body, { childList: true, subtree: true });
      setTimeout(() => { try { _jwObserver.disconnect(); } catch (e) {} }, 30000);
    }, _jwDelay);

    console.log('%c[UVP] Native Overlay Universal Player ready. 5.0.36.0', 'color:#19c3ff;font-weight:bold');
  }
  ...
  const _safeBoot = () => {
    if (_booted) return;
    _booted = true;
    try { UVP.Boot.boot(); } catch (e) { console.error('[UVP] boot failed:', e); }
  };
