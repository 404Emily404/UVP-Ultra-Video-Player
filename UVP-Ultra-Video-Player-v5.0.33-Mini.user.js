// ==UserScript==
// @name         UVP Ultra Video Player v5.0.33 Mini
...
(function () {
  ...
  let DEBUG = true; // default on; still toggled at runtime via the "UVP: Toggle debug logging" menu command
  ...
  const CONFIG = {
    ...
    hostGmOnlyTtlMs: 30000,       // re-probe page-transport this long after pinning a host to GM_xhr; a single transient CORS/HTTP failure must not stick for the session
    ...
    dashSeekMaxGraceMs: 45000,   // DASH seek/wait grace only (isDashInGrace). Not the MSE attach watchdog.
    ...
    // ---- Engine-owned ABR: configure hls.js/dash.js, never pin levels ----
    // A pin (hls.js nextLevel/currentLevel, dash.js manual quality) disables
    // the engine's ABR until reset. 'max' tunes the stock engines to seek the
    // top rung aggressively; 'auto' keeps stock engine defaults.
    abrHlsUpSwitchFactor: 0.9,          // stock abrBandWidthUpFactor 0.7 demands 1.43x headroom before climbing
    ...
    abrHlsStartupEstimateBps: 3000000,  // stock abrEwmaDefaultEstimate (500kbps) starts at the bottom rung
    ...
    abrHlsLiveMaxBufferSec: 30,         // stock hls.js maxBufferLength. Must stay > live sync (~3 segments).
    abrHlsLiveMaxMaxBufferSec: 36,      // bounded cap; stock 600
    abrHlsLiveBackBufferSec: 24,        // bounded; stock liveMaxBackBufferLength is Infinity
    abrDashSafetyFactor: 0.9,          // 'max' mode dash.js throughput bandwidthSafetyFactor (stock tuning 0.85)
    abrDashInitBitrateFraction: 0.6,   // startup rung = highest ladder bitrate <= 60% of the top rung
    ...
    window.addEventListener('pointermove', (e) => {
      ...
      if (!state.buttonSyncFrame) {
        pointerUpdateTimer = requestAnimationFrame(() => {
          ...
          if (!state.overlay && !state.buttonHiddenByScroll) UVP.Overlay.retargetPlaybackUi();
        });
        return;
      }
      const pad = 40;
  ...
  UVP.Boot.onSpaNav = async function() {
    ...
    if (UVP.Extractors.isYouTube()) {
      ...
      if (!UVP.Extractors.ytFormatsCurrent()) {
        ytFmtMap.clear(); ytDecipheredUrls.clear(); ytPlayerData.isLive = false; ytPlayerData.videoId = null; ytPlayerData.fmtTs = 0;
...
`;
  ...
  // 'max' tunes the stock hls.js/dash.js ABR to aggressively pursue the top
  // sustainable rung; 'auto' keeps stock engine defaults. Both keep ABR
  // engine-owned (no level pinning); 'max' additionally arms the engine's
  // native decode-overload guard (hls.js FPS cap). Toggled from the
  // userscript menu.
...
UVP.Extractors.extractSpankBangUrls = function() {
 ...
 // Stage 2: POST to /api/videos/stream with the data-streamkey to get a JSON
  ...
  const ytVideoCodecRank = (mime) => {
    // Same story as MPD_CODEC_PREF: at equal height, VP9 > AV1/HEVC > AVC.
  ...
  // Parent-document embed-src test. Scores which iframe gets the ▶ when
  // this document has no host <video>. Not a child-frame boot detector.
  ...
  // Largest visible iframe in THIS document. Embedish src outranks area.
  // Visibility uses the iframe box on the parent page, not the frame's
  // inner layout. Same-origin content is read later by getUrlFromPlayerFrame.
  UVP.Overlay.findPlayerIframe = function() {
    ...
    for (const f of list) {
      ...
      const embedish = UVP.Overlay.frameSrcLooksLikePlayer(src);
      const score = embedish ? (1e9 + area) : area;
      if (score > bestArea) { bestArea = score; best = f; }
    }
    return best;
  };
  ...
  UVP.Overlay.updateButtonPosition = function() {
    ...
    state.buttonUpdateFrame = requestAnimationFrame(() => {
      ...
      const btn = UVP.Overlay.getPlayButton();
      if (!btn) return;
      if (state.sitePlayerFailed || (!state.overlay && state.buttonDismissed)) { btn.style.display = 'none'; return; }
      if (state.overlay && !state.isDetachedPip && !state.isMiniPlayer) { btn.style.display = 'none'; return; }
      UVP.Overlay.rescanButtonTargets();
      UVP.Overlay.applyButtonGeometry();
    });
  }
  ...
  // target box, plus scroll / resize / visualViewport. One rAF applies the
  // current rect. No 10 Hz button loop and no perpetual overlay rAF.
  ...
  UVP.Overlay.scheduleGeometry = function() {
    if (UVP.Overlay._geomRaf) return;
    UVP.Overlay._geomRaf = requestAnimationFrame(() => {
      UVP.Overlay._geomRaf = null;
  ...
  UVP.PlaybackTransport = {
    ...
    makeDashHTTPLoader(DashLib) {
      ...
      function UvpDashHTTPLoader() {
        ...
        function load(config) {
          ...
          const resourceType = PT.dashResourceType(_request, _url);
          const type = responseType === 'arraybuffer' ? 'arraybuffer' : 'text';
          const reqTimeout = (_request && (_request.requestTimeout || _request.timeout)) || 0;
          const isManifestUrl = resourceType === 'mpd' || /\.(?:mpd|m3u8)(?:[?#]|$)/i.test(_url || '');
          const timeoutMs = reqTimeout > 0 ? reqTimeout
          ...
          const finishError = (errorType, statusText) => {
            ...
            try { _scope.abort(); } catch (e) {}
            PT.logTransportDiag({
              phase: 3, transport: 'gm-or-page', resourceType, method,
              status: /timeout/i.test(String(errorType || '')) ? 0 : 0,
              originalUrl: _url, rewrittenUrl: _url, finalUrl: _url, responseType: type,
              retryDecision: errorType || 'fail', host: _host || '',
              hostPinned: PT.isHostGmOnly(_host), timeout: timeoutMs,
              rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range)
            });
          ...
          const tryNativeXhr = () => {
            try {
              ...
              _xhr.timeout = timeoutMs;
              for (const k in reqHeaders) { try { _xhr.setRequestHeader(k, reqHeaders[k]); } catch (e) {} }
              _xhr.onload = () => {
                if (_xhr.status >= 200 && _xhr.status < 300) {
                  if (!PT.isPlausiblePlaybackBody(_xhr.response, type, resourceType)) {
                    PT.logTransportDiag({
                      phase: 1, transport: 'xhr', resourceType, method, status: _xhr.status,
                      originalUrl: _url, rewrittenUrl: _url, finalUrl: _xhr.responseURL || _url,
                      responseType: type, retryDecision: 'phase2-page', host: _host || '',
                      hostPinned: false, timeout: timeoutMs,
                      rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range),
                      bodyClass: (function () {
                        const kind = PT.classifyBody(_xhr.response, type);
                        return typeof kind === 'string' ? kind : ((kind && kind.textPrefix) || '');
                      })()
                    });
                    startFallback();
                    return;
              ...
              _xhr.onerror = () => {
                PT.logTransportDiag({
                  phase: 1, transport: 'xhr', resourceType, method, status: 0,
                  originalUrl: _url, rewrittenUrl: _url, finalUrl: _url, responseType: type,
                  retryDecision: 'phase2-page', host: _host || '', hostPinned: false,
                  timeout: timeoutMs, rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range)
                });
                startFallback();
              };
              _xhr.ontimeout = () => {
                PT.logTransportDiag({
                  phase: 1, transport: 'xhr', resourceType, method, status: 0,
                  originalUrl: _url, rewrittenUrl: _url, finalUrl: _url, responseType: type,
                  retryDecision: 'phase2-page-timeout', host: _host || '', hostPinned: false,
                  timeout: timeoutMs, rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range)
                });
                startFallback();
              };
          ...
          // Phase 2: page-transport fetch → Phase 3 GM-then-page
          const startFallback = () => {
            if (fallbackStarted) return;
            fallbackStarted = true;
            ...
            const deliverOrReject = (data, transport) => {
              if (!PT.isPlausiblePlaybackBody(data, type, resourceType)) {
                const kind = PT.classifyBody(data, type);
                PT.logTransportDiag({
                  phase: transport === 'page' ? 2 : 3, transport, resourceType, method, status: 200,
                  originalUrl: _url, rewrittenUrl: _url, finalUrl: _url, responseType: type,
                  retryDecision: transport === 'page' ? 'phase3-gm' : 'reject-invalid-body',
                  host: _host || '', hostPinned: PT.isHostGmOnly(_host), timeout: timeoutMs,
                  rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range),
                  bodyClass: typeof kind === 'string' ? kind : ((kind && kind.textPrefix) || '')
                });
                if (transport === 'page') { PT.markHostGmOnly(_host); runGmEngine(); return; }
                finishError('download', 'invalid playback body');
                return;
              }
              finishSuccess(data, 'OK', _url);
            };
            const runGmEngine = () => {
              UVP.Network.requestCrossOriginWithRetry(_url, type, onProgress, reqHeaders, 2, timeoutMs, _scope, null)
                .then((data) => deliverOrReject(data, 'gm'))
                .catch((err) => {
                  PT.clearHostGmOnly(_host);
                  const msg = UVP.Network.gmErrorText(err);
            ...
            if (PT.isHostGmOnly(_host)) {
              ...
              UVP.Network.requestPageContext(_url, type, reqHeaders, onProgress, _scope, null)
                .then((data) => {
                  PT.clearHostGmOnly(_host);
                  deliverOrReject(data, 'page');
                })
                .catch((err) => {
                  PT.logTransportDiag({
                    phase: 2, transport: 'page', resourceType, method,
                    status: (err && typeof err.status === 'number') ? err.status : 0,
                    originalUrl: _url, rewrittenUrl: _url, finalUrl: _url, responseType: type,
                    retryDecision: 'phase3-gm', host: _host || '', hostPinned: false,
                    timeout: timeoutMs, rangePresent: !!(reqHeaders['Range'] || reqHeaders['range'] || _request.range)
    ...
    // Part 1: no-op XHRLoader child for FactoryMaker.extend('XHRLoader', fn, true).
    // Raw constructor only — not getClassFactory(). FactoryMaker override
    // copies our methods ONTO parent, so parent.load becomes this wrapper.
    // Capture the stock functions before that assign or load recurses.
      ...
      function UvpDashXHRLoader() {
        ...
        return {
          load: function (commonMediaRequest, commonMediaResponse) {
            ...
            return parentLoad.call(parent, commonMediaRequest, commonMediaResponse);
          },
          abort: function () {
  ...
  UVP.Overlay.startDashPlayback = async function(options) {
    ...
    try {
      ...
      // Part 1: prove FactoryMaker XHRLoader hook. Parent-delegate only.
      // HTTPLoader extend is left in place so this part cannot change
      // two attach points at once.
      try {
      ...
      const isStructuralError = (code, msg, type) => {
        ...
        // retried by dash.js + our HTTPLoader. Only unrecoverable parse /
  ...
  UVP.Overlay.applyHlsBufferPolicy = function(live) {
    ...
    if (live) {
      ...
      cfg.backBufferLength = CONFIG.abrHlsBackBufferSec;
      cfg.liveMaxBackBufferLength = CONFIG.abrHlsLiveBackBufferSec;
    }
  };
  // Stock live shape: sync = 3 target-durations, buffer larger than that,
  // max latency Infinity so hls.js will not seek to the edge. Do not set
  // liveSyncDuration (seconds) — that overrides HOLD-BACK and fights the
  // buffer if it is shorter than maxBufferLength.
  UVP.Overlay.applyLiveHlsSyncNoSkip = function(cfg) {
    if (!cfg) return;
    try { delete cfg.liveSyncDuration; } catch (e) { cfg.liveSyncDuration = undefined; }
    cfg.liveSyncDurationCount = 3;
    cfg.liveMaxLatencyDuration = Infinity;
  ...
  UVP.Overlay.applyDeferredSeek = function(video) {
    ...
    if (state.dashActivity) {
      ...
      state.dashActivity.lastProgressTime = Date.now();
    }
  ...
  UVP.Overlay.playVideo = function(url, video, seekTo) {
    ...
    if (_isHls && UVP.Overlay.shouldUseHlsJs(video)) {
      UVP.Overlay.loadHlsScript().then(HlsLib => {
        ...
        const hlsOpts = {
          maxBufferLength: 60, maxMaxBufferLength: 120, enableWorker: true,
          ...
          ...(UVP.Utils.getPlaybackQuality() === 'max' ? {
            ...
            maxStarvationDelay: CONFIG.abrHlsMaxStarvationDelaySec,
            maxLoadingDelay: CONFIG.abrHlsMaxLoadingDelaySec
          } : {}),
          ...
          // liveMaxBackBufferLength is what liveDurationInfinity actually
          // flushes against (stock Infinity = unbounded live back-buffer).
          backBufferLength: CONFIG.abrHlsBackBufferSec,
          liveMaxBackBufferLength: CONFIG.abrHlsLiveBackBufferSec,
        ...
        if (HlsLib.DefaultConfig && HlsLib.DefaultConfig.loader) {
          const EngineLoader = UVP.PlaybackTransport.makeLoader(HlsLib, { masterUrl: url, masterQuery, masterOrigin });
          hlsOpts.pLoader = EngineLoader;
          hlsOpts.fLoader = EngineLoader;
          hlsOpts.kLoader = EngineLoader;
        }
        ...
        hls.on(HlsLib.Events.MANIFEST_PARSED, (event, data) => {
          // ABR stays engine-owned end to end: hls.js picks the start level
          // from its bandwidth estimate (tuned via abrEwmaDefaultEstimate in
          // 'max' mode) and adapts on measured throughput. No forced top
          // start, no pinning - mode differences live entirely in constructor
          // config.
  ...
  UVP.Boot.updateMenuCommands = function() {
    ...
    const playLabel = playQ === 'max' ? 'Max (aggressive ABR)' : 'Auto (stock ABR)';
    const id1 = UVP.Boot.registerMenuCommand(`▶ Playback Quality: [${playLabel}] (click to toggle)`, () => {
      ...
      UVP.Overlay.showToast(`UVP playback quality: ${next === 'max' ? 'Max (aggressive ABR tuning)' : 'Auto (stock engine ABR)'}`);
      try {
        if (state.hls) {
          // Live-tune the running engine's ABR knobs only - the engine stays
          // in control (a nextLevel/currentLevel pin would disable ABR until
          // reset to -1). Buffer sizing and start level stay constructor-only.
          const cfg = state.hls.config;
          if (cfg) {
            const max = next === 'max';
            ...
            cfg.maxStarvationDelay = max ? CONFIG.abrHlsMaxStarvationDelaySec : 4;
            cfg.maxLoadingDelay = max ? CONFIG.abrHlsMaxLoadingDelaySec : 4;
            ...
            state.dashPlayer.updateSettings({
              streaming: {
                abr: {
                  limitBitrateByPortal: false,
                  usePixelRatioInLimitBitrateByPortal: false,
                  throughput: { bandwidthSafetyFactor: max ? CONFIG.abrDashSafetyFactor : 0.85 }
                }
              }
            });
  ...
  UVP.Boot.boot = function() {
    ...
    if (CONFIG.showButton) {
      ...
      setTimeout(() => {
        ...
        const onLateVideo = (e) => {
          if (e && e.target && e.target.tagName === 'VIDEO' && e.target !== state.video) UVP.Overlay.retargetPlaybackUi();
        };
